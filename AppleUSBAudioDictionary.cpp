...
/*
    ...
    DJMConfigurationDictionary is the entry point for accessing all of this stored data.
    Directly calling any of the other objects is unnecessary. First allocate a 
	DJMConfigurationDictionary and call its init function which will do the parsing of 
	the configuration descriptor and create the other objects as necessary.  Once the 
	DJMConfigurationDictionary has been init'ed, call getNumStreamInterfaces, 
    ...
    DJMConfigurationDictionary contains arrays of control and stream interfaces.  As the 
...
#pragma mark DJM01AudioDictionary
/* ------------------------------------------------------
    DJM01AudioDictionary
------------------------------------------------------ */
OSDefineMetaClassAndStructors (DJM01AudioDictionary, OSDictionary);

OSArray * DJM01AudioDictionary::getDictionaryArray (const char * key)
{
	...
	// debugIOLog ("? DJM01AudioDictionary[%p]::getDictionaryArray (%s) = %p", this, key, dictionaryArray);
	return dictionaryArray;
}

IOReturn DJM01AudioDictionary::getDictionaryValue (const char * key, bool * value)
{
	OSObject *		dictionaryObject = NULL;
...
Exit:
	...
	{
		debugIOLog ("! DJM01AudioDictionary[%p]::getDictionaryValue (%s, %p = %d) = 0x%x", this, key, value, * value, result);
	}
	return result;
}

IOReturn DJM01AudioDictionary::getDictionaryValue (const char * key, UInt8 * value)
{
	OSObject *		dictionaryObject = NULL;
...
IOReturn DJM01AudioDictionary::getDictionaryValue (const char * key, UInt16 * value)
{
	OSObject *		dictionaryObject = NULL;
	OSNumber *		dictionaryNumber = NULL;
	IOReturn		result = kIOReturnError;
...
IOReturn DJM01AudioDictionary::getDictionaryValue (const char * key, UInt32 * value)
{
	OSObject *		dictionaryObject = NULL;
	OSNumber *		dictionaryNumber = NULL;
	IOReturn		result = kIOReturnError;
...
Exit:
	if (value && result != kIOReturnSuccess)
	{
		debugIOLog ("! DJM01AudioDictionary[%p]::getDictionaryValue (%s, %p = %lu) = 0x%x", this, key, value, * value, result);
	}
	return result;
}

// The following method is necessary to prepare an OSDictionary subclass for use after being allocated with "new."
bool DJM01AudioDictionary::initDictionaryForUse ()
{
	return initWithCapacity (1);
}

void DJM01AudioDictionary::logDescriptor (UInt8 * descriptor, UInt8 length)
{
...
IOReturn DJM01AudioDictionary::setDictionaryObjectAndRelease (const char * key, OSObject * object)
{
	IOReturn		result = kIOReturnError;
...
Exit:
	...
	debugIOLog ("? DJM01AudioDictionary[%p]::setDictionaryObject (%s, %p) = 0x%x", this, key, object, result);
	return result;
}

IOReturn DJM01AudioDictionary::setDictionaryValue (const char * key, bool value)
{
	OSBoolean *		dictionaryBoolean = NULL;
	IOReturn		result = kIOReturnError;
...
Exit:
	debugIOLog ("? DJM01AudioDictionary[%p]::setDictionaryValue (%s, %d) = 0x%x", this, key, value, result);
	return result;

}

IOReturn DJM01AudioDictionary::setDictionaryValue (const char * key, UInt8 value)
{
	OSNumber *		dictionaryNumber = NULL;
	IOReturn		result = kIOReturnError;
...
IOReturn DJM01AudioDictionary::setDictionaryValue (const char * key, UInt16 value)
{
	OSNumber *		dictionaryNumber = NULL;
	IOReturn		result = kIOReturnError;
...
IOReturn DJM01AudioDictionary::setDictionaryValue (const char * key, UInt32 value)
{
	OSNumber *		dictionaryNumber = NULL;
	IOReturn		result = kIOReturnError;
...
Exit:
	debugIOLog ("? DJM01AudioDictionary[%p]::setDictionaryValue (%s, %ld) = 0x%x", this, key, value, result);
	return result;

}

#pragma mark DJMConfigurationDictionary
/* ------------------------------------------------------
    DJMConfigurationDictionary
------------------------------------------------------ */
OSDefineMetaClassAndStructors (DJMConfigurationDictionary, DJM01AudioDictionary);

IOReturn DJMConfigurationDictionary::getIsocEndpointAddress (UInt8 * address, UInt8 interfaceNum, UInt8 altSettingID, UInt8 direction) 
{
...
bool DJMConfigurationDictionary::alternateSettingZeroCanStream (UInt8 interfaceNum)
{
	return true;
}

bool DJMConfigurationDictionary::asEndpointHasSampleFreqControl (UInt8 interfaceNum, UInt8 altSettingID) 
{
    return true;
}

IOReturn DJMConfigurationDictionary::getFormat (UInt16 * format, UInt8 interfaceNum, UInt8 altSettingID)
{
	* format = PCM;
	return kIOReturnSuccess;
}
...
OSArray * DJMConfigurationDictionary::getSampleRates (UInt8 interfaceNum, UInt8 altSettingID) 
{
	if( sampleRates==NULL ) {
...
IOReturn DJMConfigurationDictionary::getNumSampleRates (UInt8 * numSampleRates, UInt8 interfaceNum, UInt8 altSettingID) 
{
    * numSampleRates = 3;
    return kIOReturnSuccess;
}

IOReturn DJMConfigurationDictionary::getBitResolution (UInt8 * sampleSize, UInt8 interfaceNum, UInt8 altSettingID) {
    * sampleSize = 24;
    return kIOReturnSuccess;
}

IOReturn DJMConfigurationDictionary::getSubframeSize (UInt8 * subframeSize, UInt8 interfaceNum, UInt8 altSettingID) 
{
    * subframeSize = 3;
    return kIOReturnSuccess;
}

IOReturn DJMConfigurationDictionary::getIsocEndpointDirection (UInt8 * direction, UInt8 interfaceNum, UInt8 altSettingID) 
{
	* direction = 0xFF;
...
IOReturn DJMConfigurationDictionary::getNumChannels (UInt8 * numChannels, UInt8 interfaceNum, UInt8 altSettingID) 
{
	* numChannels = 8;
    return kIOReturnSuccess;
}
...
// [rdar://4867779]
IOReturn DJMConfigurationDictionary::addSampleRatesToStreamDictionary ( OSArray * sampleRates, UInt8 streamInterface, UInt8 altSetting )
{
	IOReturn						result = kIOReturnError;
	DJM01StreamDictionary *			thisStream = NULL;
...
bool DJMConfigurationDictionary::asEndpointHasMaxPacketsOnly (UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	bool						result = false;
...
IOReturn DJMConfigurationDictionary::asEndpointGetLockDelay (UInt8 * lockDelay, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::asEndpointGetLockDelayUnits (UInt8 * lockDelayUnits, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
bool DJMConfigurationDictionary::asEndpointHasPitchControl (UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	bool						result = false;
...
bool DJMConfigurationDictionary::channelHasMuteControl (UInt8 interfaceNum, UInt8 altSettingID, UInt8 featureUnitID, UInt8 channelNum) 
{
    DJM01ControlDictionary *		thisControl;
...
bool DJMConfigurationDictionary::channelHasVolumeControl (UInt8 interfaceNum, UInt8 altSettingID, UInt8 featureUnitID, UInt8 channelNum) 
{
    DJM01ControlDictionary *		thisControl;
    bool						returnBool = false;
	...
	{
		debugIOLog ( "! DJMConfigurationDictionary::channelHasVolumeControl () - no control for interface %d alt setting %d", interfaceNum, altSettingID );
	}
...
// [rdar://4867779]
bool DJMConfigurationDictionary::clockSourceHasFrequencyControl (UInt8 interfaceNum, UInt8 altSetting, UInt8 clockSourceID, bool isProgrammable)
{
    DJM01ControlDictionary *		thisControl;
	bool						returnBool = false;
	...
	{
		debugIOLog ( "! DJMConfigurationDictionary::clockSourceHasFrequencyControl () - no control for interface %d alt setting %d programmable %d", interfaceNum, altSetting, isProgrammable );
	}
...
// <rdar://7446555>
bool DJMConfigurationDictionary::clockSourceHasValidityControl (UInt8 interfaceNum, UInt8 altSetting, UInt8 clockSourceID)
{
    DJM01ControlDictionary *		thisControl;
	bool						returnBool = false;
	...
	{
		debugIOLog ( "! DJMConfigurationDictionary::clockSourceHasValidityControl () - no control for interface %d alt setting %d", interfaceNum, altSetting );
	}
	
    return returnBool;
}

IOReturn DJMConfigurationDictionary::getADCVersion (UInt16 * adcVersion)
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
DJMConfigurationDictionary * DJMConfigurationDictionary::create (const IOUSBConfigurationDescriptor * newConfigurationDescriptor, UInt8 controlInterfaceNum) 
{
    DJMConfigurationDictionary *			configDictionary = NULL;

	debugIOLog ("+ DJMConfigurationDictionary::create (%p, %d)", newConfigurationDescriptor, controlInterfaceNum);
	FailIf (NULL == newConfigurationDescriptor, Exit);
    FailIf (NULL == (configDictionary =  new DJMConfigurationDictionary), Exit);
	RELEASE_IF_FALSE (configDictionary, configDictionary->init(newConfigurationDescriptor, controlInterfaceNum));

Exit:
	debugIOLog ("- DJMConfigurationDictionary::create () = %p", configDictionary);
    return configDictionary;
}

#if DEBUGLOGGING
void DJMConfigurationDictionary::dumpConfigMemoryToIOLog (IOUSBConfigurationDescriptor * configurationDescriptor) 
{
	UInt8 *		descriptorPtr;
	UInt8 *		finalPtr;
...
IOReturn DJMConfigurationDictionary::getNextAltSettingWithNumChannels (UInt8 * altSettingID, UInt8 interfaceNum, UInt8 startingAltSettingID, UInt8 numChannelsRequested) 
{
	IOReturn						result = kIOReturnError;
...
Exit:
	...
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::getNextAltSettingWithNumChannels (%p, %d, %d, %d) = 0x%x, choosing altSetting %d", 
					this, altSettingID, interfaceNum, startingAltSettingID, numChannelsRequested, result, * altSettingID);
	}
	else
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::getNextAltSettingWithNumChannels (%p, %d, %d, %d) = 0x%x, not found", 
					this, altSettingID, interfaceNum, startingAltSettingID, numChannelsRequested, result);
	}
	return result;
}

IOReturn DJMConfigurationDictionary::getNextAltSettingWithSampleSize (UInt8 * altSettingID, UInt8 interfaceNum, UInt8 startingAltSettingID, UInt8 sampleSizeRequested) 
{
	IOReturn						result = kIOReturnError;
	UInt8							numAltSettings;
	UInt8							sampleSize;
...
Exit:
	if (		( altSettingID )
			&&	( result == kIOReturnSuccess ) )
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::getNextAltSettingWithSampleSize (%p, %d, %d, %d) = 0x%x, choosing altSetting %d", 
					this, altSettingID, interfaceNum, startingAltSettingID, sampleSizeRequested, result, * altSettingID);
	}
	else
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::getNextAltSettingWithSampleSize (%p, %d, %d, %d) = 0x%x,not found", 
					this, altSettingID, interfaceNum, startingAltSettingID, sampleSizeRequested, result);
	}
	return result;
}

IOReturn DJMConfigurationDictionary::getNextAltSettingWithSampleRate (UInt8 * altSettingID, UInt8 interfaceNum, UInt8 startingAltSettingID, UInt32 sampleRateRequested) 
{
	IOReturn						result = kIOReturnError;
	UInt8							numAltSettings;
	UInt8							altSettingIndex;
...
Exit:
	if (		( altSettingID )
			&&	( kIOReturnSuccess == result ) )
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::getNextAltSettingWithSampleRate (%p, %d, %d, %lu) = 0x%x, choosing altSetting %d", 
					this, altSettingID, interfaceNum, startingAltSettingID, sampleRateRequested, result, * altSettingID);
	}
	else
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::getNextAltSettingWithSampleRate (%p, %d, %d, %lu) = 0x%x, not found", 
					this, altSettingID, interfaceNum, startingAltSettingID, sampleRateRequested, result);
	}
	return result;
}

IOReturn DJMConfigurationDictionary::getAltSettingWithSettings (UInt8 * altSettingID, UInt8 interfaceNum, UInt8 numChannels, UInt8 sampleSize, UInt32 sampleRate) 
{
	IOReturn		result = kIOReturnError;
//(mitani)
#if 1
	* altSettingID = 1;
	...
	debugIOLog ("+ DJMConfigurationDictionary[%p]::getAltSettingWithSettings (%p, %d, %d, %d, %lu)", this, altSettingID, interfaceNum, numChannels, sampleSize, sampleRate);
				...
				&&	(potentialAltSetting != 255)) 
	{
		...
		{	
			...
			{
				debugIOLog ("? DJMConfigurationDictionary[%p]::getAltSettingWithSettings () - No alternate setting ID for interface %d, %d channels. Terminating loop.", this, interfaceNum, numChannels);
				break;		// Done, so break out of while loop
			}
			else
			{
				debugIOLog ("? DJMConfigurationDictionary[%p]::getAltSettingWithSettings () - Examining alt interface %d.", this, thisAltSetting);
	...
	debugIOLog ("- DJMConfigurationDictionary[%p]::getAltSettingWithSettings (%p = %d, %d, %d, %d, %lu) = 0x%x", this, altSettingID, * altSettingID, interfaceNum, numChannels, sampleSize, sampleRate, result);
#endif
	return result;
}

IOReturn DJMConfigurationDictionary::getAC3BSID (UInt32 * ac3BSID, UInt8 interfaceNum, UInt8 altSettingID) 
{
	DJM01StreamDictionary * 		thisStream;
	IOReturn					result = kIOReturnError;
	...
	*ac3BSID = 0;
	...
	{
		debugIOLog ("! DJMConfigurationDictionary[%p]::getAC3BSID (%p, %d, %d) - Could not get stream interface dictionary.", this, ac3BSID, interfaceNum, altSettingID);
	}
	return result;
}

IOReturn DJMConfigurationDictionary::getFeatureUnitIDConnectedToOutputTerminal (UInt8 * featureUnitID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 outputTerminalID) 
{
    DJM01ControlDictionary *		thisControl;
    IOReturn					result = kIOReturnError;
	...
	{
		debugIOLog ("! DJMConfigurationDictionary[%p]::GetFeatureUnitConnectedToOutputTerminal (%p, %d, %d, %d) - Could not get control interface dictionary.", 
					this, featureUnitID, interfaceNum, altSettingID, outputTerminalID);
	}
    return result;
}

IOReturn DJMConfigurationDictionary::getFirstStreamInterfaceNum (UInt8 * interfaceNum) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	...
	* interfaceNum = 0;
	...
	FailIf (NULL == (thisStream = OSDynamicCast (DJM01StreamDictionary, streamDictionaries->getObject (0))), Exit);
...
IOReturn DJMConfigurationDictionary::getControlledStreamNumbers (OSArray **controlledStreams, UInt8 *numControlledStreams) 
{
    DJM01ControlDictionary * 		thisControl = NULL;
	...
	* numControlledStreams = 0;
	...
	FailIf (NULL == (thisControl = OSDynamicCast (DJM01ControlDictionary, controlDictionaries->getObject (0))), Exit); 
...
/*
IOReturn DJMConfigurationDictionary::getControlInterfaceNum (UInt8 *interfaceNum) 
{
    DJM01ControlDictionary * 		thisControl = NULL;
	OSArray *					controlDictionaries = NULL;
	OSObject *					dictionaryValue = NULL;
...
IOReturn DJMConfigurationDictionary::getIsocEndpointInterval (UInt8 * interval, UInt8 interfaceNum, UInt8 altSettingID, UInt8 direction)
{
	DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getHighestSampleRate (UInt32 * sampleRate, UInt8 interfaceNum, UInt8 altSettingID) 
{
	OSArray *				sampleRates = NULL;
	OSObject *				arrayValue;
	...
	debugIOLog ("+ DJMConfigurationDictionary[%p]::getHighestSampleRate (%p, %d, %d)", this, sampleRate, interfaceNum, altSettingID);
	* sampleRate = 0;
...
Exit:
	debugIOLog ("- DJMConfigurationDictionary[%p]::getHighestSampleRate (%p = %lu, %d, %d) = 0x%x", this, sampleRate, *sampleRate, interfaceNum, altSettingID, result);
	return result;
}

IOReturn DJMConfigurationDictionary::getIsocAssociatedEndpointAddress (UInt8 * assocEndpointAddress, UInt8 interfaceNum, UInt8 altSettingID, UInt8 address) 
{
	DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIsocAssociatedEndpointMaxPacketSize (UInt16 * maxPacketSize, UInt8 interfaceNum, UInt8 altSettingID, UInt8 address) 
{
	DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIsocAssociatedEndpointRefreshInt (UInt8 * refreshInt, UInt8 interfaceNum, UInt8 altSettingID, UInt8 address) 
{
	DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIsocEndpointMaxPacketSize (UInt16 * maxPacketSize, UInt8 interfaceNum, UInt8 altSettingID, UInt8 direction)
{
	DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIsocEndpointSyncType (UInt8 * syncType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 address) 
{
	DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedFeatureUnitID (UInt8 * featureUnitID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 featureUnitIndex) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedMixerUnitID (UInt8 * mixerUnitID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 mixerUnitIndex) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedSelectorUnitID (UInt8 * selectorUnitID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 selectorUnitIndex) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedInputTerminalType (UInt16 * terminalType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 index) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedInputTerminalID (UInt8 * terminalID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 index) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedOutputTerminalID (UInt8 * terminalID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 index) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedOutputTerminalType (UInt16 * terminalType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 index) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getInputTerminalType (UInt16 * terminalType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 terminalID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedClockSourceID (UInt8 * clockSourceID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 clockSourceIndex) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedClockSelectorID (UInt8 * clockSelectorID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 clockSelectorIndex) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getIndexedClockMultiplierID (UInt8 * clockMultiplierID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 clockMultiplierIndex) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getInterfaceClass (UInt8 * interfaceClass, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getInterfaceSubClass (UInt8 * interfaceSubClass, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getLowestSampleRate (UInt32 * sampleRate, UInt8 interfaceNum, UInt8 altSettingID) 
{
	OSArray *				sampleRates = NULL;
...
IOReturn DJMConfigurationDictionary::getMaxBitRate (UInt16 * maxBitRate, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumAltSettings (UInt8 * numAltSettings, UInt8 interfaceNum) 
{
    DJM01StreamDictionary * 	thisStream = NULL;
	OSObject *				dictionaryValue = NULL;
	OSArray *				streamDictionaries = NULL;
    ...
    * numAltSettings = 0;
	...
	{
		FailIf (NULL == (thisStream = OSDynamicCast (DJM01StreamDictionary, streamDictionaries->getObject (streamIndex))), Exit);
...
IOReturn DJMConfigurationDictionary::getNumControls (UInt8 * numControls, UInt8 interfaceNum, UInt8 altSettingID, UInt8 featureUnitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumInputTerminals (UInt8 * numInputTerminals, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumOutputTerminals (UInt8 * numOutputTerminals, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumSelectorUnits (UInt8 * numSelectorUnits, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumClockSources (UInt8 * numClockSources, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumClockSelectors (UInt8 * numClockSelectors, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumClockMultipliers (UInt8 * numClockMultipliers, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumSources (UInt8 * numSources, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getNumStreamInterfaces (UInt8 * numStreamInterfaces) 
{
    DJM01StreamDictionary * 	thisStream = NULL;
	OSObject *				dictionaryValue = NULL;
	OSArray *				streamDictionaries = NULL;
...
IOReturn DJMConfigurationDictionary::getOutputTerminalType (UInt16 * terminalType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 terminalID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getSamplesPerFrame (UInt16 * samplesPerFrame, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getSelectorSources (OSArray ** selectorSources, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getClockSelectorSources (OSArray ** clockSelectorSources, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
//	<rdar://5811247>
IOReturn DJMConfigurationDictionary::getClockSourceClockType (UInt8 * clockType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID)
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
//	<rdar://5811247>
IOReturn DJMConfigurationDictionary::getClockSourceAssocTerminal (UInt8 * assocTerminal, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID)
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getSubType (UInt8 * subType, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getSourceID (UInt8 * sourceID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getSourceIDs (OSArray ** sourceIDs, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
OSArray * DJMConfigurationDictionary::getTerminalClockEntities ( UInt8 interfaceNum, UInt8 altSettingID )
{
	DJM01ControlDictionary *		thisControl = NULL;
...
IOReturn DJMConfigurationDictionary::getClockSourceID (UInt8 * clockSourceID, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
// <rdar://6394629>
IOReturn DJMConfigurationDictionary::getStringIndex (UInt8 * stringIndex, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
// <rdar://6430836>
IOReturn DJMConfigurationDictionary::getAudioClusterDescriptor (AudioClusterDescriptor * clusterDescriptor, UInt8 interfaceNum, UInt8 altSettingID, UInt8 unitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJMConfigurationDictionary::getTerminalLink (UInt8 * terminalLink, UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 		thisStream = NULL;
	IOReturn					result = kIOReturnError;
...
bool DJMConfigurationDictionary::init (const IOUSBConfigurationDescriptor * newConfigurationDescriptor, UInt8 controlInterfaceNum) 
{
	IOUSBConfigurationDescriptor *	mutableDescriptor = NULL;
    bool							result = false;

	debugIOLog ("+ DJMConfigurationDictionary[%p]::init (%p, %d)", this, newConfigurationDescriptor, controlInterfaceNum);
	...
	debugIOLog ("? DJMConfigurationDictionary[%p]::init () - Allocating %d bytes for mutable config descriptor.", this, USBToHostWord (newConfigurationDescriptor->wTotalLength));
...
Exit:
	...
	debugIOLog ("- DJMConfigurationDictionary[%p]::init () = 0x%x", this, result);
    return result;
}

bool DJMConfigurationDictionary::masterHasMuteControl (UInt8 interfaceNum, UInt8 altSettingID, UInt8 featureUnitID) 
{
    DJM01ControlDictionary *		thisControl = NULL;
    bool						result = false;
...
bool DJMConfigurationDictionary::verifySampleRateIsSupported (UInt8 interfaceNum, UInt8 altSettingID, UInt32 verifyRate) 
{
	bool							result = false;
	OSObject *						arrayValue = NULL;
	OSArray *						sampleRates = NULL;
...
bool DJMConfigurationDictionary::hasAudioStreamingInterfaces (void) 
{
	bool result = false;
...
// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
IOReturn DJMConfigurationDictionary::getInterruptEndpointAddress ( UInt8 * address, UInt8 interfaceNum, UInt8 altSettingID ) 
{
	DJM01ControlDictionary * 		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
IOReturn DJMConfigurationDictionary::getInterruptEndpointInterval ( UInt8 * interval, UInt8 interfaceNum, UInt8 altSettingID )
{
	DJM01ControlDictionary * 		thisControl = NULL;
	IOReturn					result = kIOReturnError;
...
// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
bool DJMConfigurationDictionary::hasInterruptEndpoint ( UInt8 interfaceNum, UInt8 altSettingID ) 
{
	DJM01ControlDictionary * 		thisControl = NULL;
	bool						result = false;
...
DJM01StreamDictionary * DJMConfigurationDictionary::getStreamDictionary (UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01StreamDictionary * 	thisStream = NULL;
	OSObject *				dictionaryValue = NULL;
	OSArray *				streamDictionaries = NULL;
...
//		else
//		{
//			debugIOLog ( "? DJMConfigurationDictionary::getStreamDictionary () - ( %d, %d ) != ( %d, %d )", interfaceNum, altSettingID, streamInterfaceNum, streamAltSettingID );
//		}
		streamIndex++;
	}
...
DJM01ControlDictionary * DJMConfigurationDictionary::getControlDictionary (UInt8 interfaceNum, UInt8 altSettingID) 
{
    DJM01ControlDictionary * 		thisControl = NULL;
	OSObject *					dictionaryValue = NULL;
	OSArray *					controlDictionaries = NULL;
	...
	{
		FailIf (NULL == (thisControl = OSDynamicCast (DJM01ControlDictionary, controlDictionaries->getObject (controlIndex))), Exit);
...
IOReturn DJMConfigurationDictionary::parseConfigurationDescriptor (IOUSBConfigurationDescriptor * configurationDescriptor) 
{
	IOReturn								result = kIOReturnError;
    USBInterfaceDescriptorPtr				theInterfacePtr = NULL;
	USBInterfaceAssociationDescriptorPtr	theInterfaceAssociationPtr = NULL;
    DJM01ControlDictionary *					controlDictionary = NULL;
    DJM01StreamDictionary *					streamDictionary = NULL;
	...
	debugIOLog ("+ DJMConfigurationDictionary[%p]::parseConfigurationDescriptor (%p)", this, configurationDescriptor);
				...
				&&  ( parsedLength <= totalLength ) ) 
	{
		...
		{
			debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - @ INTERFACE_ASSOCIATION (4.6)", this);
			...
			{
				debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - interfaceClass = first interface = %d, interface count = %d", this, ((USBInterfaceAssociationDescriptorPtr)theInterfacePtr)->bFirstInterface, ((USBInterfaceAssociationDescriptorPtr)theInterfacePtr)->bInterfaceCount);
		...
		{
			debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - @ INTERFACE (4.3.1/4.5.1)", this);
			...
			if (AUDIO == ((ACInterfaceDescriptorPtr)theInterfacePtr)->bInterfaceClass) 
			{
				...
				parsedLength = ( theInterfacePtr ? ( parsedLength + theInterfacePtr->bLength) : totalLength );
				debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - controlInterfaceNum = %d, thisInterfaceNumber = %d", this, controlInterfaceNum, thisInterfaceNumber);
				if	(		(AUDIOCONTROL == interfaceSubClass)
						&&	(controlInterfaceNum == thisInterfaceNumber)) 
				{
					debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Found an AUDIOCONTROL CS_INTERFACE (4.3.2)", this);
					if (NULL != (controlDictionaries = getControlDictionaries())) 
					{
						controlDictionary = OSDynamicCast (DJM01ControlDictionary, controlDictionaries->getLastObject ());
					}
					...
					if (INTERFACE_PROTOCOL_UNDEFINED == interfaceProtocol)
					{
						...
						// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
						debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Finished parsing AC Interface Descriptor", this);
						if ( kIOReturnSuccess == controlDictionary->getNumEndpoints ( &numEndpoints ) )
						{
							...
							{
								debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - numEndpoints: %d", this, numEndpoints);
							}
						}
					}
				...
				{
					debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - @ AUDIOSTREAMING CS_INTERFACE (4.5.2)", this);
					FailIf (kIOReturnSuccess != (result = getControlledStreamNumbers (&streamInterfaceNumbers, &numStreamInterfaces)), Exit);
					for (streamInterfaceIndex = 0; streamInterfaceIndex < numStreamInterfaces; streamInterfaceIndex++) 
					{
						...
						debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Comparing thisInterfaceNum = %d with %d", thisInterfaceNumber, streamInterfaceNumber->unsigned8BitValue());
						if (thisInterfaceNumber == streamInterfaceNumber->unsigned8BitValue()) 
						{
							debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Found a AUDIOSTREAMING CS_INTERFACE", this);
							if (NULL != (streamDictionaries = getStreamDictionaries())) 
							{
								streamDictionary = OSDynamicCast (DJM01StreamDictionary, streamDictionaries->getLastObject ());
							}
				...
				{
					debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - MIDI, jumping forward %d bytes", this, theInterfacePtr->bLength);
				...
				else if (AUDIOCONTROL == interfaceSubClass) 
				{
					if (INTERFACE_PROTOCOL_UNDEFINED == interfaceProtocol)
					{
						debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Found a control interface that we don't care about. Skipping %d bytes ...", 
				...
				{
					debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Unknown, skipping %d bytes", this, theInterfacePtr->bLength);
					theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					parsedLength = ( theInterfacePtr ? ( parsedLength + theInterfacePtr->bLength) : totalLength );
				}
			} // if (AUDIO == ((ACInterfaceDescriptorPtr)theInterfacePtr)->bInterfaceClass)
			...
			{
				debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Not an audio interface, skipping %d bytes", this, theInterfacePtr->bLength);
				theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
				parsedLength = ( theInterfacePtr ? ( parsedLength + theInterfacePtr->bLength) : totalLength );
			}
		} // if (INTERFACE == ((ACInterfaceDescriptorPtr)theInterfacePtr)->bDescriptorType)
		...
		{
			debugIOLog ("? DJMConfigurationDictionary[%p]::parseConfigurationDescriptor () - Default, skipping %d bytes", this, theInterfacePtr->bLength);
			theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
			parsedLength = ( theInterfacePtr ? ( parsedLength + theInterfacePtr->bLength) : totalLength );
        }
	...
	{
		IOLog ( "DJM01Audio encountered an invalid descriptor on an attached USB audio device. The device may not function properly.\n" );
		debugIOLog ( "! DJMConfigurationDictionary::parseConfigurationDescriptor () - Encountered a bad descriptor. Halting the parser ..." );
	}
...
USBInterfaceDescriptorPtr DJMConfigurationDictionary::parseInterfaceDescriptor (USBInterfaceDescriptorPtr theInterfacePtr, UInt8 * interfaceClass, UInt8 * interfaceSubClass, UInt8 * interfaceProtocol) 
{
	DJM01ControlDictionary *			controlDictionary = NULL;
	DJM01StreamDictionary *			streamDictionary = NULL;
	OSArray *						controlDictionaries = NULL;
	...
	debugIOLog ("+ DJMConfigurationDictionary[%p]::parseInterfaceDescriptor (%p, %p, %p)", this, theInterfacePtr, interfaceClass, interfaceSubClass);
	...
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::parseInterfaceDescriptor () - Found an AUDIOCONTROL interface", this);
		if (controlInterfaceNum == theInterfacePtr->bDescriptorSubtype) 
		{
			controlDictionary = DJM01ControlDictionary::create ();
	...
	{
		debugIOLog ("? DJMConfigurationDictionary[%p]::parseInterfaceDescriptor () - Found an AUDIOSTREAMING interface");
		streamDictionary = DJM01StreamDictionary::create ();
...
Exit:
	debugIOLog("- DJMConfigurationDictionary[%p]::parseInterfaceDescriptor () = %p", this, theInterfacePtr);
	return theInterfacePtr;
}

#pragma mark DJM01ControlDictionary

// DJM01ControlDictionary

//	Public methods

OSDefineMetaClassAndStructors (DJM01ControlDictionary, DJM01AudioDictionary);

DJM01ControlDictionary * DJM01ControlDictionary::create (void) 
{
    DJM01ControlDictionary *			controlDictionary;

    FailIf (NULL == (controlDictionary = new DJM01ControlDictionary), Exit);
...
IOReturn DJM01ControlDictionary::getNumControls (UInt8 * numControls, UInt8 featureUnitID) 
{
...
bool DJM01ControlDictionary::channelHasMuteControl (UInt8 featureUnitID, UInt8 channelNum) 
{
	AUAFeatureUnitDictionary *			featureUnitDictionary = NULL;
	bool								result = false;
...
bool DJM01ControlDictionary::channelHasVolumeControl (UInt8 featureUnitID, UInt8 channelNum) 
{
...
bool DJM01ControlDictionary::clockSourceHasFrequencyControl ( UInt8 clockSourceID, bool isProgrammable )
{
...
// <rdar://7446555>
bool DJM01ControlDictionary::clockSourceHasValidityControl ( UInt8 clockSourceID )
{
	AUAClockSourceDictionary *			clockSourceDictionary;
	UInt8								controls;
	bool								result = false;
...
DJM01UnitDictionary * DJM01ControlDictionary::getUnitDictionary (UInt8 unitID) 
{
    DJM01UnitDictionary * 			unitDictionary = NULL;
...
IOReturn DJM01ControlDictionary::getFeatureSourceID (UInt8 * sourceID, UInt8 featureUnitID) 
{
	AUAFeatureUnitDictionary *		featureUnitDictionary = NULL;
	IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getIndexedFeatureUnitID (UInt8 * featureUnitID, UInt8 featureUnitIndex) 
{
	AUAFeatureUnitDictionary *		featureUnitDictionary = NULL;
	IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getIndexedMixerUnitID (UInt8 * mixerUnitID, UInt8 mixerUnitIndex) 
{
...
IOReturn DJM01ControlDictionary::getIndexedSelectorUnitID (UInt8 * selectorUnitID, UInt8 selectorUnitIndex) 
{
...
IOReturn DJM01ControlDictionary::getFeatureUnitIDConnectedToOutputTerminal (UInt8 * featureUnitID, UInt8 outputTerminalID) 
{
...
IOReturn DJM01ControlDictionary::getIndexedInputTerminalType (UInt16 * terminalType, UInt8 index) 
{
...
IOReturn DJM01ControlDictionary::getIndexedInputTerminalID (UInt8 * inputTerminalID, UInt8 index) 
{
    AUAInputTerminalDictionary *		inputTerminalDictionary = NULL;
	IOReturn							result = kIOReturnError;
	OSArray *							inputTerminals = NULL;
...
IOReturn DJM01ControlDictionary::getIndexedOutputTerminalID (UInt8 * outputTerminalID, UInt8 index) 
{
    AUAOutputTerminalDictionary *		outputTerminalDictionary = NULL;
	IOReturn							result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getIndexedOutputTerminalType (UInt16 * terminalType, UInt8 index) 
{
    AUAOutputTerminalDictionary *		outputTerminalDictionary = NULL;
	IOReturn							result = kIOReturnError;
	OSArray *							outputTerminals = NULL;
...
IOReturn DJM01ControlDictionary::getIndexedClockSourceID (UInt8 * clockSourceID, UInt8 clockSourceIndex) 
{
...
IOReturn DJM01ControlDictionary::getIndexedClockSelectorID (UInt8 * clockSelectorID, UInt8 clockSelectorIndex) 
{
...
IOReturn DJM01ControlDictionary::getIndexedClockMultiplierID (UInt8 * clockMultiplierID, UInt8 clockMultiplierIndex) 
{
...
AUAFeatureUnitDictionary * DJM01ControlDictionary::getIndexedFeatureUnitDictionary (UInt8 index) 
{
...
AUAMixerUnitDictionary * DJM01ControlDictionary::getIndexedMixerUnitDictionary (UInt8 index) 
{
...
AUASelectorUnitDictionary * DJM01ControlDictionary::getIndexedSelectorUnitDictionary (UInt8 index) 
{
...
AUAClockSourceDictionary * DJM01ControlDictionary::getIndexedClockSourceDictionary (UInt8 index) 
{
...
AUAClockSelectorDictionary * DJM01ControlDictionary::getIndexedClockSelectorDictionary (UInt8 index) 
{
...
AUAClockMultiplierDictionary * DJM01ControlDictionary::getIndexedClockMultiplierDictionary (UInt8 index) 
{
...
AUAFeatureUnitDictionary * DJM01ControlDictionary::getFeatureUnitDictionary (UInt8 unitID) 
{
    AUAFeatureUnitDictionary *	 		featureUnitDictionary = NULL;
	OSArray *							featureUnits = NULL;
...
AUAInputTerminalDictionary * DJM01ControlDictionary::getInputTerminalDictionary (UInt8 unitID) 
{
    AUAInputTerminalDictionary * 	inputTerminalDictionary = NULL;
	OSArray *						inputTerminals = NULL;
...
AUAOutputTerminalDictionary * DJM01ControlDictionary::getOutputTerminalDictionary (UInt8 unitID) 
{
    AUAOutputTerminalDictionary * 	outputTerminalDictionary = NULL;
	OSArray *						outputTerminals = NULL;
...
IOReturn DJM01ControlDictionary::getInputTerminalType (UInt16 * terminalType, UInt8 index) 
{
...
IOReturn DJM01ControlDictionary::getNumInputTerminals (UInt8 * numInputTerminals) 
{
	OSArray *		inputTerminals = NULL;
	IOReturn		result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getNumOutputTerminals (UInt8 * numOutputTerminals) 
{
	OSArray *		outputTerminals = NULL;
	IOReturn		result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getNumSelectorUnits (UInt8 * numSelectorUnits) 
{
	OSArray *		selectorUnits = NULL;
...
IOReturn DJM01ControlDictionary::getNumClockSources (UInt8 * numClockSources) 
{
	OSArray *		clockSources = NULL;
...
IOReturn DJM01ControlDictionary::getNumClockSelectors (UInt8 * numClockSelectors) 
{
	OSArray *		clockSelectors = NULL;
...
IOReturn DJM01ControlDictionary::getNumClockMultipliers (UInt8 * numClockMultipliers) 
{
	OSArray *		clockMultipliers = NULL;
...
IOReturn DJM01ControlDictionary::getNumSources (UInt8 * numSources, UInt8 unitID) 
{
    DJM01UnitDictionary *		thisUnit = NULL;
	IOReturn				result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getSourceID (UInt8 * sourceID, UInt8 unitID) 
{
	DJM01UnitDictionary *		unitDictionary = NULL;
	IOReturn				result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getSourceIDs (OSArray ** sourceIDs, UInt8 unitID) 
{
	DJM01UnitDictionary *		unitDictionary = NULL;
	IOReturn				result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getSubType (UInt8 * subType, UInt8 unitID) 
{
	DJM01UnitDictionary *		unitDictionary = NULL;
	IOReturn				result = kIOReturnError;
...
OSArray * DJM01ControlDictionary::getTerminalClockEntities ()
{
	OSArray *						clockEntities = NULL;
	OSArray *						inputTerminals = NULL;
	OSArray *						outputTerminals = NULL;
	...
	debugIOLog ( "+ DJM01ControlDictionary::getTerminalClockEntities ()" );
	FailIf ( NULL == ( inputTerminals = getInputTerminals () ), Exit );
	FailIf ( NULL == ( outputTerminals = getOutputTerminals () ), Exit );
	...
	for ( UInt8 terminalIndex = 0; terminalIndex < inputTerminals->getCount (); terminalIndex++ )
	{
		...
		if ( ! isDuplicate )
		{
			debugIOLog ( "? DJM01ControlDictionary::getTerminalClockEntities () - Adding clockID %d ...", clockID ); 
...
Exit:
	debugIOLog ( "- DJM01ControlDictionary::getTerminalClockEntities () = %p", clockEntities );
	return clockEntities;
}

IOReturn DJM01ControlDictionary::getClockSourceID (UInt8 * clockSourceID, UInt8 unitID) 
{
	DJM01UnitDictionary *		unitDictionary = NULL;
	IOReturn				result = kIOReturnError;
...
// <rdar://6394629>
IOReturn DJM01ControlDictionary::getStringIndex (UInt8 * stringIndex, UInt8 unitID) 
{
	DJM01UnitDictionary *		unitDictionary = NULL;
	IOReturn				result = kIOReturnError;
...
// <rdar://6430836>
IOReturn DJM01ControlDictionary::getAudioClusterDescriptor (AudioClusterDescriptor * clusterDescriptor, UInt8 unitID) 
{
	DJM01UnitDictionary *		unitDictionary = NULL;
	IOReturn				result = kIOReturnError;
...
AUAEffectUnitDictionary * DJM01ControlDictionary::getEffectUnitDictionary (UInt8 unitID) 
{
...
AUAProcessingUnitDictionary * DJM01ControlDictionary::getProcessingUnitDictionary (UInt8 unitID) 
{
...
AUAMixerUnitDictionary * DJM01ControlDictionary::getMixerUnitDictionary (UInt8 unitID) 
{
...
AUAExtensionUnitDictionary * DJM01ControlDictionary::getExtensionUnitDictionary (UInt8 unitID) 
{
...
AUASelectorUnitDictionary * DJM01ControlDictionary::getSelectorUnitDictionary (UInt8 unitID) {
...
AUAClockSourceDictionary * DJM01ControlDictionary::getClockSourceDictionary (UInt8 unitID) 
{
...
AUAClockSelectorDictionary * DJM01ControlDictionary::getClockSelectorDictionary (UInt8 unitID) 
{
...
AUAClockMultiplierDictionary * DJM01ControlDictionary::getClockMultiplierDictionary (UInt8 unitID) 
{
...
IOReturn DJM01ControlDictionary::getOutputTerminalType (UInt16 * terminalType, UInt8 terminalID) 
{
    AUAOutputTerminalDictionary *	outputTerminalDictionary = NULL;
    IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getSelectorSources (OSArray ** selectorSources, UInt8 unitID) 
{
    AUASelectorUnitDictionary *		thisSelectorUnit = NULL;
	IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getMixerSources (OSArray ** mixerSources, UInt8 unitID) 
{
    AUAMixerUnitDictionary *		thisMixerUnit = NULL;
	IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getExtensionUnitSources (OSArray ** extensionUnitSources, UInt8 unitID) 
{
    AUAExtensionUnitDictionary *	thisExtensionUnit = NULL;
	IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getProcessingUnitSources (OSArray ** processingUnitSources, UInt8 unitID) 
{
    AUAProcessingUnitDictionary *	thisProcessingUnit = NULL;
	IOReturn						result = kIOReturnError;
...
IOReturn DJM01ControlDictionary::getClockSelectorSources (OSArray ** clockSelectorSources, UInt8 unitID) 
{
    AUAClockSelectorDictionary *	thisClockSelector = NULL;
	IOReturn						result = kIOReturnError;
...
//	<rdar://5811247>
IOReturn DJM01ControlDictionary::getClockSourceClockType (UInt8 * clockType, UInt8 unitID)
{
    AUAClockSourceDictionary *		thisClockSource = NULL;
	IOReturn						result = kIOReturnError;
...
//	<rdar://5811247>
IOReturn DJM01ControlDictionary::getClockSourceAssocTerminal (UInt8 * assocTerminal, UInt8 unitID)
{
    AUAClockSourceDictionary *		thisClockSource = NULL;
	IOReturn						result = kIOReturnError;
...
bool DJM01ControlDictionary::masterHasMuteControl (UInt8 featureUnitID) 
{
	AUAFeatureUnitDictionary *		featureUnitDictionary = NULL;
	bool							result = false;
...
USBInterfaceDescriptorPtr DJM01ControlDictionary::parseACInterfaceDescriptor (USBInterfaceDescriptorPtr theInterfacePtr, UInt8 const currentInterface, UInt16 * parsedLength, UInt16 totalLength) 
{
	OSArray *						inputTerminals = NULL;
	OSArray *						outputTerminals = NULL;
	OSArray *						featureUnits = NULL;
	...
	debugIOLog ("+ DJM01ControlDictionary[%p]::parseACInterfaceDescriptor (%p, %d)", this, theInterfacePtr, currentInterface);
				...
				&&	( * parsedLength <= totalLength ) )
	{
		...
		{
            case HEADER:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ HEADER (4.3.2)", this);
				adcVersion = USBToHostWord (* (UInt16 *) (&(((ACInterfaceHeaderDescriptorPtr)theInterfacePtr)->bcdADC[0])));
				if (adcVersion != kAUAUSBSpec1_0)
				{
					debugIOLog ("! DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - Encountered a descriptor version (0x%x) that DJM01Audio currently can't handle.", this, adcVersion);
				}
				...
				debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - numStreamInterfaces = %d", this, numStreamInterfaces);
                ...
                break;
            case INPUT_TERMINAL:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ INPUT_TERMINAL (4.3.2.1)", this);
            ...
            case OUTPUT_TERMINAL:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ OUTPUT_TERMINAL (4.3.2.2)", this);
            ...
            case FEATURE_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ FEATURE_UNIT (4.3.2.5)", this);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - There are %d controls on this feature unit", this, numControls);
            ...
            case MIXER_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ MIXER_UNIT (4.3.2.3)", this);
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - descriptor length = %d", this, theInterfacePtr->bLength);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - unit ID = %d", this, ((ACMixerUnitDescriptorPtr)theInterfacePtr)->bUnitID);
					mixerUnit->setNumInPins (((ACMixerUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - numInPins = %d", this, ((ACMixerUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - nrChannels = %d", this, nrChannels);
					...
					*channelConfig = USBToHostWord (*channelConfig);
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - channelConfig = %d", this, *channelConfig);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - controlSize = %d", this, controlSize);
            ...
            case SELECTOR_UNIT:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ SELECTOR_UNIT (4.3.2.4)", this);
				...
				debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - numInPins on selector = %d", this, ((ACSelectorUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
            ...
            case PROCESSING_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ PROCESSING_UNIT (4.3.2.6)", this);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - processing unit type = 0x%x", this, ((ACProcessingUnitDescriptorPtr)theInterfacePtr)->wProcessType);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - numInPins = %d", this, ((ACProcessingUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
            ...
            case EXTENSION_UNIT:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ EXTENSION_UNIT (4.3.2.7)", this);
				...
				break;
            default:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () - @ default. Nothing to do here.", this);
        }
		theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
		* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
    }
...
Exit:
	debugIOLog ("- DJM01ControlDictionary[%p]::parseACInterfaceDescriptor () = %p", this, theInterfacePtr);
    return theInterfacePtr;
}

USBInterfaceDescriptorPtr DJM01ControlDictionary::parseACInterfaceDescriptor_0200 (USBInterfaceDescriptorPtr theInterfacePtr, UInt8 const currentInterface, UInt16 * parsedLength, UInt16 totalLength) 
{
	OSArray *							inputTerminals = NULL;
	OSArray *							outputTerminals = NULL;
	OSArray *							featureUnits = NULL;
	...
	debugIOLog ("+ DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 (%p, %d)", this, theInterfacePtr, currentInterface);
				...
				&&	( * parsedLength <= totalLength ) )
	{
        ...
        switch (theInterfacePtr->bDescriptorSubtype) 
		{
            case USBAUDIO_0200::HEADER:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ HEADER (4.7.2)", this);
				...
				{
					debugIOLog ("! DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - Encountered a descriptor version (0x%x) that DJM01Audio currently can't handle.", this, adcVersion);
				}
            ...
            case USBAUDIO_0200::INPUT_TERMINAL:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ INPUT_TERMINAL (4.7.2.4)", this);
				FailIf (NULL == (inputTerminal = new AUAInputTerminalDictionary), Exit);
				RELEASE_IF_FALSE (inputTerminal, inputTerminal->initDictionaryForUse());
				FailIf (NULL == inputTerminal, Exit);
            ...
            case USBAUDIO_0200::OUTPUT_TERMINAL:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ OUTPUT_TERMINAL (4.7.2.5)", this);
				FailIf (NULL == (outputTerminal = new AUAOutputTerminalDictionary), Exit);
				RELEASE_IF_FALSE (outputTerminal, outputTerminal->initDictionaryForUse());
				FailIf (NULL == outputTerminal, Exit);
            ...
            case USBAUDIO_0200::FEATURE_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ FEATURE_UNIT (4.7.2.8)", this);
					FailIf (NULL == (featureUnit = new AUAFeatureUnitDictionary), Exit);
					RELEASE_IF_FALSE (featureUnit, featureUnit->initDictionaryForUse());
					FailIf (NULL == featureUnit, Exit);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - There are %d controls on this feature unit", this, numControls);
            ...
            case USBAUDIO_0200::MIXER_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ MIXER_UNIT (4.7.2.6)", this);
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - descriptor length = %d", this, theInterfacePtr->bLength);
					FailIf (NULL == (mixerUnit = new AUAMixerUnitDictionary), Exit);
					RELEASE_IF_FALSE (mixerUnit, mixerUnit->initDictionaryForUse());
					FailIf (NULL == mixerUnit, Exit);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - unit ID = %d", this, ((USBAUDIO_0200::ACMixerUnitDescriptorPtr)theInterfacePtr)->bUnitID);
					mixerUnit->setNumInPins (((USBAUDIO_0200::ACMixerUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - numInPins = %d", this, ((USBAUDIO_0200::ACMixerUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - nrChannels = %d", this, nrChannels);
					mixerUnit->setNumChannels (nrChannels);
					...
					*channelConfig = USBToHostLong (*channelConfig);
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - channelConfig = %d", this, *channelConfig);
					mixerUnit->setChannelConfig (*channelConfig);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - controlSize = %d", this, controlSize);
					...
					mixerUnit->setStringIndex (*(((UInt8*)theInterfacePtr) + theInterfacePtr->bLength - 1));	// <rdar://6394629>
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - setChannelNames = 0x%X", this, *(((UInt8*)theInterfacePtr) + (((USBAUDIO_0200::ACMixerUnitDescriptorPtr)theInterfacePtr)->bNrInPins + 10)));
            ...
            case USBAUDIO_0200::SELECTOR_UNIT:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ SELECTOR_UNIT (4.7.2.7)", this);
				FailIf (NULL == (selectorUnit = new AUASelectorUnitDictionary), Exit);
				RELEASE_IF_FALSE (selectorUnit, selectorUnit->initDictionaryForUse());
				FailIf (NULL == selectorUnit, Exit);
				...
				debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - numInPins on selector = %d", this, ((USBAUDIO_0200::ACSelectorUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
				selectorUnit->setStringIndex (*(((UInt8*)theInterfacePtr) + theInterfacePtr->bLength - 1));	// <rdar://6394629>
			...
			case USBAUDIO_0200::EFFECT_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ EFFECT_UNIT (4.7.2.10)", this);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - There are %d controls on this effect unit", this, numControls);
            ...
            case USBAUDIO_0200::PROCESSING_UNIT:
				{
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ PROCESSING_UNIT (4.7.2.11)", this);
					FailIf (NULL == (processingUnit = new AUAProcessingUnitDictionary), Exit);
					RELEASE_IF_FALSE (processingUnit, processingUnit->initDictionaryForUse());
					FailIf (NULL == processingUnit, Exit);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - processing unit type = 0x%x", this, ((USBAUDIO_0200::ACProcessingUnitDescriptorPtr)theInterfacePtr)->wProcessType);
					...
					debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - numInPins = %d", this, ((USBAUDIO_0200::ACProcessingUnitDescriptorPtr)theInterfacePtr)->bNrInPins);
            ...
            case USBAUDIO_0200::EXTENSION_UNIT:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ EXTENSION_UNIT (4.7.2.12)", this);
				FailIf (NULL == (extensionUnit = new AUAExtensionUnitDictionary), Exit);
				RELEASE_IF_FALSE (extensionUnit, extensionUnit->initDictionaryForUse());
				FailIf (NULL == extensionUnit, Exit);
			...
			case USBAUDIO_0200::CLOCK_SOURCE:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ CLOCK_SOURCE (4.7.2.1)", this);
                ...
                break;
			case USBAUDIO_0200::CLOCK_SELECTOR:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ CLOCK_SELECTOR (4.7.2.2)", this);
			...
			case USBAUDIO_0200::CLOCK_MULTIPLIER:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ CLOCK_MULTIPLIER (4.7.2.3)", this);
                ...
                break;
            default:
                debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () - @ default. Nothing to do here.", this);
        }
		theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
		* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
    }
...
Exit:
	debugIOLog ("- DJM01ControlDictionary[%p]::parseACInterfaceDescriptor_0200 () = %p", this, theInterfacePtr);
    return theInterfacePtr;
}

void DJM01ControlDictionary::parseInterfaceAssociationDescriptor (USBInterfaceAssociationDescriptorPtr theInterfaceAssociationPtr)
{
	OSNumber *						dictionaryNumber = NULL;
	OSArray *						streamInterfaceNumbers = NULL;
	UInt8							numStreamInterfaces;
	...
	debugIOLog ("+ DJM01ControlDictionary[%p]::parseInterfaceAssociationDescriptor (%p, %d)", this, theInterfaceAssociationPtr);
	...
	debugIOLog ("? DJM01ControlDictionary[%p]::parseInterfaceAssociationDescriptor () - numStreamInterfaces = %d", this, numStreamInterfaces);
	FailIf (kIOReturnSuccess != setDictionaryValue (kNumStreamInterfaces, numStreamInterfaces), Exit);
...
Exit:
	debugIOLog ("- DJM01ControlDictionary[%p]::parseInterfaceAssociationDescriptor () = %p", this, theInterfaceAssociationPtr);
}

// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
USBInterfaceDescriptorPtr DJM01ControlDictionary::parseACInterruptEndpointDescriptor ( USBInterfaceDescriptorPtr theInterfacePtr ) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = 0;

	debugIOLog ("+ DJM01ControlDictionary[%p]::parseACInterruptEndpointDescriptor (%p, %d)", this, theInterfacePtr);
	...
	if ( ENDPOINT == theInterfacePtr->bDescriptorType ) 
	{
		...
		{
			debugIOLog ("? DJM01ControlDictionary[%p]::parseACInterruptEndpointDescriptor () - @ ENDPOINT (4.6.1.1)", this);
			FailIf ( NULL == ( thisEndpoint = DJM01EndpointDictionary::create () ), Exit );
...
Exit:
	debugIOLog ("- DJM01ControlDictionary[%p]::parseACInterruptEndpointDescriptor () = 0x%x", this, theInterfacePtr);
    return theInterfacePtr;
}

IOReturn DJM01ControlDictionary::getInterruptEndpointAddress ( UInt8 * address ) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
...
IOReturn DJM01ControlDictionary::getInterruptEndpointInterval ( UInt8 * interval ) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
    IOReturn					result = kIOReturnError;
...
bool DJM01ControlDictionary::hasInterruptEndpoint () 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
    bool						result = false;
...
DJM01EndpointDictionary * DJM01ControlDictionary::getIndexedEndpointDictionary (UInt8 index) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
	
	FailIf (NULL == (endpoints = getEndpoints ()), Exit);
	FailIf (NULL == (thisEndpoint = OSDynamicCast (DJM01EndpointDictionary, endpoints->getObject (index))), Exit);
	
Exit:
	return thisEndpoint;
}

OSArray * DJM01ControlDictionary::getEndpoints ()
{
	OSArray *		endpoints = NULL;
	OSObject *		dictionaryValue = NULL;
...
#pragma mark DJM01StreamDictionary

// DJM01StreamDictionary

// Public methods

OSDefineMetaClassAndStructors (DJM01StreamDictionary, DJM01AudioDictionary);

DJM01StreamDictionary * DJM01StreamDictionary::create (void) 
{
    DJM01StreamDictionary *		streamDictionary = NULL;

    FailIf (NULL == (streamDictionary = new DJM01StreamDictionary), Exit);
...
DJM01EndpointDictionary * DJM01StreamDictionary::getIndexedEndpointDictionary (UInt8 index) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
...
IOReturn DJM01StreamDictionary::getIsocAssociatedEndpointAddress (UInt8 * assocEndpointAddress, UInt8 address) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::getIsocAssociatedEndpointMaxPacketSize (UInt16 * assocEndpointMaxPacketSize, UInt8 address) 
{
	DJM01EndpointDictionary *		thisEndpoint = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::getIsocAssociatedEndpointRefreshInt (UInt8 * assocEndpointRefreshInt, UInt8 address) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::getIsocEndpointAddress (UInt8 * address, UInt8 direction) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::getIsocEndpointDirection (UInt8 * direction, UInt8 index) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::getIsocEndpointInterval (UInt8 * interval, UInt8 direction) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
    IOReturn					result = kIOReturnError;
	...
	* interval = 0;
	...
	while ( ! ( * interval ) && endpointIndex < endpoints->getCount ()) 
	{
		...
		if ( direction == thisDirection ) 
		{
			...
			FailIf ( kIOReturnSuccess != ( result = thisEndpoint->getInterval ( interval ) ), Exit );
			debugIOLog ( "? DJM01StreamDictionary::getIsocEndpointInterval ( %p, %d ) = %d", interval, direction, * interval );
		}
		else
		{
			debugIOLog ( "? DJM01StreamDictionary::getIsocEndpointInterval () - Endpoint at index %d has wrong direction %d (!= %d)", endpointIndex, thisDirection, direction );
		}
		endpointIndex++;
	}
...
IOReturn DJM01StreamDictionary::getIsocEndpointMaxPacketSize (UInt16 * maxPacketSize, UInt8 direction)
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	OSArray *					endpoints = NULL;
    IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::getIsocEndpointSyncType (UInt8 * syncType, UInt8 address) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
    IOReturn					result = kIOReturnError;
...
USBInterfaceDescriptorPtr DJM01StreamDictionary::parseASInterfaceDescriptor (USBInterfaceDescriptorPtr theInterfacePtr, UInt8 const currentInterface, UInt16 * parsedLength, UInt16 totalLength) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	...
	debugIOLog ("+ DJM01StreamDictionary[%p]::parseASInterfaceDescriptor (%p, %d)", this, theInterfacePtr, currentInterface);
				...
				&&	( * parsedLength <= totalLength ) ) 
	{
		...
		if (CS_INTERFACE == theInterfacePtr->bDescriptorType) 
		{
			...
			{
                case AS_GENERAL:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ AS_GENERAL (4.5.2)", this);
					...
					debugIOLog ( "? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - formatTag = 0x%x", this, formatTag );
					...
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                case FORMAT_TYPE:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ FORMAT_TYPE", this);
					switch (((ASFormatTypeIDescriptorPtr)theInterfacePtr)->bFormatType) 
					{
						case FORMAT_TYPE_I:
						case FORMAT_TYPE_III:
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ FORMAT_TYPE_I/FORMAT_TYPE_III (Format 2.2.5/2.4.1)", this);
							...
							if (0 != numSampleFreqs) 
							{
								debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - Interface has a discrete number (%d) of sample rates ", this, numSampleFreqs);
							...
							{
								debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - Device has a variable number of sample rates", this);
						...
						case FORMAT_TYPE_II:
							...
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ FORMAT_TYPE_II (Format 2.3.6)", this);
							...
							break;
						default:
							debugIOLog ("! DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ Unknown Format Type!", this);
					}
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
				case FORMAT_SPECIFIC:
                    ...
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ FORMAT_SPECIFIC", this);
					...
					{
						case MPEG:
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ MPEG (2.3.8.1.1)", this);
							...
							break;
						case AC3:
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ AC3 (Format 2.3.8.2.1)", this);
							...
							break;
						default:
							debugIOLog ("! DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ Unknown format type 0x%x", this, formatTag);
							break;
					}
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                default:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ Default", this);
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
            }
        } 
		else 
		{
            ...
            switch (theInterfacePtr->bDescriptorType) 
			{
                case INTERFACE:
                    ...
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ INTERFACE", this);
                    done = true;
                    break;
                case ENDPOINT:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ ENDPOINT (4.6.1.1)", this);
                    FailIf (NULL == (thisEndpoint = DJM01EndpointDictionary::create ()), Exit);
					...
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                case CS_ENDPOINT:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ CS_ENDPOINT (4.6.1.2)", this);
                    if (EP_GENERAL == ((ASEndpointDescriptorPtr)theInterfacePtr)->bDescriptorSubtype) 
					{
                        debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ EP_GENERAL", this);
					...
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                default:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () - @ Default (else)", this);
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
            }
        }
...
Exit:
	debugIOLog ("- DJM01StreamDictionary[%p]::parseASInterfaceDescriptor () = 0x%x", this, theInterfacePtr);
    return theInterfacePtr;
}

USBInterfaceDescriptorPtr DJM01StreamDictionary::parseASInterfaceDescriptor_0200 (USBInterfaceDescriptorPtr theInterfacePtr, UInt8 const currentInterface, UInt16 * parsedLength, UInt16 totalLength) 
{
    DJM01EndpointDictionary *		thisEndpoint = NULL;
	AUAASEndpointDictionary *	asIsocEndpoint = NULL;
	DJM01EndpointDictionary *		dataEndpoint = NULL;
	DJM01EndpointDictionary *		feedbackEndpoint = NULL;
	...
	debugIOLog ("+ DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 (%p, %d)", this, theInterfacePtr, currentInterface);
				...
				&&	( * parsedLength <= totalLength ) ) 
	{
		...
		if (CS_INTERFACE == theInterfacePtr->bDescriptorType) 
		{
            switch (theInterfacePtr->bDescriptorSubtype) 
			{
                case USBAUDIO_0200::AS_GENERAL:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ AS_GENERAL (4.9.2)", this);
                ...
                case USBAUDIO_0200::FORMAT_TYPE:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ FORMAT_TYPE", this);
					switch (((USBAUDIO_0200::ASFormatTypeIDescriptorPtr)theInterfacePtr)->bFormatType) 
					{
						case USBAUDIO_0200::FORMAT_TYPE_I:
						case USBAUDIO_0200::FORMAT_TYPE_III:
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ FORMAT_TYPE_I/FORMAT_TYPE_III (Format 2.3.1.6/2.3.3.1)", this);
						...
						case USBAUDIO_0200::FORMAT_TYPE_II:
							...
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ FORMAT_TYPE_II (Format 2.3.2.6)", this);
							...
							break;
						default:
							debugIOLog ("! DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ Unknown/Unsupported Format Type!", this);
					}
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
				case USBAUDIO_0200::ENCODER:					
                    ...
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ ENCODER", this);
					bEncoder = ((USBAUDIO_0200::ASEncoderDescriptorPtr)theInterfacePtr)->bEncoder;
					switch (bEncoder) 
					{
						default:
							debugIOLog ("! DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ Unknown/unsupported encoder type 0x%x", this, bEncoder);
							break;
					}
				...
				case USBAUDIO_0200::DECODER:
                    ...
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ DECODER", this);
					bDecoder = ((USBAUDIO_0200::ASDecoderDescriptorPtr)theInterfacePtr)->bDecoder;
					switch (bDecoder) 
					{
						case USBAUDIO_0200::MPEG_DECODER:
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ MPEG (4.9.5.1)", this);
							bmMPEGCapabilities = USBToHostWord(
						...
						case USBAUDIO_0200::AC3_DECODER:
							debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ AC3 (4.9.5.2)", this);
							bmAC3BSID = USBToHostLong(
							...
							break;
						default:
							debugIOLog ("! DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ Unknown/unsupported decoder type 0x%x", this, bDecoder);
							break;
					}
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                default:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ Default", this);
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
            }
        } 
		else 
		{
            ...
            switch (theInterfacePtr->bDescriptorType) 
			{
                case INTERFACE:
                    // Need to make a new interface dictionary for this new interface or new alternate setting
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ INTERFACE", this);
                    done = true;
                    break;
                case ENDPOINT:
                    ...
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ ENDPOINT (4.10.1.1)", this);
                    FailIf (NULL == (thisEndpoint = DJM01EndpointDictionary::create ()), Exit);
					...
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                case CS_ENDPOINT:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ CS_ENDPOINT (4.10.1.2)", this);
                    if (EP_GENERAL == ((ASEndpointDescriptorPtr)theInterfacePtr)->bDescriptorSubtype) 
					{
                        debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ EP_GENERAL", this);
					...
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
                    break;
                default:
                    debugIOLog ("? DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () - @ Default (else)", this);
                    theInterfacePtr = (USBInterfaceDescriptorPtr)((UInt8 *)theInterfacePtr + theInterfacePtr->bLength);
					* parsedLength = ( theInterfacePtr ? ( * parsedLength + theInterfacePtr->bLength ) : totalLength );
            }
        }
...
Exit:
	debugIOLog ("- DJM01StreamDictionary[%p]::parseASInterfaceDescriptor_0200 () = 0x%x", this, theInterfacePtr);
    return theInterfacePtr;
}

// Private methods

IOReturn DJM01StreamDictionary::addSampleRate (UInt32 sampleRate)
{
	OSArray *		sampleRates = NULL;
	OSNumber *		sampleRateNumber = NULL;
	IOReturn		result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::addSampleRatesToStreamDictionary ( OSArray * sampleRates )
{
	IOReturn		result = kIOReturnError;
	...
	for ( UInt16 sampleRateIndex = 0; sampleRateIndex < sampleRates->getCount (); sampleRateIndex++ )
	{
		...
		if ( ! isDuplicate )
		{
			...
			{
				OSArray *						endpoints = NULL;
				DJM01EndpointDictionary *			endpoint = NULL;
				...
				{
					FailIf ( NULL == ( endpoint = OSDynamicCast ( DJM01EndpointDictionary, endpoints->getObject ( endpointIndex ) ) ), Exit );
				...
				if ( 0 == interval )
				{
					debugIOLog ( "! DJM01StreamDictionary[%p]::addSampleRatesToStreamDictionary () - ERROR! Isoc endpoint has a refresh interval of 0! Treating as 4 ...", this );
				...
				{
					debugIOLog ( "! DJM01StreamDictionary::addSampleRatesToStreamDictionary () - cannot add sample rate %lu due to packet size constraints!", sampleRateNumber->unsigned32BitValue () );
					continue;
				}
				else
				{
					debugIOLog ( "? DJM01StreamDictionary::addSampleRatesToStreamDictionary () - adding sample rate %lu", sampleRateNumber->unsigned32BitValue () );
...
AUAASEndpointDictionary * DJM01StreamDictionary::getASEndpointDictionary ()
{
	OSObject *					dictionaryValue = NULL;
...
OSArray * DJM01StreamDictionary::getEndpoints ()
{
	OSArray *		endpoints = NULL;
	OSObject *		dictionaryValue = NULL;
...
DJM01EndpointDictionary * DJM01StreamDictionary::getEndpointByAddress (UInt8 address) 
{
    DJM01EndpointDictionary *		 	thisEndpoint = NULL;
	OSArray *						endpoints = NULL;
    UInt8							endpointIndex;
...
DJM01EndpointDictionary * DJM01StreamDictionary::getEndpointDictionaryByAddress (UInt8 address) 
{
    DJM01EndpointDictionary *		 	thisEndpoint = NULL;
	OSArray *						endpoints = NULL;
	...
	while (!found && endpointIndex < endpoints->getCount ()) 
	{
		FailIf (NULL == (thisEndpoint = OSDynamicCast (DJM01EndpointDictionary, endpoints->getObject (endpointIndex))), Exit);
		FailIf (kIOReturnSuccess != thisEndpoint->getAddress (&thisAddress), Exit);
...
bool DJM01StreamDictionary::asEndpointHasMaxPacketsOnly ()
{
	AUAASEndpointDictionary *	asEndpoint = NULL;
	bool						result = false;
...
IOReturn DJM01StreamDictionary::asEndpointGetLockDelay (UInt8 * lockDelay)
{
	AUAASEndpointDictionary *	asEndpoint = NULL;
	IOReturn					result = kIOReturnError;
...
IOReturn DJM01StreamDictionary::asEndpointGetLockDelayUnits (UInt8 * lockDelayUnits)
{
	AUAASEndpointDictionary *	asEndpoint = NULL;
	IOReturn					result = kIOReturnError;
...
bool DJM01StreamDictionary::asEndpointHasPitchControl ()
{
	AUAASEndpointDictionary *	asEndpoint = NULL;
	bool						result = false;
...
bool DJM01StreamDictionary::asEndpointHasSampleFreqControl ()
{
	AUAASEndpointDictionary *	asEndpoint = NULL;
	bool						result = false;
...
#pragma mark DJM01EndpointDictionary

//    DJM01EndpointDictionary

OSDefineMetaClassAndStructors (DJM01EndpointDictionary, DJM01AudioDictionary);

DJM01EndpointDictionary * DJM01EndpointDictionary::create (void) 
{
    DJM01EndpointDictionary *		endpointDictionary = NULL;

    FailIf (NULL == (endpointDictionary = new DJM01EndpointDictionary), Exit);
...
IOReturn DJM01EndpointDictionary::getDirection (UInt8 * direction)
{
	IOReturn		result = kIOReturnError;
	UInt8			address;
...
IOReturn DJM01EndpointDictionary::getSyncType (UInt8 * syncType) 
{
	IOReturn		result = kIOReturnError;
	UInt8			attributes;
...
bool DJM01EndpointDictionary::isIsocStreaming ()
{
	bool			result = false;
	UInt8			attributes;
...
bool DJM01EndpointDictionary::isIsocFeedback ()
{
	bool			result = false;
	UInt8			attributes;
...
# pragma mark DJM01UnitDictionary

//    DJM01UnitDictionary

OSDefineMetaClassAndStructors (DJM01UnitDictionary, DJM01AudioDictionary);
