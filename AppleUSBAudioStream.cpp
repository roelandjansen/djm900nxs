...
OSDefineMetaClassAndStructors(DJM01AudioStream, IOAudioStream)

#pragma mark -IOKit Routines-

void DJM01AudioStream::free () {
	UInt32			i;

	debugIOLog ("+ DJM01AudioStream[%p]::free ()", this);
...
//(mitani)引数を変更
bool DJM01AudioStream::initWithAudioEngine (DJM01AudioDevice * device, DJM01AudioEngine * engine, UInt16 interfaceNumber, IOAudioSampleRate sampleRate, const char *streamDescription, OSDictionary *properties) {
	DJMConfigurationDictionary *		configDictionary;
	Boolean								result = false;
	UInt32								startChannelID;

	debugIOLog("+ DJM01AudioStream[%p]::initWithAudioEngine ()", this);
...
//	FailIf (NULL == streamInterface, Exit);
	...
	debugIOLog ("? DJM01AudioStream[%p]::initWithAudioEngine () - mInterfaceNumber = %d", this, mInterfaceNumber);
...
bool DJM01AudioStream::requestTerminate (IOService * provider, IOOptionBits options) {
	bool					result;

	debugIOLog ("+ DJM01AudioStream[%p]::requestTerminate (%p, %x)", this, provider, options);
	...
	debugIOLog ("- DJM01AudioStream[%p]::requestTerminate (%p, %x) = %d", this, provider, options, result);
	return result;
}

void DJM01AudioStream::stop (IOService * provider) {
    debugIOLog("+ DJM01AudioStream[%p]::stop (%p)", this, provider);
	...
	{
		debugIOLog ("! DJM01AudioStream[%p]::stop () - mStreamInterface was still open when stop() was called. Closing ...", this);
...
//Exit:
	...
	debugIOLog ("- DJM01AudioStream[%p]::stop (%p) - rc=%ld", this, provider, getRetainCount());
}

bool DJM01AudioStream::terminate (IOOptionBits options) {
	...
	debugIOLog ("+ DJM01AudioStream[%p]::terminate ()", this);
...
bool DJM01AudioStream::matchPropertyTable(OSDictionary * table, SInt32 *score)
{
	bool		returnValue = false;
	
	//debugIOLog ("+ DJM01AudioStream[%p]::matchPropertyTable (%p, %p)", this, table, score);
	...
	//debugIOLog ("- DJM01AudioStream[%p]::matchPropertyTable (%p, %p) = %d", this, table, score, returnValue);
...
void DJM01AudioStream::registerService(IOOptionBits options)
{
	debugIOLog ("+ DJM01AudioStream[%p]::registerService ( 0x%lx )", this, options);
...
IOReturn DJM01AudioStream::addAvailableFormats (DJMConfigurationDictionary * configDictionary)
{
...
//	Boolean								hasDigitalOutput = FALSE;
	...
	debugIOLog ("+ DJM01AudioStream[%p]::addAvailableFormats (%p)", this, configDictionary);
...
//	debugIOLog ("? DJM01AudioStream[%p]::addAvailableFormats () - There are %d alternate interfaces @ interface %d", this, numAltInterfaces, mInterfaceNumber);
...
//	altSettingIndex = configDictionary->alternateSettingZeroCanStream (mInterfaceNumber) ? 0 : 1;
	...
	altSettingIndex = 1;
	{
		...
		debugIOLog ("? DJM01AudioStream[%p]::addAvailableFormats () - Interface %d, Alt %d has a ", this, mInterfaceNumber, altSettingIndex);
...
Exit:
	debugIOLog ("- DJM01AudioStream[%p]::addAvailableFormats (%p) = 0x%x", this, configDictionary, result);
	return result;
}

// <rdar://7259238>
IOReturn DJM01AudioStream::setFormat(const IOAudioStreamFormat *streamFormat, bool callDriver)
{
	IOReturn	result;
	
	debugIOLog ("- DJM01AudioStream[%p]::setFormat (%p, %d)", this, streamFormat, callDriver );
	
	result = super::setFormat ( streamFormat, callDriver );
	
	debugIOLog ("- DJM01AudioStream[%p]::setFormat (%p, %d) = 0x%x", this, streamFormat, callDriver, result);
	
	return result;
}

// <rdar://7259238>
IOReturn DJM01AudioStream::setFormat(const IOAudioStreamFormat *streamFormat, const IOAudioStreamFormatExtension *formatExtension, OSDictionary *formatDict, bool callDriver)
{
	IOReturn	result = kIOReturnError;
	...
	debugIOLog ("- DJM01AudioStream[%p]::setFormat (%p, %p, %p, %d)", this, streamFormat, formatExtension, formatDict, callDriver );
	...
	{
		debugIOLog ("? DJM01AudioStream[%p]::setFormat (%p, %p, %p, %d) - Delaying %u ms...", this, streamFormat, formatExtension, formatDict, callDriver, kFormatChangeDelayInMs );
...
Exit:
	debugIOLog ("- DJM01AudioStream[%p]::setFormat (%p, %p, %p, %d) = 0x%x", this, streamFormat, formatExtension, formatDict, callDriver, result);
	
	return result;
}

// [rdar://4487489] - Use this method to allocate all USB buffers.
IOBufferMemoryDescriptor * DJM01AudioStream::allocateBufferDescriptor (IOOptionBits options, vm_size_t capacity, vm_offset_t alignment)
{
	...
	debugIOLog ("+ DJM01AudioStream[%p]::allocateBufferDescriptor ()", this);
	...
	options |= usbOptions;
	debugIOLog ("? DJM01AudioStream[%p]::allocateBufferDescriptor () - allocating a buffer with mask 0x%x", this, physicalMask);
...
Exit:
	debugIOLog ("- DJM01AudioStream[%p]::allocateBufferDescriptor () = %p", this, bufferDescriptorPtr);
	return bufferDescriptorPtr;
}

void DJM01AudioStream::calculateSamplesPerPacket (UInt32 sampleRate, UInt16 * averageFrameSamples, UInt16 * additionalSampleFrameFreq) 
{
	UInt32					divisor;
	UInt32					modulus;
	...
	* averageFrameSamples = sampleRate / modulus;
	...
	debugIOLog ( "? DJM01AudioStream[%p]::calculateSamplesPerPacket - transferSampleRate = 0x%8X, currentSampleRate = 0x%X", this, transferSampleRate, currentSampleRate );
						...
						||	( accumulatedSamples % ( 1000 << 16 ) ) ) )
	{
		...
		{
			debugIOLog ( "? DJM01AudioStream[%p]::calculateSamplesPerPacket - large packet on transfer %d (1-indexed)", this , transfer );
			accumulatedSamples -= ( 1000 << 16 );
		}
		transfer++;
		cycleLength++;
	...
	debugIOLog ( "? DJM01AudioStream[%p]::calculateSamplesPerPacket - cycleLength = %d", this, cycleLength );
	#endif
}

IOReturn DJM01AudioStream::checkForFeedbackEndpoint (DJMConfigurationDictionary * configDictionary)
{
	...
	if (kAsynchSyncType == syncType) 
	{
		...
		if (assocEndpoint != 0) 
		{
			debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - assocEndpoint = 0x%x", this, assocEndpoint);
			...
			debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - Synch endpoint has refresh interval %d, feedback packet size %d", this, mRefreshInterval, mFeedbackPacketSize);
			...
			{
				debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - Need to adjust mNumUSBFramesPerList: %ld < %ld", mFramesUntilRefresh, mNumUSBFramesPerList);
				if (NULL != mUSBIsocFrames) 
				{
					debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - Disposing of current mUSBIsocFrames [%p]", this, mUSBIsocFrames);
					IOFree (mUSBIsocFrames, mNumUSBFrameLists * mNumTransactionsPerList * sizeof (IOUSBLowLatencyIsocFrame));
					mUSBIsocFrames = NULL;
				}
				...
				debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - mNumUSBFramesPerList = %d, mNumUSBFrameListsToQueue = %d, mNumUSBFrameLists = %d", this, mNumUSBFramesPerList, mNumUSBFrameListsToQueue, mNumUSBFrameLists);
				...
				debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - mUSBIsocFrames is now %p", this, mUSBIsocFrames);
		...
		{
			debugIOLog ("! DJM01AudioStream[%p]::checkForFeedbackEndpoint () - No associated synch endpoint found.", this);
		}
	} // if (kAsynchSyncType == syncType)
	else 
	{
		debugIOLog ("? DJM01AudioStream[%p]::checkForFeedbackEndpoint () - No associated synch endpoint.", this);
	}
...
IOReturn DJM01AudioStream::CoalesceInputSamples (UInt32 numBytesToCoalesce, IOUSBLowLatencyIsocFrame * pFrames) {
	...
	#if DEBUGINPUT
	debugIOLog ("+ DJM01AudioStream[%p]::CoalesceInputSamples (%lu, %p)", this, numBytesToCoalesce, pFrames); 
	#endif
	...
	if (0 != numBytesToCoalesce) 
	{
		...
		#if DEBUGINPUT
		debugIOLog ("! DJM01AudioStream[%p]::CoalesceInputSamples () - Coalesce from %ld %ld bytes (framelist %ld) on CoreAudio thread", this, originalBufferOffset, numBytesToCoalesce, mCurrentFrameList);
		#endif
		if ( mMasterMode && !mHaveTakenFirstTimeStamp )
		{
			debugIOLog ("! DJM01AudioStream[%p]::CoalesceInputSamples () - CoreAudio thread is asking for samples without having been sent a timestamp!", this );
		}
	}
	else
	{
			...
			&& (-1 != pFrames[usbFrameIndex].frStatus))				// IOUSBFamily hasn't gotten here yet
	{
		...
		{
			debugIOLog ("! DJM01AudioStream[%p]::CoalesceInputSamples () - encountered unusual frame with status 0x%x in frame list %lu", this, pFrames[usbFrameIndex].frStatus, usbFrameIndex);
				...
				&&	(pFrames[usbFrameIndex].frActCount > 0))
		{
			...
			{
				debugIOLog ("? DJM01AudioStream::CoalesceInputSamples () - Taking first time stamp.");
		...
		{			
			...
			if ( ( 0 != numBytesToCoalesce ) && ( 0 >= numBytesLeft ) )
			{
				...
				if ( actualCount > 0 )
				{
					...
					debugIOLog ("! DJM01AudioStream[%p]::CoalesceInputSamples () - numBytesMargin: %lu frames: %lu\n", this, numBytesMargin, numBytesMargin/mSampleSize );
				}
			}
#endif
		}
	...
	{
		debugIOLog ("! DJM01AudioStream[%p]::CoalesceInputSamples () - Requested: %lu, Remaining: %lu on frame list %lu\n", this, numBytesToCoalesce, numBytesLeft, mCurrentFrameList);
	}
	...
	if ( kIOReturnSuccess != result )
	{
		debugIOLog ( "! DJM01AudioStream[%p]::CoalesceInputSamples (%lu, %p) = 0x%x", this, numBytesToCoalesce, pFrames, result );
	}
	return result;
}

// [rdar://3918719] The following method now does the work of performFormatChange after being regulated by DJM01AudioDevice::formatChangeController().
IOReturn DJM01AudioStream::controlledFormatChange (const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
	IOReturn							result;
	...
	debugIOLog ("+ DJM01AudioStream[%p]::controlledFormatChange (%p, %p)", this, newFormat, newSampleRate);
	...
	if (newFormat->fNumChannels != this->format.fNumChannels) 
	{
		...
		debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - Need to adjust channel controls (cur = %d, new = %d)", this, this->format.fNumChannels, newFormat->fNumChannels);
	...
	if (newSampleRate) 
	{
		debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - Changing sampling rate to %d", this, newSampleRate->whole);
	...
	{
		debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - %d channel %d bit @ %d Hz is not supported by alternate setting %d.", this,
	...
	debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - about to set: mInterfaceNumber = %d & newAlternateSettingID = %d", this, mInterfaceNumber, newAlternateSettingID);
			...
			&&	( kUSBDeviceSpeedHigh == mUSBAudioDevice->getDeviceSpeed () ) )
	{
		...
		if ( 0 == interval )
		{
			debugIOLog ( "! DJM01AudioStream[%p]::controlledFormatChange () - ERROR! Isoc endpoint has a refresh interval of 0! Treating as 4 ...", this );
	...
	debugIOLog ( "? DJM01AudioStream[%p]::controlledFormatChange () - mSamplesPerPacket: %u(whole) %u(fraction)", this, mSamplesPerPacket.whole, mSamplesPerPacket.fraction );
	
	calculateSamplesPerPacket (mCurSampleRate.whole, &averageFrameSamples, &additionalSampleFrameFreq);
	debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - averageFrameSamples = %d", this, averageFrameSamples);
	...
	debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - mAverageFrameSize = %d, mAlternateFrameSize = %d", this, mAverageFrameSize, mAlternateFrameSize);
	...
	debugIOLog("? DJM01AudioStream[%p]::controlledFormatChange () - New mSampleBufferSize = %d numSamplesInBuffer = %d", this, mSampleBufferSize, numSamplesInBuffer );
	...
	debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - Called setNumSampleFramesPerBuffer with %d", this, mSampleBufferSize / (mSampleSize ? mSampleSize : 1));
	debugIOLog ("? DJM01AudioStream[%p]::controlledFormatChange () - newFormat->fNumChannels = %d, newFormat->fBitWidth %d", this, newFormat->fNumChannels, newFormat->fBitWidth);

	result = kIOReturnSuccess;

Exit:
	debugIOLog ("- DJM01AudioStream[%p]::controlledFormatChange () = 0x%x", this, result);
    return result;
...
void DJM01AudioStream::updateSampleOffsetAndLatency ()
{
	...
	if (kUSBIn == mDirection) 
	{
		...
		{
			debugIOLog ("? DJM01AudioStream[%p]::updateSampleOffsetAndLatency () - Compensating for high speed timing difference in sample offset", this);
		...
		//		mUSBAudioEngine->setSampleOffset (cautiousSafeSampleOffset);
		setProperty(kIOAudioEngineInputSampleOffsetKey, cautiousSafeSampleOffset, sizeof(UInt32)*8);
		debugIOLog ("? NMAudioEngine[%p]::controlledFormatChange () - setting input sample offset to %lu sample frames", this, minimumSafeSampleOffset);
		
		// setSampleLatency chosen via heuristics
		mUSBAudioEngine->setInputSampleLatency (averageFrameSamples * (/* kMinimumFrameOffset + */ 1));
#else
		...
		{
			debugIOLog ("? DJM01AudioStream[%p]::updateSampleOffsetAndLatency () - Compensating for time stamp generation in sample offset", this);
		...
		if (NULL != sampleOffsetDictionary)
		{
			...
			{				
				debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - override input sample offset (%lu) to %lu sample frames", this, newSampleOffset, sampleOffset->unsigned32BitValue ());
		...
		debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - setting input sample offset to %lu sample frames", this, newSampleOffset);
		...
		if (NULL != sampleLatencyDictionary)
		{
			...
			{				
				debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - override input sample latency (%lu) to %lu sample frames", this, newSampleLatency, sampleLatency->unsigned32BitValue ());
		...
		debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - setting input sample latency to %lu sample frames", this, newSampleLatency);
#endif
	} 
	else 
	{
		...
		minimumSafeSampleOffset = cautiousSafeSampleOffset;
		highSpeedCompensation = true;
		minimumSafeSampleOffset = minimumSafeSampleOffset + (minimumSafeSampleOffset / kUSBInputRecoveryTimeFraction) + (highSpeedCompensation ? (5 * minimumSafeSampleOffset / 3) : 0);
		
		mUSBAudioEngine->setSampleOffset (minimumSafeSampleOffset);
		debugIOLog ("? NMAudioEngine[%p]::controlledFormatChange () - setting output sample offset to %lu sample frames", this, minimumSafeSampleOffset);
		
		// setSampleLatency chosen via heuristics 
		//		mUSBAudioEngine->setSampleLatency (additionalSampleFrameFreq ? averageFrameSamples + 1 : averageFrameSamples);
		mUSBAudioEngine->setOutputSampleLatency (additionalSampleFrameFreq ? averageFrameSamples + 1 : averageFrameSamples);
#else
		...
		if (NULL != sampleOffsetDictionary)
		{
			...
			if ( NULL != sampleOffset )
			{				
				debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - override output sample offset (%lu) to %lu sample frames", this, newSampleOffset, sampleOffset->unsigned32BitValue ());
				newSampleOffset = sampleOffset->unsigned32BitValue ();
			}
		}
		...
		debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - setting output sample offset to %lu sample frames", this, newSampleOffset);
		...
		if (NULL != sampleLatencyDictionary)
		{
			...
			if ( NULL != sampleLatency )
			{				
				debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - override output sample latency (%lu) to %lu sample frames", this, newSampleLatency, sampleLatency->unsigned32BitValue ());
				newSampleLatency = sampleLatency->unsigned32BitValue ();
			}
		}
		...
		debugIOLog ("? DJM01AudioEngine[%p]::updateSampleOffsetAndLatency () - setting output sample latency to %lu sample frames", this, newSampleLatency);
#endif
	}
}
...
#if defined(__i386__)
IOReturn DJM01AudioStream::copyAnchor (UInt64 anchorFrame, UInt64 * anchorTime, UInt64 * usbCycleTime)
#else
IOReturn DJM01AudioStream::copyAnchor (UInt64 anchorFrame, AbsoluteTime * anchorTime, UInt64 * usbCycleTime)
#endif
{
	IOReturn		result = kIOReturnError;
...
UInt64 DJM01AudioStream::jitterFilter (UInt64 curr, UInt32 nIter) 
{
...
// <rdar://problem/7378275> Improved timestamp generation accuracy
#if defined(__i386__)
UInt64 DJM01AudioStream::generateTimeStamp (SInt32 transactionIndex, UInt32 preWrapBytes, UInt32 byteCount)
#else
AbsoluteTime DJM01AudioStream::generateTimeStamp (SInt32 transactionIndex, UInt32 preWrapBytes, UInt32 byteCount)
#endif
{
...
#if DEBUGTIMESTAMPS
		debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp () - prewrap not zero, value: %lu", this, preWrapBytes );
#endif
	}
...
#if DEBUGTIMESTAMPS		
	debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp () - thisFrameNum = %llu, transactionIndex: %d numOutstandingTransactions: %lu numOutStandingUSBFrames: %lu remainingFullTransactions: %lu", this, thisFrameNum, transactionIndex, numOutstandingTransactions, numOutStandingUSBFrames, remainingFullTransactions ); 
#endif	
...
#if DEBUGTIMESTAMPS
	debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp () - partialFrame: %lu, preWrapBytes: %lu usbCycleTime: %llu", this, partialFrame, preWrapBytes, usbCycleTime );
#endif	
...
#if DEBUGTIMESTAMPS	
	debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp () - time_nanos before filter: %llu", this, raw_time_nanos );
#endif	
...
#define MAGNITUDEOF( x ) ( ( ( x ) > 0 ) ? ( x ) : ( - ( x ) ) )
		...
		if	(getDirection () == kIOAudioStreamDirectionInput)
		{
			debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp (I)   stampDifference = %lld, stampJitter = %lld, mStampDrift = %lld, sampleRate = %llu \n", this, filteredStampDifference, stampJitter, mStampDrift, sampleRate );
		}
		else
		{
			debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp (O)   stampDifference = %lld, stampJitter = %lld, mStampDrift = %lld, sampleRate =                  %llu \n", this, filteredStampDifference, stampJitter, mStampDrift, sampleRate );
		}
		...
		if	(getDirection () == kIOAudioStreamDirectionInput)
		{
			debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp (RI)   stampDifference = %lld, sampleRate = %llu \n", this, rawStampDifference, sampleRate );
		}
		else
		{
			debugIOLog ( "? DJM01AudioStream[%p]::generateTimeStamp (RO)   stampDifference = %lld, sampleRate =                  %llu \n", this, rawStampDifference, sampleRate );
		}
#endif
	}
...
UInt32 DJM01AudioStream::getCurrentSampleFrame () {
...
IOReturn DJM01AudioStream::GetDefaultSettings (UInt8 * altSettingID, IOAudioSampleRate * sampleRate) {
	IOReturn						result;
	UInt16							format;
	...
	debugIOLog ("+ DJM01AudioStream[%p]::GetDefaultSettings ()", this);
	result = kIOReturnError;
					...
					&&	( PCM != format ) ) ) )
	{
		...
		debugIOLog ("? DJM01AudioStream[%p]::GetDefaultSettings () - Taking first available alternate setting (%d)", this, newAltSettingID);
	...
	debugIOLog ("? DJM01AudioStream[%p]::GetDefaultSettings () - Default sample rate is %d", this, newSampleRate.whole);
	debugIOLog ("? DJM01AudioStream[%p]::GetDefaultSettings () - Default alternate setting ID is %d", this, newAltSettingID);
...
Exit:
	debugIOLog ("- DJM01AudioStream[%p]::GetDefaultSettings (%d, %lu) = 0x%x", this, *altSettingID, sampleRate->whole, result);
	return result;
}

#if DEBUGLATENCY
UInt64 DJM01AudioStream::getQueuedFrameForSample (UInt32 sampleFrame)
{
	...
	// debugIOLog ("+ DJM01AudioStream::getQueuedFrameForSample (%lu)", sampleFrame);
	FailIf (NULL == mFrameQueuedForList, Exit);
	...
	// debugIOLog ("? DJM01AudioStream::getQueuedFrameForSample (%lu) - mLastPreparedBufferOffset = %lu, sampleByteOffset = %lu, bytesToQueuePoint = %lu, sampleBufferSize = %lu", sampleFrame, mLastPreparedBufferOffset, sampleByteOffset, bytesToQueuePoint, getSampleBufferSize());
	
	if (bytesToQueuePoint > mThisFrameListSize + mLastFrameListSize)
	{
		debugIOLog ("? DJM01AudioStream::getQueuedFrameForSample (%lu) - sample frame is not queued in the previous two frame lists", sampleFrame);
	}
	else
	{
		...
		if (bytesToQueuePoint <= mThisFrameListSize)
		{
			...
			// debugIOLog ("? DJM01AudioStream::getQueuedFrameForSample (%lu) - sample frame is queued in *PREVIOUS* frame list (%d) (mThisFrameListSize = %lu, mLastFrameListSize = %lu)", sampleFrame, frameListNumber, mThisFrameListSize, mLastFrameListSize);
		}
		else
		{
			...
			// debugIOLog ("? DJM01AudioStream::getQueuedFrameForSample (%lu) - sample frame was queued in frame list %d (mThisFrameListSize = %lu, mLastFrameListSize = %lu)", sampleFrame, frameListNumber, mThisFrameListSize, mLastFrameListSize);
		}
		...
		// debugIOLog ("? DJM01AudioStream::getQueuedFrameForSample (%lu) - frame list %d starting queueing at byte %lu", sampleFrame, frameListNumber, bufferByteOffset); 
...
Exit:
	// debugIOLog ("- DJM01AudioStream::getQueuedFrameForSample (%lu) = %llu", sampleFrame, usbFrame);
	return usbFrame;
...
//--------------------------------------------------------------------------------
bool DJM01AudioStream::configureAudioStream (IOAudioSampleRate sampleRate) {
...
//	UInt8								deviceSubclass;
	...
	#endif
    debugIOLog ("+ DJM01AudioStream[%p]::configureAudioStream ()", this);
	...
	if (kUSBIn == mDirection) 
	{
		...
		debugIOLog ("? DJM01AudioStream[%p]::initHardware () - This is an input type endpoint (mic, etc.)", this);
		index = 0;
		do 
		{
	...
	{
		debugIOLog ("? DJM01AudioStream[%p]::configureAudioStream () - This is an output type endpoint (speaker, etc.)", this);
		index = 0;
		do 
		{
...
Exit:
    debugIOLog("- DJM01AudioStream[%p]::configureAudioStream(), resultCode = %x, resultBool = %d", this, resultCode, resultBool);
    return resultBool;
}

bool DJM01AudioStream::openStreamInterface () {
	bool	result = FALSE;
...
void DJM01AudioStream::closeStreamInterface () {
...
void DJM01AudioStream::queueInputFrames () {
	...
	if (0 == mShouldStop && TRUE != mInCompletion && NULL != mStreamInterface) 
	{
		...
		if ( framesLeftInQueue < ( mNumUSBFramesPerList * ( mNumUSBFrameListsToQueue / 2 )) / 2 ) 	// <rdar://problem/6327095>
		{
			...
			{
				#if DEBUGLOADING
				debugIOLog ("! DJM01AudioEngine::convertInputSamples () - Queue a read from convertInputSamples: framesLeftInQueue = %ld", (UInt32)framesLeftInQueue);
				#endif
...
void DJM01AudioStream::queueOutputFrames () {
	UInt64	curUSBFrameNumber;
	UInt64	framesLeftInQueue;
	...
	if (0 == mShouldStop && TRUE != mInCompletion && NULL != mStreamInterface) 
	{
		...
		if ( framesLeftInQueue < ( mNumUSBFramesPerList * ( mNumUSBFrameListsToQueue / 2 )) / 2 )	// <rdar://problem/6327095>
		{
			debugIOLog ("! DJM01AudioStream::queueSampleFrameWrite () - Queue a write from clipOutputSamples: framesLeftInQueue = %ld", (UInt32)framesLeftInQueue);
...
UInt16 DJM01AudioStream::getAlternateFrameSize () {
	return mAlternateFrameSize;
}

// <rdar://7568547> Initialize the USB frame list to proper values.
void DJM01AudioStream::initializeUSBFrameList ( IOUSBLowLatencyIsocFrame * usbIsocFrames, UInt32 numFrames )
{
...
IOReturn DJM01AudioStream::PrepareWriteFrameList (UInt32 arrayIndex) {
	const IOAudioStreamFormat *			theFormat;
	IOReturn							result;
...
IOReturn DJM01AudioStream::PrepareAndReadFrameLists (UInt8 sampleSize, UInt8 numChannels, UInt32 usbFrameListIndex) {
	IOReturn							result;
	UInt32								firstFrame;
	UInt32								numTransactionsPrepared;
	UInt16								averageFrameSamples;
	...
	#if DEBUGINPUT
	debugIOLog ("+ DJM01AudioStream::PrepareAndReadFrameLists (%d, %d, %ld)", sampleSize, numChannels, usbFrameListIndex);
	#endif
	...
	if (NULL != mPipe) 
	{
		...
		{
			debugIOLog ("! DJM01AudioStream[%p]::PrepareAndReadFrameLists () - Error 0x%x reading from pipe", this, result);
		}
	...
	{
		debugIOLog ("! DJM01AudioStream[%p]::PrepareAndReadFrameLists () - mPipe is NULL!", this);
	}

	#if DEBUGINPUT
	debugIOLog ("- DJM01AudioStream::PrepareAndReadFrameLists ()");
	#endif
	return result;
}
...
void DJM01AudioStream::primeInputPipe (IOUSBPipe * pipeToPrime, UInt32 bytesPerUSBFrame, UInt32 usbFramesToDelay) 
{
	...
	if (result == kIOReturnSuccess)
	{
		...
		debugIOLog ("? DJM01AudioStream::primeInputPipe (%p, %lu) - %d frames primed. mUSBFrameToQueue = %llu", pipeToPrime, bytesPerUSBFrame, usbFramesToDelay, mUSBFrameToQueue);
	}
...
void DJM01AudioStream::primeInputPipeHandler (void * object, void * parameter, IOReturn result, IOUSBLowLatencyIsocFrame * pFrames)
{
	DJM01AudioStream *	self;
	UInt32					usbFramesToDelay;
	
	debugIOLog ("+ DJM01AudioStream::primeInputPipeHandler (%p, %lu, 0x%x, %p)", object, (uintptr_t) parameter, result, pFrames); 
	self = (DJM01AudioStream *)object;
...
IOReturn DJM01AudioStream::readFrameList (UInt32 frameListNum) {
	const IOAudioStreamFormat *			theFormat;
    IOReturn							result;

	#if DEBUGINPUT
	debugIOLog ("+ DJM01AudioStream::readFrameList ()");
	#endif
	theFormat = this->getFormat ();
...
void DJM01AudioStream::readHandler (void * object, void * parameter, IOReturn result, IOUSBLowLatencyIsocFrame * pFrames) {
	DJM01AudioStream *			self;
	...
	#if DEBUGINPUT
	debugIOLog ("+ DJM01AudioStream::readHandler ()");
	#endif
	self = (DJM01AudioStream *)object;
	...
	{
		debugIOLog ("! DJM01AudioStream::readHandler () - Encountered fatal error 0x%x on frame list %d (frReqCount = %d).", result, self->mCurrentFrameList, pFrames->frReqCount );
		// [rdar://5417631] Drop a once-per-attach log message that the device is generating overruns.
		if ( false == self->mGeneratesOverruns )
		{
			IOLog ( "WARNING: DJM01Audio has detected that a connected USB audio device is sending too much audio data.\n" );
		...
		if ( flagOverrun )
		{
			// This is a fatal error. Notify the DJM01AudioDevice to sync the sample rates when possible if this device has two streaming interfaces.
	...
	if (kIOReturnAborted != result)
	{
		...
		for (frameIndex = 0; frameIndex < self->mNumTransactionsPerList && pFrames; frameIndex++)
		{
			...
			{
				debugIOLog ("! DJM01AudioStream::readHandler () - Frame list %d frame index %d returned error 0x%x (frActCount = %lu, result = 0x%x)", self->mCurrentFrameList, frameIndex, thisStatus, thisActCount, result);
			}
			#endif
			
			if (thisActCount < minimumUSBFrameSize)
			{
				// IOLog ("DJM01Audio: ERROR on input! Short packet of size %lu encountered when %lu bytes were requested.\n", thisActCount, (pFrames + frameIndex)->frReqCount);
			}
	...
	if (self->mShouldStop > 0) 
	{
				...
				||	(self->mShouldStop == self->mNumUSBFrameListsToQueue))
		{
			...
			debugIOLog("? DJM01AudioStream::readHandler() - stopping: %d", self->mShouldStop);
		}
		self->mShouldStop++;
	} 
...
//  <rdar://problem/7345441> DJM01Audio: getRateFromSamplesPerPacket() has a rounding error in sample rate calculation
UInt32 DJM01AudioStream::getRateFromSamplesPerPacket ( IOAudioSamplesPerFrame samplesPerPacket )
{
	UInt64 sampleRate = 0;
...
*/
void DJM01AudioStream::sampleRateHandler (void * target, void * parameter, IOReturn result, IOUSBIsocFrame * pFrames) {
    DJM01AudioStream	*			self;
					...
					||	(kIOReturnUnderrun == result)))
	{
			  ...
			  ( newSamplesPerFrame.whole != oldSamplesPerFrame.whole || newSamplesPerFrame.fraction != oldSamplesPerFrame.fraction ) ) 
		{
			...
			{
				debugIOLog ( "! DJM01AudioStream::sampleRateHandler () - ignoring sample rate %d as out of bounds", self->getRateFromSamplesPerPacket ( newSamplesPerFrame ) );
			}
			else
			{
				...
				debugIOLog ("? DJM01AudioStream::sampleRateHandler () - Sample rate changed, requestedFrameRate: %u mSamplesPerPacket: %lu %lu\n", self->getRateFromSamplesPerPacket ( self->mSamplesPerPacket ), self->mSamplesPerPacket.whole, self->mSamplesPerPacket.fraction );
#endif
			}
		}
#if DEBUGSAMPLERATEHANDLER
		debugIOLog ("? DJM01AudioStream::sampleRateHandler () - currentFrameRate: %u mSamplesPerPacket: %lu %lu\n", self->getRateFromSamplesPerPacket ( self->mSamplesPerPacket ), self->mSamplesPerPacket.whole, self->mSamplesPerPacket.fraction );
#endif
	}
	else
	{
		debugIOLog ("! DJM01AudioStream::sampleRateHandler () - ignoring isoc result due to error 0x%x", result);
		if ( pFrames )
		{
	...
	if (0 == self->mShouldStop) 
	{
		...
		if (NULL != self->mAssociatedPipe) 
		{
						...
						&&	(framesToAdvance <= kMaxFeedbackPollingInterval))
			{
				...
				{
					debugIOLog ( "! DJM01AudioStream::sampleRateHandler () - framesToAdvance = %d, mNextSyncReadFrame = %llu, readStatus = 0x%x", framesToAdvance, self->mNextSyncReadFrame, readStatus );
					self->mNextSyncReadFrame -= framesToAdvance;
					framesToAdvance *= 2;
				}
			}
			...
			if ( kIOReturnSuccess != readStatus )
			{
				debugIOLog ( "! DJM01AudioStream::sampleRateHandler () - Could not queue feedback endpoint isoc request. Feedback request chain is halted!" );
	...
	{
		debugIOLog ( "? DJM01AudioStream::sampleRateHandler () - Stopping feedback chain because stream is stopping." );
	}
...
IOReturn DJM01AudioStream::setSampleRateControl (UInt8 address, UInt32 sampleRate) {
...
//IOLog("setSampleRateControl address=%x\n",address);
	...
	debugIOLog ("? DJM01AudioStream[%p]::SetSampleRate () - Control interface %d, alt setting %d, endpoint address 0x%x, sample rate (little endian) 0x%x", this, mInterfaceNumber, mAlternateSettingID, devReq.wIndex, theSampleRate);
...
Exit:
	if (kIOReturnSuccess != result)
	{
		debugIOLog ("! DJM01AudioStream[%p]::setSampleRateControl () = 0x%x", this, result);
	}
	return result;
}

UInt8 DJM01AudioStream::getSyncType () {	
	DJMConfigurationDictionary *		configDictionary;
//	UInt8								direction = 0;
//	UInt8								address = 0;
...
UInt32 DJM01AudioStream::getLockDelayFrames () {
	DJMConfigurationDictionary *		configDictionary;
...
IOReturn DJM01AudioStream::prepareUSBStream () {
	const IOAudioStreamFormat *			theFormat;
	IOReturn							resultCode;
    ...
    debugIOLog ("+ DJM01AudioStream[%p]::prepareUSBStream ()", this);
	...
	remainder = mCurSampleRate.whole - ( mSamplesPerPacket.whole * mTransactionsPerUSBFrame * 1000 );	// same as (mCurSampleRate.whole % 1000) * mTransactionsPerUSBFrame
	mSamplesPerPacket.fraction = ( remainder * 65536 ) / mTransactionsPerUSBFrame;
	debugIOLog ( "? DJM01AudioStream[%p]::prepareUSBStream () - mSamplesPerPacket: %u(whole) %u(fraction)", this, mSamplesPerPacket.whole, mSamplesPerPacket.fraction );
...
//	resultCode = mStreamInterface->SetAlternateInterface (this, mAlternateSettingID);
	...
	debugIOLog ("? DJM01AudioStream[%p]::prepareUSBStream () - mStreamInterface->SetAlternateInterface (this, %d) = 0x%x", this, mAlternateSettingID, resultCode);
...
//	FailIf ( 0 == maxPacketSize, Exit );
	...
	if (kUSBIn == mDirection) 
	{
		...
		debugIOLog ( "? DJM01AudioStream[%p]::prepareUSBStream () - maxPacketSize = %d, maxFrameSize = %d", this, maxPacketSize, maxFrameSize );
	...
	debugIOLog ("? DJM01AudioStream[%p]::prepareUSBStream () - calling SetPipePolicy (%d)", this, maxFrameSize);
	resultCode = mPipe->SetPipePolicy (maxFrameSize, 0);
	FailIf (kIOReturnSuccess != resultCode, Exit);
Exit:
    debugIOLog ("- DJM01AudioStream[%p]::prepareUSBStream () = %x", this, resultCode);
    return resultCode;
}

IOReturn DJM01AudioStream::startUSBStream (UInt64 currentUSBFrame, UInt32 usbFramesToDelay) {
	const IOAudioStreamFormat *			theFormat;
	...
	debugIOLog ("+ DJM01AudioStream[%p]::startUSBStream (%lld, %ld, %d)", this, currentUSBFrame, usbFramesToDelay, maxFrameSize);
	...
	if (kUSBIn == mDirection) 
	{
		...
		// [rdar://4664738] Never call setPipePolicy in excess of the maximum packet size for the endpoint.
		maxFrameSize = ( maxFrameSize > maxPacketSize ) ? maxPacketSize : maxFrameSize;
		debugIOLog ( "? DJM01AudioStream[%p]::startUSBStream () - maxPacketSize = %d, maxFrameSize = %d", this, maxPacketSize, maxFrameSize );
		mReadUSBFrameSize = maxFrameSize;
	} 
	else 
	{
	...
	debugIOLog ("? DJM01AudioStream[%p]::startUSBStream () - mUSBFrameToQueue = %llu", this, mUSBFrameToQueue);
	
	if (NULL != mAssociatedPipe) 
	{
		mNextSyncReadFrame = mUSBFrameToQueue;
		debugIOLog ("? DJM01AudioStream[%p]::startUSBStream () - Starting feedback endpoint stream at frame %llu", this, mNextSyncReadFrame);
		...
		debugIOLog ("? DJM01AudioStream[%p]::startUSBStream () - Feedback endpoint stream started.", this);		
	}
	...
	{				
		...
		debugIOLog ("? DJM01AudioStream[%p]::startUSBStream () - Priming input stream at frame %llu ( %d USB frames ) in %d transaction(s)", this, mUSBFrameToQueue, usbFramesToDelay, transactionsToQueue );
		remainingFrames = usbFramesToDelay;
	
		// No longer does the following as it it compensated by kStartDelayOffset in DJM01AudioEngine::performAudioEngineStart().
		...
		#endif
		debugIOLog ("? DJM01AudioStream[%p]::startUSBStream () - Starting input stream at frame %llu", this, mUSBFrameToQueue);
		for (frameListNum = mCurrentFrameList; frameListNum < mNumUSBFrameListsToQueue; frameListNum++) 
		{
			...
			{
				debugIOLog ("! DJM01AudioStream[%p]::startUSBStream () - readFrameList (%d) failed with error 0x%x!", this, frameListNum, interimResult);
	...
	else 
	{
		...
		* (UInt64 *) ( & (mUSBIsocFrames[0].frTimeStamp)) = 0xFFFFFFFFFFFFFFFFull;
		...
		for (frameListNum = mCurrentFrameList; frameListNum < mNumUSBFrameListsToQueue; frameListNum++) 
		{
			...
			if (kIOReturnSuccess != interimResult)
			{
				debugIOLog ("! DJM01AudioStream[%p]::startUSBStream () - writeFrameList (%d) failed with error 0x%x!", this, frameListNum, interimResult);
				encounteredQueuingError = true;
			}
		}
	}
	...
	{
		debugIOLog ("! DJM01AudioStream[%p]::startUSBStream () - Stream will *NOT* start because of queuing errors", this);
		resultCode = kIOReturnError;
	}
	else
	{
...
Exit:
    debugIOLog ("- DJM01AudioStream[%p]::startUSBStream () = %x", this, resultCode);
    return resultCode;
}

IOReturn DJM01AudioStream::stopUSBStream () {
	debugIOLog ("+ DJM01AudioStream[%p]::stopUSBStream ()", this);
...
bool DJM01AudioStream::willTerminate (IOService * provider, IOOptionBits options) {
	debugIOLog ("+ DJM01AudioStream[%p]::willTerminate (%p)", this, provider);

	if ( (mUSBAudioEngine == provider) || (mStreamInterface == provider) )
	{
		mTerminatingDriver = TRUE;
		// [rdar://5535738] Always close the stream interface here. Don't do it in the isoc completion routines.
		debugIOLog ("? DJM01AudioStream[%p]::willTerminate () - Closing stream interface", this);
	...
	debugIOLog ("- DJM01AudioStream[%p]::willTerminate ()", this);

	return super::willTerminate (provider, options);
}

IOReturn DJM01AudioStream::writeFrameList (UInt32 frameListNum) {
    IOReturn							result;
	...
	if (mNeedTimeStamps) 
	{
		if (mUHCISupport)
		{
			...
			#if DEBUGUHCI
			debugIOLog ("? DJM01AudioStream::writeFrameList () - Writing mSampleBufferWrapDescriptors[0]");
			#endif
			...
			{
				#if DEBUGUHCI
				debugIOLog ("? DJM01AudioStream::writeFrameList () - writeFrameList: Writing mSampleBufferWrapDescriptors[1] to frame %ld", mUSBFrameToQueue + mNumFramesInFirstList);
				#endif
...
Exit:
	if (kIOReturnSuccess != result)
	{
		debugIOLog ("? DJM01AudioStream[%p]::writeFrameList () - failed with error 0x%x", this, result);
	}
	return result;
}

void DJM01AudioStream::writeHandler (void * object, void * parameter, IOReturn result, IOUSBLowLatencyIsocFrame * pFrames) {
    DJM01AudioStream *   self;
#if defined(__i386__)
	UInt64					time;
#else
	...
	#if DEBUGUHCI
	debugIOLog ("? DJM01AudioStream::writeHandler () - writeHandler: curUSBFrameNumber = %llu parameter = 0x%x mUSBFrameToQueue = %llu", curUSBFrameNumber, (UInt32)parameter, self->mUSBFrameToQueue);
	debugIOLog ("? DJM01AudioStream::writeHandler () - %llu ?> %lu", frameDifference, expectedFrames);
	#endif
    ...
    {
        debugIOLog ("? DJM01AudioStream::writeHandler () - Not advancing frame list");
        goto Exit;
    }
    ...
    if (kIOReturnAborted != result) 
    {
		...
		if (kIOReturnSuccess != result)
		{
			debugIOLog ("! DJM01AudioStream::writeHandler () - Frame list %d write returned with error 0x%x", self->mCurrentFrameList, result);
        }
				...
				&&	(0 == self->mBufferOffset))
		{
			...
			for (UInt16 i = 0; i < numberOfFramesToCheck && pFrames; i++)
			{
				if (pFrames[i].frActCount && !self->mShouldStop )					// <rdar://problem/7378275>
				{
					...
					debugIOLog ("? DJM01AudioStream::writeHandler () - Taking first time stamp on frame list %lu frame index %d", self->mCurrentFrameList, i); 
		...
		for (UInt16 i = 0; i < numberOfFramesToCheck && pFrames; i++) 
        {
            ...
            {
                debugIOLog ("! DJM01AudioStream::writeHandler () - pFrames[%d].frStatus = 0x%x", i, pFrames[i].frStatus);
				debugIOLog ("     pFrames[%d].frReqCount = %lu", i, pFrames[i].frReqCount);
				debugIOLog ("     pFrames[%d].frActCount = %lu", i, pFrames[i].frActCount);
				debugIOLog ("     pFrames[%d].frTimeStamp = 0x%x", i, * (UInt64 *) &(pFrames[i].frTimeStamp));
				// IOLog ("DJM01Audio: ERROR on output! Short packet of size %lu encountered when %lu bytes were requested.\n", pFrames[i].frActCount, pFrames[i].frReqCount);
            }
        }
		#endif
        ...
        {
			debugIOLog ("! DJM01AudioStream::writeHandler - Fell behind! mUSBFrameToQueue = %llu, curUSBFrameNumber = %llu", self->mUSBFrameToQueue, curUSBFrameNumber);
    ...
    if (0 != parameter) 
    {
			 ...
			 &&	(frameDifference > expectedFrames - 1))
		{
			...
			if (self->mShouldStop > 0) 
			{
				debugIOLog ("? DJM01AudioStream::writeHandler() - stopping: %d", self->mShouldStop);
				self->mShouldStop++;
			} // mShouldStop check
...
void DJM01AudioStream::writeHandlerForUHCI (void * object, void * parameter, IOReturn result, IOUSBLowLatencyIsocFrame * pFrames) {
    DJM01AudioStream *   self;
	UInt64                  curUSBFrameNumber;
	SInt64                  frameDifference;
    SInt32                  expectedFrames;
	...
	#if DEBUGUHCI
	debugIOLog ("? DJM01AudioStream[%p]::writeHandlerForUHCI () - writeHandlerForUHCI: curUSBFrameNumber = %llu parameter = 0x%x mUSBFrameToQueue = %llu", curUSBFrameNumber, (UInt32)parameter, self->mUSBFrameToQueue);
	debugIOLog ("? DJM01AudioStream[%p]::writeHandlerForUHCI () -  %llu ?> %lu", frameDifference, expectedFrames);
	#endif
    ...
    {        
		if (kIOReturnSuccess != result)
		{
			debugIOLog ("! DJM01AudioStream::writeHandlerForUHCI () - Frame list %d (split for UHCI) write returned with error 0x%x", self->mCurrentFrameList, result);
        }
		#ifdef DEBUG
		// Comb the isoc frame list for alarming statuses.
		...
		for (UInt16 i = 0; i < numberOfFramesToCheck && pFrames; i++) 
        {
            if	(		(kIOReturnSuccess != pFrames[i].frStatus)
					||	(pFrames[i].frActCount != pFrames[i].frReqCount))
            {
                debugIOLog ("! DJM01AudioStream::writeHandlerForUHCI () - pFrames[%d].frStatus = 0x%x ", i, pFrames[i].frStatus);
				debugIOLog ("     pFrames[%d].frReqCount = %lu", i, pFrames[i].frReqCount);
				debugIOLog ("     pFrames[%d].frActCount = %lu", i, pFrames[i].frActCount);
				debugIOLog ("     pFrames[%d].frTimeStamp = 0x%x", i, * (UInt64 *) &(pFrames[i].frTimeStamp));
				// IOLog ("DJM01Audio: ERROR on output! Short packet of size %lu encountered when %lu bytes were requested.\n", pFrames[i].frActCount, pFrames[i].frReqCount);
        ...
        if (self->mUSBFrameToQueue <= curUSBFrameNumber) 
        {
			debugIOLog ("! DJM01AudioStream[%p]::writeHandlerForUHCI () - Fell behind! mUSBFrameToQueue = %llu, curUSBFrameNumber = %llu", self->mUSBFrameToQueue, curUSBFrameNumber);
			debugIOLog ("! DJM01AudioStream[%p]::writeHandlerForUHCI () - Skipping ahead ...");
            self->mUSBFrameToQueue = curUSBFrameNumber + kMinimumFrameOffset;
        }
    }
	...
	{
		debugIOLog ("? DJM01AudioStream[%p]::writeHandlerForUHCI () - Halting.", self);
	}
...
#if defined(__i386__)
void DJM01AudioStream::takeTimeStamp (bool incrementLoopCount, UInt64 *timestamp)
#else
void DJM01AudioStream::takeTimeStamp (bool incrementLoopCount, AbsoluteTime *timestamp)
#endif
{
	if ( NULL != mUSBAudioEngine )
	{
		if (false == mHaveTakenFirstTimeStamp)
		{
			...
			debugIOLog ("? DJM01AudioStream[%p]::takeTimeStamp (0, %p) - First timestamp taken", this, timestamp);
...
void DJM01AudioStream::registerPlugin (DJM01AudioPlugin * thePlugin) {
	mPlugin = thePlugin;
...
void DJM01AudioStream::pluginLoaded (DJM01AudioStream * usbAudioStreamObject) {
	IOReturn							result;
...
IOReturn DJM01AudioStream::pluginDeviceRequest (IOUSBDevRequest * request, IOUSBCompletion * completion) {
	IOReturn						result;
...
void DJM01AudioStream::pluginSetConfigurationApp (const char * bundleID) {
	if (bundleID) 
	{
...
#pragma mark -DJM01AudioStreamNode-

//  <rdar://problem/6686515>
OSDefineMetaClassAndStructors ( DJM01AudioStreamNode, IOService )

bool DJM01AudioStreamNode::start ( IOService * provider ) 
{
	debugIOLog ( "+ DJM01AudioStreamNode[%p]::start (%p)", this, provider );
	
	if ( IOService::start ( provider ) )
	{
		if ( provider )
		{
			provider->setProperty ( "DJM01AudioStreamPropertiesReady", "Yes" );
		}
