...
OSDefineMetaClassAndStructors (DJM01AudioDevice, super)


void DJM01AudioDevice::free () {
    debugIOLog ("+ DJM01AudioDevice[%p]::free ()", this);
	...
	if ( 0 != mEngineArray )
	{
		...
		{
			DJM01AudioEngine * engine = OSDynamicCast ( DJM01AudioEngine, mEngineArray->getObject ( engineIndex ) );
			if ( 0 != engine )
			{
				engine->release ();
			}
...
bool DJM01AudioDevice::ControlsStreamNumber (UInt8 streamNumber) {
	...
	if (mConfigDictionary)
	{
		...
		for (streamIndex = 0; streamIndex < numStreams; streamIndex++) 
		{
			...
			debugIOLog ("? DJM01AudioDevice[%p]::ControlsStreamNumber () - Checking stream %d against controled stream %d", this, streamNumber, arrayNumber->unsigned8BitValue());
...
bool DJM01AudioDevice::start (IOService * provider) {
	bool								result;

    debugIOLog ("+ DJM01AudioDevice[%p]::start (%p)", this, provider);
	result = FALSE;
	...
	mInitHardwareThread = thread_call_allocate ((thread_call_func_t)DJM01AudioDevice::initHardwareThread, (thread_call_param_t)this);
	FailIf (NULL == mInitHardwareThread, Exit);

	result = super::start (provider);				// Causes our initHardware routine to be called.

Exit:
	//  <rdar://problems/6576824&6674310> DJM01Audio fails to unload properly
	if ( !result )
	{
		stop ( provider );		
	}
	
	debugIOLog ("- DJM01AudioDevice[%p]::start (%p) = result = %d", this, provider, result);

	return result;
}

//  <rdar://problem/6686515>
bool DJM01AudioDevice::initHardware (IOService * provider) {
	bool								result;
	...
	//	<rdar://6223517>	Retain a reference in case the DJM01AudioDevice object is release while it is trying to execute the thread.
	retain ();
...
//  <rdar://problem/6686515>
void DJM01AudioDevice::initHardwareThread (DJM01AudioDevice * aua, void * provider) {
	IOCommandGate *						cg;
	IOReturn							result;
...
IOReturn DJM01AudioDevice::initHardwareThreadAction (OSObject * owner, void * provider, void * arg2, void * arg3, void * arg4) {
	DJM01AudioDevice *				aua;
	IOReturn							result;

	result = kIOReturnError;

	aua = (DJM01AudioDevice *)owner;
	FailIf (NULL == aua, Exit);
...
IOReturn DJM01AudioDevice::protectedInitHardware (IOService * provider) {
...
//	UInt8							numStreams;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::protectedInitHardware (%p)", this, provider);

	resultCode = FALSE;
	FailIf (NULL == mControlInterface, Exit);

	debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - %d configuration(s) on this device. This control interface number is %d", this, mControlInterface->GetDevice()->GetNumConfigurations (), mControlInterface->GetInterfaceNumber ());
		
	debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - Attempting to create configuration dictionary...", this);
//	mConfigDictionary = DJMConfigurationDictionary::create (getConfigurationDescriptor(), mControlInterface->GetInterfaceNumber());	// rdar://5495653
	mConfigDictionary = DJMConfigurationDictionary::create (getConfigurationDescriptor(), 0);	// rdar://5495653
	FailIf (NULL == mConfigDictionary, Exit);
	debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - Successfully created configuration dictionary.", this);

//	if ( !mConfigDictionary->hasAudioStreamingInterfaces () )
//	{
		// Bail since here is nothing related to audio streaming.
//		debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - No audio streaming interfaces in configuration dictionary.", this);
//		goto Exit;
//	}
...
//	debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - %d controlled stream(s). %d stream interface(s).", this, numStreams, numStreamInterfaces);
//	FailIf (0 == numStreams, Exit);
	...
	debugIOLog ( "? DJM01AudioDevice[%p]::protectedInitHardware () - Trying to retrieve the USB Interface Name... ", this );
	nameObject = mControlInterface->getProperty ( "USB Interface Name" );
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "? DJM01AudioDevice[%p]::protectedInitHardware () - Retrieved product name %s from registry", this, nameString->getCStringNoCopy () );
	...
	else
	{
		...
		if (0 != stringIndex) 
		{
			...
			if (kIOReturnSuccess != err)
			{
				...
				debugIOLog ("! DJM01AudioDevice[%p]::protectedInitHardware () - couldn't get string descriptor. Retrying ...", this);
				err = mControlInterface->GetDevice()->GetStringDescriptor (stringIndex, string, kStringBufferSize);
			}
			if (kIOReturnSuccess != err)
			{
				// Reset the device and try one last time
				debugIOLog ("! DJM01AudioDevice[%p]::protectedInitHardware () - Still couldn't get string descriptor. Resetting device ...", this);
				mControlInterface->GetDevice()->ResetDevice();      // Doesn't matter if this fails
				IOSleep (50);										// Give the device 50 ms to get ready
				debugIOLog ("! DJM01AudioDevice[%p]::protectedInitHardware () - Last retry ...", this);
				err = mControlInterface->GetDevice()->GetStringDescriptor (stringIndex, string, kStringBufferSize);
			}
		} 
		else 
		{
			...
			nameObject = NULL;
			nameString = NULL;
			debugIOLog ( "! DJM01AudioDevice[%p]::protectedInitHardware () - Trying to retrieve the product name from the IOUSBDevice ... ", this );
			...
			else
			{
				...
				debugIOLog ("! DJM01AudioDevice[%p]::protectedInitHardware () - There was no USB Product Name", this);
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "? DJM01AudioDevice[%p]::protectedInitHardware () - Retrieved vendor name %s from registry", this, nameString->getCStringNoCopy () );
			strncpy ( string, nameString->getCStringNoCopy (), kStringBufferSize );
			err = kIOReturnSuccess;
		} 
	}
	...
	if ( NULL != localizedBundle )
	{
		debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - setting kIOAudioDeviceLocalizedBundleKey property to %s", this, localizedBundle->getCStringNoCopy () );
		setProperty ( kIOAudioDeviceLocalizedBundleKey, localizedBundle );
	}
	#if LOCALIZABLE
	else
	{
		debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - setting kIOAudioDeviceLocalizedBundleKey property to DJM01Audio.kext", this);
		setProperty ( kIOAudioDeviceLocalizedBundleKey, "DJM-900nexus USBAudio.kext" );	
	}
	#endif
	...
	FailIf ( FALSE == resultCode, Exit );	// <rdar://problem/6576824> DJM01Audio fails to unload properly
	...
	debugIOLog ("? DJM01AudioDevice[%p]::protectedInitHardware () - starting rate timer", this);
...
// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
void DJM01AudioDevice::checkForStatusInterruptEndpoint( void )
{	
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::checkForStatusInterruptEndpoint ()", this);
...
void DJM01AudioDevice::statusInterruptHandler( void * target, void * parameter, IOReturn status, UInt32 bufferSizeRemaining )
{
	DJM01AudioDevice *		self;
	
	FailIf ( NULL == target, Exit );
	self = ( DJM01AudioDevice * )target;
	
	debugIOLog ("+ DJM01AudioDevice[%p]::statusInterruptHandler ()", self);
	...
	{
		debugIOLog ("! DJM01AudioDevice[%p]::statusInterruptHandler () error from USB: 0x%X or IOService inactive: %u, NOT reposting read to interrupt pipe", self, status, self->isInactive ());
	}
	else
	{
		if ( kIOUSBPipeStalled == status )
		{
			debugIOLog ("! DJM01AudioDevice[%p]::statusInterruptHandler () error from USB: kIOUSBPipeStalled, reposting read to interrupt pipe", self);
			// <rdar://7865729> Defer the ClearPipeStall() to our own thread.
			self->mInterruptPipeStalled = true;
		}
		else if ( kIOReturnSuccess != status )
		{
			debugIOLog ("! DJM01AudioDevice[%p]::statusInterruptHandler () error from USB: 0x%X, reposting read to interrupt pipe", self, status);
		}
...
void	DJM01AudioDevice::processStatusInterrupt ( void * arg )
{
	DJM01AudioDevice *		self;
	IOCommandGate *				cg;
	
	FailIf ( NULL == arg, Exit );
	self = ( DJM01AudioDevice * )arg;
...
IOReturn DJM01AudioDevice::runStatusInterruptTask( OSObject * target, void * arg0, void * arg1, void * arg2, void * arg3 )
{
	DJM01AudioDevice *		self;
...
void DJM01AudioDevice::handleStatusInterrupt ( void )
{
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::handleStatusInterrupt ()", this);
	...
	{
		debugIOLog ("! DJM01AudioDevice[%p]::handleStatusInterrupt () clearing pipe stall", this);
		if ( mInterruptPipe )
		{
		...
		if ( originatedFromACInterface )	// AudioControl interface
		{
			...
			if ( ( FEATURE_UNIT == subType ) || ( SELECTOR_UNIT == subType ) )	//	<rdar://6523676>
			{
				...
				for ( UInt8 engineIndex = 0; engineIndex < mRegisteredEngines->getCount (); engineIndex++ )
				{
					...
					currentEngine = OSDynamicCast ( DJM01AudioEngine, currentEngineInfo->getObject ( kEngine ) );
			...
			{
				debugIOLog ("? DJM01AudioDevice[%p]::handleStatusInterrupt () - CLOCK_SOURCE : %d", this, bOriginator );
				FailIf ( NULL == mRegisteredEngines, Exit );
				for ( UInt8 engineIndex = 0; engineIndex < mRegisteredEngines->getCount (); engineIndex++ )
				{
					currentEngineInfo = OSDynamicCast ( OSDictionary, mRegisteredEngines->getObject ( engineIndex ) );
...
void DJM01AudioDevice::controlHasChangedOnDevice ( UInt8 controlID, OSSet * defaultAudioControls )
{
										...
										if ( NULL != selectionDictionary )
										{
											...
											if ( NULL != selectionNumber )
											{
												...
												if ( ( selection & 0x000000FF ) == newSelectorPosition )
												{
													...
													debugIOLog ( "? DJM01AudioDevice[%p]::controlHasChangedOnDevice () - Switch input selector over to selection = 0x%x", this, selection );
...
IOReturn DJM01AudioDevice::performPowerStateChange (IOAudioDevicePowerState oldPowerState, IOAudioDevicePowerState newPowerState, UInt32 *microSecsUntilComplete) {
	IOReturn						result;
	bool							performDeviceResetOnWake = false;
	IOUSBDevice *					usbDevice = NULL;
	
	// debugIOLog ("+ DJM01AudioDevice[%p]::performPowerStateChange (%d, %d, %p)", this, oldPowerState, newPowerState, microSecsUntilComplete);
			...
			&&	(kIOAudioDeviceSleep == newPowerState))
	{
		...
		debugIOLog ("? DJM01AudioDevice[%p]::performPowerStateChange () - Going to sleep - stopping the rate timer.", this);
		mUpdateTimer->cancelTimeout ();
	...
	if (oldPowerState == kIOAudioDeviceSleep) 
	{
		...
		if ( kIOReturnSuccess == usbDevice->message( kIOUSBMessageHubIsDeviceConnected, NULL, 0 ) )	// <rdar://7499125>
		{
			...
			{
				debugIOLog ("? DJM01AudioDevice[%p]::performPowerStateChange () - Resetting port after wake from sleep ...", this);
		...
		debugIOLog ("? DJM01AudioDevice[%p]::performPowerStateChange () - Waking from sleep - restarting the rate timer.", this);
		TimerAction ( this, mUpdateTimer);
		
		debugIOLog ("? DJM01AudioDevice[%p]::performPowerStateChange () - Flushing controls to the device ...", this);
		flushAudioControls ();
	}
...
void DJM01AudioDevice::stop (IOService *provider) {

	debugIOLog ("+ DJM01AudioDevice[%p]::stop (%p) - audioEngines = %p - rc=%d", this, provider, audioEngines, getRetainCount());
	...
	if (mUpdateTimer)
	{
		...
		debugIOLog ("? DJM01AudioDevice[%p]::stop () - Cancelling time stamp rate timer ...", this);
		mUpdateTimer->cancelTimeout ();
	...
	debugIOLog ("? DJM01AudioDevice[%p]::stop () - mControlInterface now closing ...", this);
	...
	debugIOLog("- DJM01AudioDevice[%p]::stop ()", this);
}

// <rdar://problem/7666699>
bool DJM01AudioDevice::allEnginesStopped ( void )
{
	bool result = TRUE;
...
bool DJM01AudioDevice::createAudioEngines (void) {
...
//	OSBoolean *	useSingleAudioEngine = NULL;
	...
	// Find USB interfaces.
	debugIOLog ("+ DJM01AudioDevice[%p]::createAudioEngines ()", this);

	FailIf (NULL == mControlInterface, Exit);
	
//	FailIf (kIOReturnSuccess != mConfigDictionary->getControlledStreamNumbers (&streamNumberArray, &numStreams), Exit);
//	FailIf (kIOReturnSuccess != mConfigDictionary->getNumStreamInterfaces (&numStreamInterfaces), Exit);
//	debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - streamNumberArray = %p, numStreams = %lu, numStreamInterfaces = %lu", this, streamNumberArray, numStreams, numStreamInterfaces);
...
/*	
	if (NULL != useSingleAudioEngine)
	{
		...
		{
			debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - Forced (via override kext) single audio engine", this);
			FailMessage ( false == ( result = createAudioEngine ( streamNumberArray ) ) );
		}
		else
		{
			debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - Forced (via override kext) separate audio engines", this);
	...
	{
		debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - Known devices that requires separate audio engines", this);
	...
	else
	{
		...
		{
			debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - Known single sample rate device", this);
			...
			while ( NULL != ( commonSampleRatesStreamList = findStreamsWithCommonSampleRates ( availableStreamsList ) ) )
			{
				...
				while ( NULL != ( compatibleEndpointsStreamList = findStreamsWithCompatibleEndpoints ( commonSampleRatesStreamList ) ) )
				{
					...
					{					
						...
						{
							debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - USB Audio 2.0 device", this);
					...
					{
						debugIOLog ("? DJM01AudioDevice[%p]::createAudioEngines () - USB Audio 1.0 device", this);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::createAudioEngines () - 0x%x", this, result);
	
	return result;
}

bool DJM01AudioDevice::activateAudioEngines (void) {
	
	bool	result = true;

	debugIOLog ("+ DJM01AudioDevice[%p]::activateAudioEngines ()", this);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::activateAudioEngines () - 0x%x", this, result);
	
	return result;
}

bool DJM01AudioDevice::createAudioEngine (OSArray * streamInterfaceNumberArray) {

	DJM01AudioEngine *	engine = NULL;
	bool					result = false;
	OSArray *				newStreamInterfaceArray = NULL;	//  <rdar://problem/6892754>
//	bool					formatSupported;				//  <rdar://problem/6892754>
	
	debugIOLog ( "+ DJM01AudioDevice[%p]::createAudioEngine ()", this );
	...
	debugIOLog ( "? DJM01AudioDevice[%p]::createAudioEngine () - create audio engine with stream interfaces:", this );
...
//			if ( ( kIOReturnSuccess != interfaceContainsSupportedFormat ( streamInterfaceNumber->unsigned8BitValue (), &formatSupported ) ) || formatSupported )
		...
		{
			debugIOLog ( "! DJM01AudioDevice[%p]::createAudioEngine () - could not get streamInterfaceNumber", this );
		}
	}
	
	debugIOLog ( "? DJM01AudioDevice[%p]::createAudioEngine () final new array count: %d", this, newStreamInterfaceArray->getCount() );
	...
	if ( newStreamInterfaceArray->getCount() > 0 )
	{
		...
		engine = OSTypeAlloc ( DJM01AudioEngine );
...
IOReturn DJM01AudioDevice::interfaceContainsSupportedFormat ( UInt8 streamInterface, bool *formatSupported ) {
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::interfaceContainsSupportedFormat ()", this );
	...
	*formatSupported = false;
	...
	for ( altSettingIndex = ( startAtZero ? 0 : 1 ); altSettingIndex < numAltSettings; altSettingIndex++ ) 
	{
		...
		{
			IOLog( "WARNING: DJM01Audio has detected an unsupported format type: Type II AC-3" );
		}
	}
	
Exit:
	
	debugIOLog ( "- DJM01AudioDevice[%p]::interfaceContainsSupportedFormat () streamInterface: %u returnCode: %d formatSupported: %u", this, streamInterface, returnCode, *formatSupported );
...
Boolean DJM01AudioDevice::ShouldUpdatePRAM (void) {
	...
	debugIOLog ("? DJM01AudioDevice[%p]::ShouldUpdatePRAM () - speaker's vendorID = 0x%x", this, vendorID);
	...
	if (TRUE == speakersGood) 
	{
		...
		if (kIOUSBVendorIDAppleComputer == vendorID) 
		{
			...
			if (OSDynamicCast (IOUSBRootHubDevice, usbRegEntry)) 
			{
				...
				connectionGood = TRUE;
				debugIOLog ("? DJM01AudioDevice[%p]::ShouldUpdatePRAM () - Directly connected to the root hub", this);
			} 
			else 
			{
...
Exit:
	debugIOLog ("? DJM01AudioDevice[%p]::ShouldUpdatePRAM () - result = %d", this, result);
	return result;
}

Boolean DJM01AudioDevice::FindSoundNode (void) {
...
IOReturn DJM01AudioDevice::message (UInt32 type, IOService * provider, void * arg) {
	DJM01AudioEngine	*	currentEngine = NULL;
	OSDictionary *			currentEngineInfo = NULL;
	
	debugIOLog ("+ DJM01AudioDevice[%p]::message (0x%x, %p) - rc=%d", this, type, provider, getRetainCount ());
	...
	switch (type) 
	{
		...
		case kIOUSBMessagePortHasBeenReset:
			...
			debugIOLog ("? DJM01AudioDevice[%p]::message () - Flushing controls to the device.", this);
			flushAudioControls ();
			FailIf (NULL == mRegisteredEngines, Exit);
			debugIOLog ("? DJM01AudioDevice[%p]::message () - Resetting engines.", this);
			for (UInt8 engineIndex = 0; engineIndex < mRegisteredEngines->getCount (); engineIndex++)
			{
				currentEngineInfo = OSDynamicCast (OSDictionary, mRegisteredEngines->getObject (engineIndex));
				FailIf (NULL == currentEngineInfo, Exit);
				...
				debugIOLog ("? DJM01AudioDevice[%p]::message () - Resetting engine %p...", this, currentEngine);
				...
				if (currentEngine)
				{
					...
					//(mitani)2010/12/16追加でスリープから復帰するようになる
					mControlInterface->close (this);
...
IOUSBInterface * DJM01AudioDevice::getUSBInterface ( UInt8 interfaceNumber ) {
...
const IOUSBConfigurationDescriptor * DJM01AudioDevice::getConfigurationDescriptor () {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::getConfigurationDescriptor ()", this);
	...
	debugIOLog ("? DJM01AudioDevice[%p]::getConfigurationDescriptor () - numConfigs = %d", this, numConfigs);
	
	if (1 < numConfigs)
	{
		usbDevice->GetConfiguration (&currentConfigValue);	

		debugIOLog ("? DJM01AudioDevice[%p]::getConfigurationDescriptor () - currentConfigValue = %d", this, currentConfigValue);
		...
		for (index = 0; index < numConfigs; index++)
		{
			...
			{
				debugIOLog ("? DJM01AudioDevice[%p]::getConfigurationDescriptor () - Found config %d (%p) at index %d", this, currentConfigValue, descriptor, index);
...
Exit:
	debugIOLog ("- DJM01AudioDevice[%p]::getConfigurationDescriptor () - configDescriptor=%p", this, configDescriptor);
	return configDescriptor;
}

// added for rdar://4168019 . Returns the device speed (high, full, or low).
UInt8 DJM01AudioDevice::getDeviceSpeed () {
	IOUSBDevice *			usbDevice;
	UInt8					speed = 0;
	...
	switch (speed)
	{
		case kUSBDeviceSpeedLow:
			debugIOLog ("? DJM01AudioDevice[%p]::getDeviceSpeed () = kUSBDeviceSpeedLow", this);
			break;
		case kUSBDeviceSpeedFull:
			debugIOLog ("? DJM01AudioDevice[%p]::getDeviceSpeed () = kUSBDeviceSpeedFull", this);
			break;
		case kUSBDeviceSpeedHigh:
			debugIOLog ("? DJM01AudioDevice[%p]::getDeviceSpeed () = kUSBDeviceSpeedHigh", this);
			break;
		default:
			debugIOLog ("? DJM01AudioDevice[%p]::getDeviceSpeed () = %d (UNKNOWN)", this, speed);
	}
	#endif
...
UInt64 DJM01AudioDevice::getUSBFrameNumber () {
	IOUSBDevice *			usbDevice;
...
UInt8 DJM01AudioDevice::getManufacturerStringIndex () {
	IOUSBDevice *			usbDevice;
	UInt8					stringIndex = 0;
...
UInt8 DJM01AudioDevice::getProductStringIndex () {
	IOUSBDevice *			usbDevice;
	UInt8					stringIndex = 0;
...
UInt8 DJM01AudioDevice::getSerialNumberStringIndex () {
	IOUSBDevice *			usbDevice;
	UInt8					stringIndex = 0;
...
IOReturn DJM01AudioDevice::getStringDescriptor(UInt8 index, char *buf, int maxLen, UInt16 lang) {
	IOUSBDevice *			usbDevice;
...
UInt16 DJM01AudioDevice::getVendorID () {
	IOUSBDevice *			usbDevice;
	UInt16					vendorID = 0;
...
UInt16 DJM01AudioDevice::getProductID () {
	IOUSBDevice *			usbDevice;
	UInt16					productID = 0;
...
OSNumber * DJM01AudioDevice::getLocationID () {
	IOUSBDevice *			usbDevice;
...
bool DJM01AudioDevice::detectSplitTransactions () {
	IOUSBDevice *			usbDevice;
	...
	{
		debugIOLog ("? DJM01AudioDevice::detectSplitTransactions () - This is a high speed device, so there are no split transactions.");
				...
				&&	(usbDevice))
	{
		...
		if (kUSBDeviceSpeedHigh == speed)
		{
			...
			debugIOLog ("? DJM01AudioDevice::detectSplitTransactions () = true");
...
Boolean DJM01AudioDevice::checkForUHCI () {
	...
	if (!strcmp (serviceName, "AppleUSBUHCI"))
	{
		...
		debugIOLog ("? DJM01AudioDevice::checkForUHCI () - UHCI connection detected!\n");
		result = TRUE;
	}
Exit:
	return result;
...
// <rdar://6420832>
IOReturn DJM01AudioDevice::registerEngineInfo (DJM01AudioEngine * usbAudioEngine) {
...
SInt32 DJM01AudioDevice::getEngineInfoIndex (DJM01AudioEngine * inAudioEngine) {
	OSDictionary *						engineInfo;
	DJM01AudioEngine *				usbAudioEngine;
	UInt16								engineIndex;
	SInt32								returnIndex;
	...
	if (mRegisteredEngines) 
	{
		for (engineIndex = 0; engineIndex < mRegisteredEngines->getCount (); engineIndex++) 
		{
			...
			if (engineInfo) 
			{
				usbAudioEngine = OSDynamicCast (DJM01AudioEngine, engineInfo->getObject (kEngine));
...
// <rdar://6420832>
IOReturn DJM01AudioDevice::registerStreamInfo (UInt8 interfaceNum, UInt8 altSettingNum) {
...
// <rdar://6420832>
SInt32 DJM01AudioDevice::getStreamInfoIndex (UInt8 interfaceNum) {
	OSDictionary *						streamInfo;
	OSNumber *							streamInterfaceNumber;
	UInt16								streamIndex;
	SInt32								returnIndex;
...
IOReturn DJM01AudioDevice::doControlStuff (IOAudioEngine *audioEngine, UInt8 interfaceNum, UInt8 altSettingNum) {
	DJM01AudioEngine *				usbAudioEngine;
	...
	debugIOLog ("+ DJM01AudioDevice::doControlStuff(0x%x, %d, %d)", audioEngine, interfaceNum, altSettingNum);
	result = kIOReturnError;
	inputSelector = NULL;
	done = FALSE;

    usbAudioEngine = OSDynamicCast (DJM01AudioEngine, audioEngine);
    FailIf (NULL == usbAudioEngine, Exit);
	FailIf (NULL == mControlInterface, Exit);
	debugIOLog ("? DJM01AudioDevice::doControlStuff () - This usbAudioEngine = %p", usbAudioEngine);
...
//(mitani)
	...
	if (direction == kIOAudioStreamDirectionOutput) 
	{
		...
		debugIOLog("? DJM01AudioDevice::doControlStuff () - numConnectedOutputTerminals = %d inputTerminalID: %d", numConnectedOutputTerminals, inputTerminalID);
		...
		for (outputTerminalIndex = 0; outputTerminalIndex < numOutputTerminals && FALSE == done; outputTerminalIndex++) 
		{
			...
			if (terminalType != USB_STREAMING) 
			{
				...
				for (pathsToOutputTerminalN = 0; ( pathsToOutputTerminalN < numOutputTerminalArrays ) && !finished; pathsToOutputTerminalN++) 
				{
					...
					// <rdar://problem/6656791> DJM01Audio: Output controls not set up properly for stream interfaces
						...
						if ( ( inputUnitID == inputTerminalID ) && ( outputUnitID == outputTerminalID ) )	//	<rdar://6413207>
						{
							if ( !hasInitializedOutputControls )
							{
								...
								if (featureUnitID) 
								{
									...
									debugIOLog("? DJM01AudioDevice::doControlStuff () - Creating output gain controls");
								...
								if (featureUnitID) 
								{
									debugIOLog("? DJM01AudioDevice::doControlStuff () - Creating output mute controls");
			...
			for ( arrayOfPathsFromOutputTerminalIndex = 0; ( arrayOfPathsFromOutputTerminalIndex < numPathsFromOutputTerminals ) && !finished; arrayOfPathsFromOutputTerminalIndex++ )
			{
				...
				if (unitID == outputTerminalID) 
				{
					...
					if (numPathsFromOutputTerminal > 1 && numSelectorUnits) 
					{
						...
						for (unitIndexInPath = 1; unitIndexInPath < numUnitsInPath; unitIndexInPath++) 
						{
							...
							{							
								...
								{
									debugIOLog ( "? DJM01AudioDevice::doControlStuff () - Unable to get the selector setting. Defaulting to 1" );
									defaultSelectorSetting = 1;
								}
								debugIOLog ( "? DJM01AudioDevice::doControlStuff () - Default selector setting: %d", defaultSelectorSetting );
								...
								debugIOLog ( "? DJM01AudioDevice::doControlStuff () - Selected aPath = %p, pathIndex = %d, inputTerminalID = %d", aPath, pathIndex, inputTerminalID );
								...
								if (kIOReturnSuccess == setSelectorSetting (unitID, defaultSelectorSetting))	//	<rdar://6523676>
								{
									...
									if ( ( -1 != engineIndex ) && ( -1 != streamIndex ) ) 	//	<rdar://6420832>
									{
										...
										if (featureUnitID) 
										{
											...
											debugIOLog("? DJM01AudioDevice::doControlStuff () - Creating input gain controls");
											...
											debugIOLog("? DJM01AudioDevice::doControlStuff () - Creating input mute controls");
						...
						else 
						{
							...
							if (featureUnitID) 
							{
								...
								debugIOLog("? DJM01AudioDevice::doControlStuff () - Creating playthrough volume controls");
								addVolumeControls (usbAudioEngine, featureUnitID, inputTerminalID, interfaceNum, altSettingNum, kIOAudioControlUsageInput);	// <rdar://6413207>
								featureUnitID = 0;
							}
							...
							if (featureUnitID) 
							{
								...
								debugIOLog ( "? DJM01AudioDevice::doControlStuff () - Creating input mute control (no selectors)" );
								addMuteControl (usbAudioEngine, featureUnitID, inputTerminalID, interfaceNum, altSettingNum, kIOAudioControlUsageInput);		//	<rdar://6413207>
								featureUnitID = 0;
							}
						}
					...
					else 
					{
						...
						{
							// Create the playthrough volume controls
							debugIOLog ( "? DJM01AudioDevice::doControlStuff () - Creating input volume control (no selectors)" );
							addVolumeControls (usbAudioEngine, featureUnitID, inputTerminalID, interfaceNum, altSettingNum, kIOAudioControlUsageInput);		// <rdar://6413207>
							featureUnitID = 0;
						}
						featureUnitID = getBestFeatureUnitInPath (aPath, kIOAudioControlUsageInput, interfaceNum, altSettingNum, kMuteControl);
					...
					if (playThroughPaths) 
					{
						...
						debugIOLog("? DJM01AudioDevice::doControlStuff () - performing playthrough setup");
...
IOReturn DJM01AudioDevice::doPlaythroughSetup (DJM01AudioEngine * usbAudioEngine, OSArray * playThroughPaths, UInt8 interfaceNum, UInt8 altSettingNum, UInt8 inputTerminalID) {	//	<rdar://5366067>
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	OSString *							nameString;
...
//  <rdar://problem/6656791>
UInt32 DJM01AudioDevice::getPathIndexForSelectorSetting (OSArray * arrayOfPathsFromOutputTerminal, UInt32 pathsToOutputTerminalN, UInt32 graphPathIndex, UInt8 selectorUnitIndex, UInt8 selectorSetting) {
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	OSString *							nameString;
...
// <rdar://problem/6656791>
IOReturn DJM01AudioDevice::addSelectorSourcesToSelectorControl (IOAudioSelectorControl * theSelectorControl, OSArray * arrayOfPathsFromOutputTerminal, UInt32 pathsToOutputTerminalN, UInt32 graphPathIndex, UInt8 selectorIndex) {
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	OSString *							nameString;
	UInt32								selectorSourceIndex;
...
//	<rdar://6413207>
UInt8 DJM01AudioDevice::getDefaultOutputTerminalID (UInt8 inputTerminalID) {
	OSArray *							arrayOfPathsFromOutputTerminal;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	UInt8								controlInterfaceNum;
...
//	<rdar://6413207>
UInt32 DJM01AudioDevice::getNumConnectedOutputTerminals (UInt8 inputTerminalID) {
	OSArray *							arrayOfPathsFromOutputTerminal;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	UInt8								controlInterfaceNum;
...
//	<rdar://6430836>
OSString * DJM01AudioDevice::getNameForTerminal (UInt8 terminalID, UInt8 direction) {
	...
	if ( ( kIOReturnSuccess == mConfigDictionary->getStringIndex ( &stringIndex, controlInterfaceNum, 0, terminalID ) ) &&
		 ...
		 ( kIOReturnSuccess == getStringDescriptor ( stringIndex, stringBuffer, kStringBufferSize ) ) )
	{						 
		debugIOLog ("? DJM01AudioDevice::getNameForTerminal () - terminalID = %d, stringIndex = %d, stringBuffer = %s", terminalID, stringIndex, stringBuffer);
...
OSString * DJM01AudioDevice::getNameForPath (OSArray * arrayOfPathsFromOutputTerminal, UInt32 * pathIndex, UInt8 startingPoint) {
	OSString *							theString;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
...
OSString * DJM01AudioDevice::getNameForMixerPath (OSArray * arrayOfPathsFromOutputTerminal, UInt32 * pathIndex, UInt8 startingPoint) {
	char								string[255];
	OSString *							theString;
	OSString *							tempString;
	OSArray *							aPath;
...
void DJM01AudioDevice::addVolumeControls (DJM01AudioEngine * usbAudioEngine, UInt8 featureUnitID, UInt8 terminalID, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 usage) {	// <rdar://6413207>
	...
	debugIOLog ("+ DJM01AudioDevice::addVolumeControls (0x%x, %d, %d, %d, %d, %u)", usbAudioEngine, featureUnitID, terminalID, interfaceNum, altSettingNum, usage);
	FailIf (NULL == mControlInterface, Exit);
	...
	for (channelNum = 0; channelNum <= numControls; channelNum++) 
	{
		...
		{
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Creating volume controls for channel %d", this, channelNum);
			FailIf (kIOReturnSuccess != getCurVolume (featureUnitID, channelNum, &deviceCur), Error);
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - deviceCur = 0x%04x", this, deviceCur);
			FailIf (kIOReturnSuccess != getMinVolume (featureUnitID, channelNum, &deviceMin), Error);
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - deviceMin = 0x%04x", this, deviceMin);
			FailIf (kIOReturnSuccess != getMaxVolume (featureUnitID, channelNum, &deviceMax), Error);
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - deviceMax = 0x%04x", this, deviceMax);
			getVolumeResolution (featureUnitID, channelNum, &volRes);
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - volRes = 0x%04x", this, volRes);
			// [rdar://4511427] Need to check volRes for class compliance (Audio Spec 5.2.2.4.3.2).
			...
			if ( ( SInt16 ) kNegativeInfinity != deviceCur )
			{
				...
				if (		( deviceCur < deviceMin )
						||	( deviceCur > deviceMax ) )
				{
					debugIOLog ( "! DJM01AudioDevice::addVolumeControls () - deviceCur is not in volume range! Setting to deviceMin ..." );
					deviceCur = deviceMin;
				}
				debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Attempting to set volume to current volume ...", this);
				FailIf (kIOReturnSuccess != setCurVolume (featureUnitID, channelNum, HostToUSBWord(deviceCur)), Error);
				debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Volume was set successfully.", this);
			}

			if ( (SInt16) kNegativeInfinity == deviceMin ) 
			{
				debugIOLog ( "! DJM01AudioDevice[%p]::addVolumeControls () - device violates USB 2.0 audio spec Section 5.2.5.7.2! Setting deviceMin to 0x8001 ..." );
			...
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Creating control... [%d : %d : %d]", this, controlMin, controlCur, controlMax);
			...
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Created control %p", this, theLevelControl);
			...
			switch (usage) 
			{
				...
				case kIOAudioControlUsageOutput:
					...
					if (mDeviceIsInMonoMode)
					{
						...
						debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Added channel %d to mono controls array", this, channelNum);
						number->release ();
					}
					
					break;
				default:
					debugIOLog ("! DJM01AudioDevice[%p]::addVolumeControls () - Control %p has an unknown usage!", this, theLevelControl);
			}
			theLevelControl->release ();
		} 
		else 
		{
			debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Channel %d has no volume controls; skipping ...", this, channelNum);
		}
		goto NoError;
Error: 
		debugIOLog ("! DJM01AudioDevice[%p]::addVolumeControls () - Error creating controls for channel %d!", this, channelNum);
NoError:
		debugIOLog ("? DJM01AudioDevice[%p]::addVolumeControls () - Done with channel %d", this, channelNum);
	}
...
void DJM01AudioDevice::addMuteControl (DJM01AudioEngine * usbAudioEngine, UInt8 featureUnitID, UInt8 terminalID, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 usage)	// <rdar://6413207>
{
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::addMuteControl ( %p, %d, %d, %d, %d, 0%x )", this, usbAudioEngine, featureUnitID, terminalID, interfaceNum, altSettingNum, usage );
	FailIf ( NULL == mControlInterface, Exit );
	...
	for ( channelNum = 0; channelNum <= numControls; channelNum++ ) 
	{
		if ( mConfigDictionary->channelHasMuteControl ( controlInterfaceNum, 0, featureUnitID, channelNum ) )
		{
			...
			debugIOLog ( "? DJM01AudioDevice::addMuteControl () - channel %d, feature unit %d has current mute value 0x%x", channelNum, featureUnitID, deviceCur );
			...
			{
				debugIOLog ("! DJM01AudioDevice::addMuteControl () - forcing channel %d of this device to unmute in hardware.", channelNum );
				deviceCur = 0x0;
			}
			// <rdar://6413207> Add the terminal ID to the control ID so that it is unique. 
...
Exit:
	debugIOLog ( "- DJM01AudioDevice[%p]::addMuteControl ( %p, %d, %d, %d, %d, 0x%x )", this, usbAudioEngine, featureUnitID, terminalID, interfaceNum, altSettingNum, usage );
	return;
}
...
IOFixed DJM01AudioDevice::ConvertUSBVolumeTodB (SInt16 volume) {
...
IOReturn DJM01AudioDevice::getFeatureUnitRange (UInt8 controlSelector, UInt8 unitID, UInt8 channelNumber, UInt8 requestType, SubRange16 * target) {
    IOReturn							result;
	IOUSBDevRequestDesc					devReq;
	struct {
...
IOReturn DJM01AudioDevice::getFeatureUnitSetting (UInt8 controlSelector, UInt8 unitID, UInt8 channelNumber, UInt8 requestType, SInt16 * target) {
    IOReturn							result;
	IOUSBDevRequestDesc					devReq;
	UInt16								theSetting;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
...
IOReturn DJM01AudioDevice::setFeatureUnitSetting (UInt8 controlSelector, UInt8 unitID, UInt8 channelNumber, UInt8 requestType, UInt16 newValue, UInt16 newValueLen) {
    IOUSBDevRequestDesc					devReq;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
	IOReturn							result;
...
DeviceInactive:
	debugIOLog("? DJM01AudioDevice::setFeatureUnitSetting () - ERROR attempt to send a device request to an inactive device");
	goto Exit;
}

IOReturn DJM01AudioDevice::getCurMute (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM01AudioDevice::getCurVolume (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM01AudioDevice::getMaxVolume (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM01AudioDevice::getMinVolume (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM01AudioDevice::getVolumeResolution (UInt8 unitID, UInt8 channelNumber, UInt16 * target) {
	IOReturn							result;
...
IOReturn DJM01AudioDevice::setCurVolume (UInt8 unitID, UInt8 channelNumber, SInt16 volume) {
	return setFeatureUnitSetting (VOLUME_CONTROL, unitID, channelNumber, SET_CUR, volume, 2);
}

IOReturn DJM01AudioDevice::setCurMute (UInt8 unitID, UInt8 channelNumber, SInt16 mute) {
	return setFeatureUnitSetting (MUTE_CONTROL, unitID, channelNumber, SET_CUR, mute, 1);
}

IOReturn DJM01AudioDevice::controlChangedHandler (OSObject * target, IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
	DJM01AudioDevice *				self;

	result = kIOReturnError;

	self = OSDynamicCast (DJM01AudioDevice, target);
...
IOReturn DJM01AudioDevice::protectedControlChangedHandler (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
...
IOReturn DJM01AudioDevice::doSelectorControlChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
...
IOReturn DJM01AudioDevice::doVolumeControlChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	IOReturn							result;
	SInt16								newVolume;
	SInt16								deviceMin;
	SInt16								offset;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::doVolumeControlChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
	     ...
	     || (FALSE == mDeviceIsInMonoMode))
	{
		...
		debugIOLog ("? DJM01AudioDevice[%p]::doVolumeControlChange () - Setting channel %d (unit %d) volume to 0x%x", this, channelNum, unitID, newVolume);
	...
	{	// mono output case
		...
		debugIOLog ("? DJM01AudioDevice[%p]::doVolumeControlChange () - Performing mono volume control change", this);
		for (i = 0; i < mMonoControlsArray->getCount (); i++)
		{
			...
			result = setCurVolume (unitID, channelNum, HostToUSBWord (newVolume));
			debugIOLog ("? DJM01AudioDevice[%p]::doVolumeControlChange () - Set volume for channel %d to 0x%x = %d", this, channelNum, newVolume, result);
		}
	}
	
Exit:
	debugIOLog ("- DJM01AudioDevice[%p]::doVolumeControlChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, result);
	return result;
}

IOReturn DJM01AudioDevice::doToggleControlChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	IOReturn							result;
	UInt8								unitID;
	UInt8								channelNum;

	debugIOLog ("+ DJM01AudioDevice[%p]::doToggleControlChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);

	unitID = audioControl->getControlID () & 0xFF;		// <rdar://6413207>
	channelNum = audioControl->getChannelID ();

	debugIOLog ("? DJM01AudioDevice[%p]::doToggleControlChange( %p, 0x%x, 0x%x ) - unitID = %d, channelNum = %d", this, audioControl, oldValue, newValue, unitID, channelNum);

	result = setCurMute (unitID, channelNum, HostToUSBWord (newValue));

	debugIOLog ("- DJM01AudioDevice[%p]::doToggleControlChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, kIOReturnSuccess);

	return result;
}

IOReturn DJM01AudioDevice::doPassThruSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	DJM01AudioEngine *				usbAudioEngine;
	OSArray *							playThroughPaths;
	OSArray *							passThruVolControlsArray;
	OSArray *							passThruToggleControlsArray;	//	<rdar://5366067>
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::doPassThruSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);

	if ( oldValue != newValue )		//	<rdar://5366067>
	{
		pathIndex = (newValue & 0x000000FF);
		
		debugIOLog ("? DJM01AudioDevice[%p]::doPassThruSelectorChange( %p, 0x%x, 0x%x ) - controlID = 0x%x", this, audioControl, oldValue, newValue, audioControl->getControlID ());
...
Exit:
	debugIOLog ("- DJM01AudioDevice[%p]::doPassThruSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, kIOReturnSuccess);
	return kIOReturnSuccess;
}

IOReturn DJM01AudioDevice::doInputSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	DJM01AudioEngine *				usbAudioEngine;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::doInputSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
	...
	if ( oldValue != newValue )	//	<rdar://5366067>
	{
		...
		if ( !foundMixerUnit )
		{
			...
			if (playThroughPaths) 
			{
				debugIOLog("? DJM01AudioDevice::doInputSelectorChange () - performing playthrough setup");
...
Exit:
	debugIOLog ("- DJM01AudioDevice[%p]::doInputSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, result);
	return result;
}

//	<rdar://6413207>
IOReturn DJM01AudioDevice::doOutputSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	DJM01AudioEngine *				usbAudioEngine;
	OSArray *							arrayOfPathsFromOutputTerminal;
	UInt32								numOutputTerminalArrays;
	UInt32								pathsToOutputTerminalN;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::doOutputSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
...
Exit:
	debugIOLog ("- DJM01AudioDevice[%p]::doOutputSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, kIOReturnSuccess);
	return kIOReturnSuccess;
}
...
OSArray * DJM01AudioDevice::getPlaythroughPaths (UInt8 inputTerminalID) {	//	<rdar://5366067>
	OSArray *							arrayOfPathsFromOutputTerminal;
	OSArray *							playThroughPaths;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
...
UInt8 DJM01AudioDevice::getBestFeatureUnitInPath (OSArray * thePath, UInt32 direction, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 controlTypeWanted) {
	OSNumber *							theUnitIDNum;
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::getBestFeatureUnitInPath (%p, %lu, %d, %d, %lu)", this, thePath, direction, interfaceNum, altSettingNum, controlTypeWanted );
	featureUnitID = 0;
	FailIf (NULL == mControlInterface, Exit);
	FailIf (NULL == thePath, Exit);
	controlInterfaceNum = mControlInterface->GetInterfaceNumber ();
	...
	switch (direction) 
	{
		case kIOAudioControlUsagePassThru:
			...
			for (unitIndex = numUnitsInPath - 2; unitIndex > endingUnitIndex && !foundFeatureUnit; unitIndex--) 
			{
				...
				{
					debugIOLog ( "! DJM01AudioDevice::getBestFeatureUnitInPath () - something is wrong here!!!" );
				}
			} // for unitIndex
			break;
		case kIOAudioControlUsageInput:
		...
		default:
			...
			debugIOLog ("? DJM01AudioDevice::getBestFeatureUnitInPath () - kIOAudioControlUsageOutput ");
...
Exit:
	debugIOLog ( "- DJM01AudioDevice[%p]::getBestFeatureUnitInPath () = %d", this, featureUnitID );
	return featureUnitID;
}

UInt8 DJM01AudioDevice::pathsContaining (UInt8 unitID)
{
...
//	<rdar://5366067>
UInt8 DJM01AudioDevice::pathsContainingFeatureUnitButNotMixerUnit (UInt8 featureUnitID, UInt8 mixerUnitID)
{
	OSArray *	thisPathGroup = NULL;
	OSArray *	thisPath = NULL;
	OSNumber *  unitNumber = NULL;
...
UInt8 DJM01AudioDevice::getSelectorSetting (UInt8 selectorID) {
    IOUSBDevRequestDesc					devReq;
	IOReturn							result;
	UInt8								setting;
...
IOReturn DJM01AudioDevice::setSelectorSetting (UInt8 selectorID, UInt8 setting) {
    IOUSBDevRequestDesc					devReq;
	IOReturn							result;
	IOBufferMemoryDescriptor *			settingDesc = NULL;
...
void DJM01AudioDevice::setMonoState (Boolean state)
{
	mDeviceIsInMonoMode = state;
}

IOReturn DJM01AudioDevice::createControlsForInterface (IOAudioEngine *audioEngine, UInt8 interfaceNum, UInt8 altSettingNum) {
    DJM01AudioEngine *				usbAudioEngine;
    IOReturn							result;
    debugIOLog ("? DJM01AudioDevice[%p]::createControlsForInterface () - Interface %d alternate setting %d", this, interfaceNum, altSettingNum);
...
// [rdar://4867843]
OSArray * DJM01AudioDevice::getOptimalClockPath ( DJM01AudioEngine * thisEngine, UInt8 streamInterface, UInt8 altSetting, UInt32 sampleRate, Boolean * otherEngineNeedSampleRateChange, UInt8 * clockPathGroupIndex )	//	<rdar://5811247>
{
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::getOptimalClockPath ( %d, %d, %d )", this, streamInterface, altSetting, sampleRate );
	FailIf ( NULL == mConfigDictionary, Exit );
	FailIf ( NULL == mControlInterface, Exit );
	...
	debugIOLog (" ? DJM01AudioDevice::getOptimalClockPath () - interface %d, alt setting %d, clock source ID %d", streamInterface, altSetting, clockSourceID );
...
Exit:
	...
	debugIOLog ( "- DJM01AudioDevice[%p]::getOptimalClockPath ( %d, %d, %d ) = %p", this, streamInterface, altSetting, sampleRate, optimalPathArray );
	return optimalPathArray;
}

OSArray * DJM01AudioDevice::getClockPathGroup ( UInt8 streamInterface, UInt8 altSetting, UInt8 * clockPathGroupIndex )	//	<rdar://5811247>
{
	OSNumber *						clockSourceIDNumber = NULL;
	OSArray *						pathGroupArray = NULL;
	OSArray *						pathArray = NULL;
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::getClockPathGroup ( %d, %d )", this, streamInterface, altSetting );
	FailIf ( NULL == mConfigDictionary, Exit );
	FailIf ( NULL == mControlInterface, Exit );
	...
	debugIOLog (" ? DJM01AudioDevice::getClockPathGroup () - interface %d, alt setting %d, clock source ID %d", streamInterface, altSetting, clockSourceID );
...
Exit:
	...
	debugIOLog ( "- DJM01AudioDevice[%p]::getClockPathGroup ( %d, %d ) = %p", this, streamInterface, altSetting, pathGroupArray );
	return pathGroupArray;
}

//	<rdar://5811247>
OSArray * DJM01AudioDevice::getClockPathGroup ( UInt8 pathGroupIndex )
{
	OSArray *						pathGroupArray = NULL;
	
	debugIOLog ( "+ DJM01AudioDevice[%p]::getClockPathGroup ( %d )", this, pathGroupIndex );
...
Exit:
	...
	debugIOLog ( "- DJM01AudioDevice[%p]::getClockPathGroup ( %d ) = %p", this, pathGroupIndex, pathGroupArray );
	return pathGroupArray;
}

//	<rdar://5811247>
IOReturn DJM01AudioDevice::getClockSelectorIDAndPathIndex (UInt8 * selectorID, UInt8 * pathIndex, OSArray * clockPath) {
	IOReturn				result;
...
Boolean DJM01AudioDevice::supportSampleRateInClockPath ( OSArray * pathArray, UInt32 sampleRate )
{
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::supportSampleRateInClockPath ()", this );
...
Exit:
	debugIOLog ( "- DJM01AudioDevice[%p]::supportSampleRateInClockPath () = %d", this, sampleRateSupported );
	return sampleRateSupported;
}

UInt32 DJM01AudioDevice::determineClockPathUnitUsage ( DJM01AudioEngine * thisEngine, OSArray * thisClockPath )
{
	UInt32							usageCount = 0;
	SInt32							engineIndex;
	OSDictionary *					otherAudioEngineInfo = NULL;
	DJM01AudioEngine *			otherAudioEngine = NULL;

	debugIOLog ( "+ DJM01AudioDevice[%p]::determineClockPathUnitUsage ( %p, %p )", this, thisEngine, thisClockPath );
	...
	if ( NULL != mRegisteredEngines )
	{
		...
		{	
			...
			if ( NULL == ( otherAudioEngine = OSDynamicCast (DJM01AudioEngine, otherAudioEngineInfo->getObject (kEngine) ) ) )
			{
				break;
			}
			...
			if (thisEngine != otherAudioEngine)
			{
				if ( NULL != otherAudioEngine->mIOAudioStreamArray )
				{
					...
					{
						DJM01AudioStream * audioStream = OSDynamicCast ( DJM01AudioStream, otherAudioEngine->mIOAudioStreamArray->getObject (index) );
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::determineClockPathUnitUsage ( %p, %p ) = %d", this, thisEngine, thisClockPath, usageCount );
	return usageCount;
}

Boolean DJM01AudioDevice::clockPathCrossed ( OSArray * clockPathA, OSArray * clockPathB )
{
...
// [rdar://4867779]
IOReturn DJM01AudioDevice::addSampleRatesFromClockSpace ()
{
	IOReturn						result = kIOReturnError;
	OSNumber *						clockSourceIDNumber = NULL;
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::addSampleRatesFromClockSpace ()", this );
	FailIf ( NULL == mControlInterface, Exit );		// <rdar://7085810>
	FailIf ( NULL == mConfigDictionary, Exit );
	...
	for ( UInt8 streamInterfaceIndex = 0; streamInterfaceIndex < streamInterfaceNumbers->getCount (); streamInterfaceIndex++ )
	{
					...
					altSettingIndex++ )
		{
			...
			debugIOLog (" ? DJM01AudioDevice::addSampleRatesFromClockSpace () - interface %d, alt setting %d, clock source ID %d", streamInterface, altSettingIndex, clockSourceID );
			// Find the path group that begins with the clockSourceID.
			for ( UInt8 pathGroupIndex = 0; pathGroupIndex < mClockGraph->getCount (); pathGroupIndex++ )
			{
				FailIf ( NULL == ( pathGroupArray = OSDynamicCast ( OSArray, mClockGraph->getObject ( pathGroupIndex ) ) ), Exit );
...
Exit:
	debugIOLog ( "- DJM01AudioDevice[%p]::addSampleRatesFromClockSpace () = 0x%x", this, result );
	return result;
}

IOReturn DJM01AudioDevice::addSampleRatesFromClockPath ( OSArray * path, UInt8 streamInterface, UInt8 altSetting )
{
	IOReturn						result = kIOReturnError;
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::addSampleRatesFromClockPath ( %p, %d, %d )", this, path, streamInterface, altSetting );
...
Exit:
	...
	debugIOLog ( "- DJM01AudioDevice[%p]::addSampleRatesFromClockPath ( %p, %d, %d ) = 0x%x", this, path, streamInterface, altSetting, result );
	return result;
}

IOReturn DJM01AudioDevice::getClockSourceSampleRates ( OSArray ** sampleRates, UInt8 clockSource )
{
	IOReturn						result = kIOReturnError;
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::getClockSourceSampleRates ( %p, %d )", this, sampleRates, clockSource );
	FailIf ( NULL == mConfigDictionary, Exit );
	...
	* sampleRates = NULL;
		 ...
		 mConfigDictionary->clockSourceHasFrequencyControl ( mControlInterface->GetInterfaceNumber (), 0, clockSource, false ) )
	{
		...
		for ( UInt8 sampleRangeIndex = 0; sampleRangeIndex < numSampleRanges; sampleRangeIndex++ )
		{
			...
			if ( ! subRangeIsValid )
			{
				debugIOLog ( "! DJM01AudioDevice[%p]::getClockSourceSampleRates () - invalid subrange, skipping ...", this );
...
Exit:
	debugIOLog ( "- DJM01AudioDevice[%p]::getClockSourceSampleRates ( %p, %d ) = 0x%x", this, sampleRates, clockSource, result );
	return result;
}

OSArray * DJM01AudioDevice::buildClockGraph ( UInt8 controlInterfaceNum )
{
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::buildClockGraph ( %d )", this, controlInterfaceNum );
	...
	for ( UInt8 clockIndex = 0; clockIndex < terminalClockEntities->getCount (); clockIndex++) 
	{
		...
		debugIOLog ( "? DJM01AudioDevice[%p]::buildClockGraph () - Building clock paths from ID %d", this, clockID ); 
	...
	debugIOLog ( "? DJM01AudioDevice[%p]::buildClockGraph ( %d ) - Displaying graph ...", this, controlInterfaceNum, allClockPaths->getCount() );
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::buildClockGraph (%d) = %p", this, controlInterfaceNum, allClockPaths );
	return allClockPaths;
}

OSArray * DJM01AudioDevice::buildClockPath ( UInt8 controlInterfaceNum, UInt8 startingUnitID, OSArray * allPaths, OSArray * startingPath ) 
{
	...
	debugIOLog ( "+ DJM01AudioDevice[%p]::buildClockPath ( %d, %d, %p, %p )", controlInterfaceNum, startingUnitID, allPaths, startingPath );
	FailIf ( NULL == mConfigDictionary, Exit );
...
Exit:
	...
	debugIOLog ( "- DJM01AudioDevice[%p]::buildClockPath () = %p", curPath );
	return curPath;
}

OSArray * DJM01AudioDevice::BuildConnectionGraph (UInt8 controlInterfaceNum) 
{
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::BuildConnectionGraph (%d)", this, controlInterfaceNum);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::BuildConnectionGraph (%d) = %p", this, controlInterfaceNum, allOutputTerminalPaths);
	return allOutputTerminalPaths;
}

OSArray * DJM01AudioDevice::BuildPath (UInt8 controlInterfaceNum, UInt8 startingUnitID, OSArray * allPaths, OSArray * startingPath) {
	OSArray *						curPath = NULL;
...
char * DJM01AudioDevice::TerminalTypeString (UInt16 terminalType) 
{
...
//	<rdar://5811247>
char * DJM01AudioDevice::ClockTypeString (UInt8 clockType) 
{
...
IOReturn DJM01AudioDevice::deviceRequest (IOUSBDevRequestDesc * request, IOUSBCompletion * completion) {
	IOReturn						result;
	UInt32							timeout;
	Boolean							done;
	...
	debugIOLog ("? DJM01AudioDevice[%p]::deviceRequest (%p, %p) = %lx", this, request, completion, result);
	#endif
Exit:
	return result;
}

IOReturn DJM01AudioDevice::deviceRequest (IOUSBDevRequest * request, IOUSBCompletion * completion) {
	IOReturn						result;
	UInt32							timeout;
	Boolean							done;
...
IOReturn DJM01AudioDevice::deviceRequest (IOUSBDevRequest *request, DJM01AudioDevice * self, IOUSBCompletion *completion) {
	IOReturn						result;
	UInt32							timeout;
	Boolean							done;
	...
	#if LOGDEVICEREQUESTS
	debugIOLog ("? DJM01AudioDevice[%p]::deviceRequest (%p, %p) = %lx", self, request, completion, result);
	#endif
Exit:
	return result;
}

bool DJM01AudioDevice::willTerminate (IOService * provider, IOOptionBits options) {
	debugIOLog ("+ DJM01AudioDevice[%p]::willTerminate (%p)", this, provider);
	...
	debugIOLog ("- DJM01AudioDevice[%p]::willTerminate ()", this);

	return super::willTerminate (provider, options);
}

void DJM01AudioDevice::setConfigurationApp (const char *bundleID) {
	setConfigurationApplicationBundle (bundleID);
}

#ifdef DEBUG

void DJM01AudioDevice::retain() const
{
//    debugIOLog("DJM01AudioDevice(%p)::retain() - rc=%d", this, getRetainCount());
    super::retain();
}

void DJM01AudioDevice::release() const
{
//    debugIOLog("DJM01AudioDevice(%p)::release() - rc=%d", this, getRetainCount());
	super::release();
}
...
bool DJM01AudioDevice::matchPropertyTable(OSDictionary * table, SInt32 *score)
{
	bool		returnValue = false;
	OSObject *	deviceName;
	
	// debugIOLog ("+DJM01AudioDevice[%p]::matchPropertyTable (%p, %p)", this, table, score);
	...
	if (    (deviceName)
		 && (returnValue))
	{
		debugIOLog ("? DJM01AudioDevice[%p]::matchPropertyTable (%p, %p) = %d (custom dictionary match)", 
...
#if DEBUGANCHORS
void DJM01AudioDevice::accumulateAnchor (UInt64 anchorFrame, AbsoluteTime timeStamp)
{
	UInt32		i;
	...
	if (mAnchorFrames[(kAnchorsToAccumulate - 1)] != 0ull)
	{
		...
		debugIOLog ("? DJM01AudioDevice::accumulateAnchor () - Frame # %d accumulated.", kAnchorsToAccumulate);
		for (i = 0; i < kAnchorsToAccumulate ; i++)
		{
...
IOReturn DJM01AudioDevice::getAnchorFrameAndTimeStamp (UInt64 *frame, UInt64 *time) {
	UInt64			finishTime;
	UInt64			offset;
	UInt64			curTime;
	UInt64			prevTime;
	...
	UInt64			diffAbs;
#else
IOReturn DJM01AudioDevice::getAnchorFrameAndTimeStamp (UInt64 *frame, AbsoluteTime *time) {
...
UInt64 DJM01AudioDevice::getWallTimeInNanos (void)
{
#if defined(__i386__) || defined(__LP64__)
	UInt64			time;
#else
...
#if DEBUGANCHORS	
	debugIOLog ("? DJM01AudioDevice::updateAnchorTime () - index: %u X: %llu Y: %llu", anchorTime->index, X, Y);
#endif	
...
UInt64 DJM01AudioDevice::getTimeForFrameNumber ( UInt64 frameNumber )
{
	UInt64 result = 0;
...
void DJM01AudioDevice::updateUSBCycleTime ( void )
{
#if defined(__i386__) || defined(__LP64__)
	UInt64			timeStamp;
#else
...
#if DEBUGTIMESTAMPS	 
		debugIOLog ("? DJM01AudioDevice::updateUSBCycleTime() mWallTimePerUSBCycle = %llu frames elapsed: %llu\n", mWallTimePerUSBCycle, frameNumber - lastAnchorFrame() );
#endif				
...
// <rdar://problem/7666699>
void DJM01AudioDevice::applyOffsetAmountToFilter ( void )
{
	UInt64			timeOffset = 0;
...
#if DEBUGANCHORS						
			debugIOLog ("? DJM01AudioDevice::applyOffsetAmountToFilter timeOffset: %llu framesElapsed: %llu currentFrame: %llu predictedTime: %llu actualTime: %llu", 
...
void DJM01AudioDevice::calculateOffset ( void )
{
	debugIOLog ( "+ DJM01AudioDevice::calculateOffset (%p)", this );
...
void DJM01AudioDevice::resetRateTimer ()
{
	mWallTimePerUSBCycle = 1000000ull * kWallTimeExtraPrecision;
...
// <rdar://problem/7666699>
UInt64 DJM01AudioDevice::lastAnchorFrame ( void )
{
	return ( mAnchorTime.index ? mAnchorTime.X[mAnchorTime.index - 1] : mAnchorTime.X[MAX_ANCHOR_ENTRIES - 1] );
}

void DJM01AudioDevice::TimerAction (OSObject * owner, IOTimerEventSource * sender) 
{
	DJM01AudioDevice *	self;
	
	#if DEBUGTIMER
		debugIOLog ("+ DJM01AudioDevice::TimerAction (%p, %p)", owner, sender);
	#endif
	FailIf (NULL == owner, Exit);
	self = (DJM01AudioDevice *) owner;
	FailIf (NULL == self, Exit);
	self->doTimerAction ( sender );
Exit:
	#if DEBUGTIMER
		debugIOLog ("- DJM01AudioDevice::TimerAction ()");
	#endif
	return;
}

void DJM01AudioDevice::doTimerAction (IOTimerEventSource * timer) 
{
	...
	#if DEBUGTIMER
		debugIOLog ("+ DJM01AudioDevice::doTimerAction (%p)", timer);
	#endif
	...
	if ( ++mTimerCallCount >= ( kRefreshInterval / curRefreshInterval ) )		// Always perform these items at kRefreshInterval
	{
		...
		{
			debugIOLog ("! DJM01AudioDevice[%p]::doTimerAction () - Detected failing audio engine (%p)! Performing emergency format change.", this, mFailingAudioEngine);
		...
		{
			// [rdar://5355808] If DJM01AudioEngine::CoalesceInputSamples () gets more than a USB frame's worth of samples behind, we need to restart the engine.
			debugIOLog ( "! DJM01AudioDevice[%p]::doTimerAction () - Restarting engine %p", this, mEngineToRestart );
...
Exit:
	#if DEBUGTIMER
		debugIOLog ("- DJM01AudioDevice::doTimerAction ()");
	#endif
	return;
}
...
UInt32 DJM01AudioDevice::formatChangeController (IOAudioEngine *audioEngine, IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
	DJM01AudioEngine *			thisAudioEngine = NULL;
	DJM01AudioEngine *			otherAudioEngine = NULL;
	DJM01AudioStream *			thisStream = NULL;
	DJM01AudioStream *			otherStream = NULL;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::formatChangeController (%p, %p, %p, %p)", this, audioEngine, audioStream, newFormat, newSampleRate);
	FailIf (NULL == mControlInterface, Exit);
	
	thisAudioEngine = (DJM01AudioEngine *) audioEngine;
	...
	if (mustMatchFormats)
	{
		debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - Attempting to match this format with the format for the other stream interface.", this);
		...
		debugIOLog ("\n");
		debugIOLog ("-------------------- BEFORE --------------------");
		debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - engine %p (interface %d, alternateSetting %d) info:", this, thisAudioEngine, thisStream->mInterfaceNumber, thisStream->mAlternateSettingID);
		...
		debugIOLog ("\n");
		debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - engine %p (interface %d, alternateSetting %d) info:", this, otherAudioEngine, otherStream->mInterfaceNumber, otherStream->mAlternateSettingID);
		...
		debugIOLog ("\n");
		debugIOLog (" DJM01AudioDevice[%p]::formatChangeController () - newFormat = %p", this, newFormat);
		if (newFormat)
		{
		...
		debugIOLog ( " DJM01AudioDevice[%p]::formatChangeController () - newSampleRate = %p", this, newSampleRate );
		if ( newSampleRate )
		{
		...
		if ( ( false == mSingleSampleRateDevice ) && ( false == otherEngineNeedSampleRateChange ) )
		{
			...
			if (kIOReturnSuccess == mConfigDictionary->getAltSettingWithSettings (&altSetting, thisStream->mInterfaceNumber, thisFormat->fNumChannels, thisFormat->fBitDepth, otherSampleRate->whole))
			{
				...
				{
					debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - This engine (%p) sample rate changed successfully to %lu.", this, thisAudioEngine, otherSampleRate->whole);
			...
			else if (kIOReturnSuccess == mConfigDictionary->getAltSettingWithSettings (&altSetting, otherStream->mInterfaceNumber, otherFormat->fNumChannels, otherFormat->fBitDepth, thisSampleRate->whole))
			{
				...
				if (kIOReturnSuccess == formatChangeReturnCode)
				{
					debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - Other engine (%p) sample rate changed succsesfully to %lu.", this, otherAudioEngine, thisSampleRate->whole);
					result = kAUAFormatChangeForced;
			...
			else
			{
				...
				debugIOLog ("! DJM01AudioDevice[%p]::formatChangeController () - Restoring both engines to their default settings.", this);
				...
				if (kIOReturnSuccess == formatChangeReturnCode)
				{
					debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - This engine (%p) restored to default settings succsesfully.", this, thisAudioEngine);
					result = kAUAFormatChangeForced;
				...
				if (kIOReturnSuccess == formatChangeReturnCode)
				{
					debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - Other engine (%p) restored to default settings succsesfully.", this, otherAudioEngine);
		...
		else
		{
					...
					&&	(newSampleRate->whole != otherSampleRate->whole))
			{
				...
				{
					// Fail this request.
					debugIOLog ("! DJM01AudioDevice[%p]::formatChangeController () - Other audio engine (%p) does not support sample rate %lu at %d bit %d channel(s). Failing.", 
	...
	{
		debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - Attempting normal format change request.", this);
	...
	if (kIOReturnSuccess != formatChangeReturnCode)
	{
		...
		debugIOLog ("! DJM01AudioDevice[%p]::formatChangeController () - This format change failed with error 0x%x.", this, result);
	}
	else
	{
		debugIOLog ("? DJM01AudioDevice[%p]::formatChangeController () - This format change was successful.", this);
		if	(		(kAUAFormatChangeNormal != result)
				&&	(kAUAFormatChangeForced != result))
		{
			debugIOLog ("! DJM01AudioDevice[%p]::formatChangeController () - Forced format change failed.", this);
			result = kAUAFormatChangeForceFailure;
		}
	}
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::formatChangeController (%p, %p, %p, %p) = 0x%x", this, audioEngine, audioStream, newFormat, newSampleRate, result);
	return result;
}

DJM01AudioEngine * DJM01AudioDevice::otherEngine (DJM01AudioEngine * thisEngine)
{
	SInt32	engineIndex;
	...
	otherAudioEngine = OSDynamicCast (DJM01AudioEngine, otherAudioEngineInfo->getObject (kEngine));
	FailIf (NULL == otherAudioEngine, Exit);
Exit:
	return otherAudioEngine;
}

IOReturn DJM01AudioDevice::getBothEngines (DJM01AudioEngine ** firstEngine, DJM01AudioEngine ** secondEngine)
{
	SInt32			engineIndex;
	...
	*firstEngine = OSDynamicCast (DJM01AudioEngine, firstAudioEngineInfo->getObject (kEngine));
...
void DJM01AudioDevice::attemptDeviceRecovery ()
{
	debugIOLog ("+ DJM01AudioDevice[%p]::attemptDeviceRecovery ()", this);
	
	FailIf (NULL == mControlInterface, Exit);
	/*
	debugIOLog ("? DJM01AudioDevice[%p]::attemptDeviceRecovery () - Issuing device reset.", this);
...
IOReturn DJM01AudioDevice::getClockSetting (UInt8 controlSelector, UInt8 unitID, UInt8 requestType, void * target, UInt16 length) {
    IOReturn							result;
	IOUSBDevRequestDesc					devReq;
	UInt16								theSetting;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
...
IOReturn DJM01AudioDevice::setClockSetting (UInt8 controlSelector, UInt8 unitID, UInt8 requestType, void * target, UInt16 length) {
    IOUSBDevRequestDesc					devReq;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
	IOReturn							result;
...
DeviceInactive:
	debugIOLog("? DJM01AudioDevice::setClockSourceSetting () - ERROR attempt to send a device request to an inactive device");
	goto Exit;
}

IOReturn DJM01AudioDevice::getNumClockSourceSamplingFrequencySubRanges (UInt8 unitID, UInt16 * numSubRanges) {
	struct {
	UInt16								wNumSubRanges;
...
IOReturn DJM01AudioDevice::getIndexedClockSourceSamplingFrequencySubRange (UInt8 unitID, SubRange32 * subRange, UInt16 subRangeIndex) {
	struct {
	UInt16								wNumSubRanges;
	SubRange32							SubRanges[1];
	}									rangeParameterBlock;
...
IOReturn DJM01AudioDevice::getCurClockSourceSamplingFrequency (UInt8 unitID, UInt32 * samplingFrequency, bool * validity) {
...
IOReturn DJM01AudioDevice::setCurClockSourceSamplingFrequency (UInt8 unitID, UInt32 samplingFrequency) {
	UInt32								frequency;
...
IOReturn DJM01AudioDevice::getCurClockSelector (UInt8 unitID, UInt8 * selector) {
	return getClockSetting (USBAUDIO_0200::CX_CLOCK_SELECTOR_CONTROL, unitID, USBAUDIO_0200::CUR, selector, 1);
}

IOReturn DJM01AudioDevice::setCurClockSelector (UInt8 unitID, UInt8 selector) {
	return setClockSetting (USBAUDIO_0200::CX_CLOCK_SELECTOR_CONTROL, unitID, USBAUDIO_0200::CUR, &selector, 1);
}

IOReturn DJM01AudioDevice::getCurClockMultiplier (UInt8 unitID, UInt16 * numerator, UInt16 * denominator) {
...
IOReturn DJM01AudioDevice::getNumSampleRatesForClockPath (UInt8 * numSampleRates, OSArray * clockPath) {
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt8								clockID;
	UInt8								subType;
...
IOReturn DJM01AudioDevice::getIndexedSampleRatesForClockPath (SubRange32 * sampleRates, OSArray * clockPath, UInt32 rangeIndex) {
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt32								clockIndex;
	UInt8								clockID;
...
IOReturn DJM01AudioDevice::getClockPathCurSampleRate (UInt32 * sampleRate, Boolean * validity, Boolean * isReadOnly, OSArray * clockPath) {		//	<rdar://6945472>
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt32								clockIndex;
	UInt8								clockID;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::getClockPathCurSampleRate", this);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::getClockPathCurSampleRate (%d, %d) = %d", this, clockRate, clockValidity, result);

	return result;
}

IOReturn DJM01AudioDevice::setClockPathCurSampleRate (UInt32 sampleRate, OSArray * clockPath, bool failIfReadOnly ) {	//	<rdar://6945472>
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt32								clockIndex;
	UInt8								clockID;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::setClockPathCurSampleRate (%d)", this, sampleRate);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::setClockPathCurSampleRate (%d) = %d", this, sampleRate, result);
...
OSArray * DJM01AudioDevice::findStreamsWithCommonSampleRates (OSArray * availableStreamList) {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::findStreamsWithCommonSampleRates (%p)", this, availableStreamList);
	...
	if ( NULL != result )
	{
		debugIOLog ("? DJM01AudioDevice[%p]::findStreamsWithCommonSampleRates (%p) - Found streams:", this, availableStreamList);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::findStreamsWithCommonSampleRates (%p) - result = %p", this, availableStreamList, result);

	return result;
}

//	<rdar://5131786>	Determine if the stream's sample rates are common with at least one other stream in the stream list.
bool DJM01AudioDevice::isSampleRateCommonWithAtLeastOneStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM01AudioDevice::isSampleRateCommonWithAllStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM01AudioDevice::streamsHaveCommonSampleRates (OSNumber * streamInterfaceNumberA, OSNumber * streamInterfaceNumberB) {
	
	bool		result = false;
	OSArray *	sampleRatesA = NULL;
	OSArray *	sampleRatesB = NULL;
	 
	debugIOLog ("+ DJM01AudioDevice[%p]::streamsHaveCommonSampleRates (%p, %p)", this, streamInterfaceNumberA, streamInterfaceNumberB);

	FailIf ( NULL == streamInterfaceNumberA, Exit );
	FailIf ( NULL == streamInterfaceNumberB, Exit );

	debugIOLog ("? DJM01AudioDevice[%p]::streamsHaveCommonSampleRates (%d, %d)", this, streamInterfaceNumberA->unsigned8BitValue (), streamInterfaceNumberB->unsigned8BitValue ());
...
Exit:
	debugIOLog ("+ DJM01AudioDevice[%p]::streamsHaveCommonClocks (%p, %p) - result = %d", this, streamInterfaceNumberA, streamInterfaceNumberB, result);

	return result;
}

//	<rdar://5131786>	Get the sample rates supported by a stream interface.
OSArray * DJM01AudioDevice::getSampleRatesFromStreamInterface (OSNumber * streamInterfaceNumber) {
...
void DJM01AudioDevice::mergeSampleRates (OSArray * thisArray, OSArray * otherArray)
{
...
bool DJM01AudioDevice::compareSampleRates (OSArray * sampleRatesA, OSArray * sampleRatesB)
{
...
//	The caller is responsible to release the returned stream array.
OSArray * DJM01AudioDevice::findStreamsWithCompatibleEndpoints (OSArray * availableStreamList) {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::findStreamsWithCompatibleEndpoints (%p)", this, availableStreamList);
	...
	if ( NULL != result )
	{
		debugIOLog ("? DJM01AudioDevice[%p]::findStreamsWithCompatibleEndpoints (%p) - Found streams:", this, availableStreamList);
		for ( UInt32 streamInterfaceIndex = 0; streamInterfaceIndex < result->getCount (); streamInterfaceIndex++ ) 
		{
			OSNumber * streamInterfaceNumber = OSDynamicCast (OSNumber, result->getObject ( streamInterfaceIndex ) );
			FailMessage ( NULL == streamInterfaceNumber );
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::findStreamsWithCompatibleEndpoints (%p) - result = %p", this, availableStreamList, result);

	return result;
}

//	<rdar://5131786>	Find streams with the specified direction & sync type in the available list.
OSArray * DJM01AudioDevice::findStreamsWithDirectionAndSyncType (OSArray * availableStreamList, UInt8 direction, UInt8 syncType ) {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::findStreamsWithDirectionAndSyncType (%p, %d, %d)", this, availableStreamList, direction, syncType);
	...
	if ( NULL != result )
	{
		debugIOLog ("- DJM01AudioDevice[%p]::findStreamsWithDirectionAndSyncType (%p, %d, %d) - Found streams:", this, availableStreamList, direction, syncType);
		for ( UInt32 streamInterfaceIndex = 0; streamInterfaceIndex < result->getCount (); streamInterfaceIndex++ ) 
		{
			OSNumber * streamInterfaceNumber = OSDynamicCast (OSNumber, result->getObject ( streamInterfaceIndex ) );
			FailMessage ( NULL == streamInterfaceNumber );
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::findStreamsWithDirectionAndSyncType (%p, %d, %d) - result = %p", this, availableStreamList, direction, syncType, result);

	return result;
}

//	<rdar://5131786>	Determine if the stream's endpoints are compatible with the specified direction & sync type.
bool DJM01AudioDevice::streamEndpointsHaveSpecifiedDirectionAndSyncType ( OSNumber * streamInterfaceNumber, UInt8 endpointDirection, UInt8 endpointSyncType ) {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::streamEndpointsHaveSpecifiedDirectionAndSyncType (%p, %d, %d)", this, streamInterfaceNumber, endpointDirection, endpointSyncType);

	FailIf ( NULL == streamInterfaceNumber, Exit );

	interfaceNum = streamInterfaceNumber->unsigned8BitValue ();
	
	debugIOLog ("? DJM01AudioDevice[%p]::streamEndpointsHaveSpecifiedDirectionAndSyncType (%d, %d, %d)", this, interfaceNum, endpointDirection, endpointSyncType);
...
Exit:
	...
	debugIOLog ("- DJM01AudioDevice[%p]::streamEndpointsHaveSpecifiedDirectionAndSyncType (%p, %d, %d) - result = %d", this, streamInterfaceNumber, endpointDirection, endpointSyncType, result);
...
//	The caller is responsible to release the returned stream array.
OSArray * DJM01AudioDevice::findStreamsWithCommonClocks (OSArray * availableStreamList) {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::findStreamsWithCommonClocks (%p)", this, availableStreamList);
	...
	if ( NULL != result )
	{
		debugIOLog ("? DJM01AudioDevice[%p]::findStreamsWithCommonClocks (%p) - Found streams:", this, availableStreamList);
		for ( UInt32 streamInterfaceIndex = 0; streamInterfaceIndex < result->getCount (); streamInterfaceIndex++ ) 
		{
			OSNumber * streamInterfaceNumber = OSDynamicCast (OSNumber, result->getObject ( streamInterfaceIndex ) );
			FailMessage ( NULL == streamInterfaceNumber );
...
Exit:	
	...
	debugIOLog ("- DJM01AudioDevice[%p]::findStreamsWithCommonClocks (%p) - result = %p", this, availableStreamList, result);

	return result;
}

//	<rdar://5131786>	Determine if the stream's clocks are common with at least one other stream in the stream list.
bool DJM01AudioDevice::isClockCommonWithAtLeastOneStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM01AudioDevice::isClockCommonWithAllStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM01AudioDevice::streamsHaveCommonClocks (OSNumber * streamInterfaceNumberA, OSNumber * streamInterfaceNumberB) {
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::streamsHaveCommonClocks (%p, %p)", this, streamInterfaceNumberA, streamInterfaceNumberB);

	FailIf ( NULL == streamInterfaceNumberA, Exit );
	FailIf ( NULL == streamInterfaceNumberB, Exit );
	
	debugIOLog ("? DJM01AudioDevice[%p]::streamsHaveCommonClocks (%d, %d)", this, streamInterfaceNumberA->unsigned8BitValue (), streamInterfaceNumberB->unsigned8BitValue ());
...
bool DJM01AudioDevice::isClockPathCrossed (OSNumber * streamInterfaceNumber, OSArray * otherClockPath, UInt32 sampleRate)
{
	UInt8		interfaceNum;
	UInt8		numAltSettings = 0;
	OSArray *	clockPathGroup;
	...
	debugIOLog ("+ DJM01AudioDevice[%p]::isClockPathCrossed (%p, %p, %lu)", this, streamInterfaceNumber, otherClockPath, sampleRate);
...
Exit:
	debugIOLog ("- DJM01AudioDevice[%p]::isClockPathCrossed (%p, %p, %lu) - result = %d", this, streamInterfaceNumber, otherClockPath, sampleRate, result);
