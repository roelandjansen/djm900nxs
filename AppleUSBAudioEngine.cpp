...
OSDefineMetaClassAndStructors(DJM01AudioEngine, IOAudioEngine)

#pragma mark -IOKit Routines-

void DJM01AudioEngine::free () {
	debugIOLog ("+ DJM01AudioEngine[%p]::free ()", this);
...
bool DJM01AudioEngine::init ( OSArray * streamInterfaceNumberArray ) {
	Boolean			result = false;

	debugIOLog("+ DJM01AudioEngine[%p]::init ()", this);
...
bool DJM01AudioEngine::requestTerminate (IOService * provider, IOOptionBits options) {
	bool						result = TRUE;

	debugIOLog ("+ DJM01AudioEngine[%p]::requestTerminate (%p, %x)", this, provider, options);
	...
	debugIOLog ("- DJM01AudioEngine[%p]::requestTerminate (%p, %x) = %d", this, provider, options, result);
	return result;
}

bool DJM01AudioEngine::start (IOService * provider, IOAudioDevice * device) {
	bool								resultCode;

	debugIOLog ("+ DJM01AudioEngine[%p]::start (%p)", this, provider);

	resultCode = FALSE;		
	
	FailIf (NULL == device, Exit);

	mUSBAudioDevice = OSDynamicCast (DJM01AudioDevice, device);
	//	<rdar://62775111>	Retain a reference to the DJM01AudioDevice object here so that it doesn't goes away
...
Exit:    
	...
	debugIOLog ("- DJM01AudioEngine[%p]::start (%p) = %d", this, provider, resultCode);
	return resultCode;
}
    
void DJM01AudioEngine::stop (IOService * provider) {
    debugIOLog("+ DJM01AudioEngine[%p]::stop (%p)", this, provider);
	...
	debugIOLog ( "- DJM01AudioEngine[%p]::stop (%p) - rc=%ld", this, provider, getRetainCount() );
}

bool DJM01AudioEngine::terminate (IOOptionBits options) {
	...
	debugIOLog ("+ DJM01AudioEngine[%p]::terminate ()", this);
...
bool DJM01AudioEngine::matchPropertyTable(OSDictionary * table, SInt32 *score)
{
	bool		returnValue = false;
	
	//debugIOLog ("+ DJM01AudioEngine[%p]::matchPropertyTable (%p, %p)", this, table, score);
	...
	//debugIOLog ("- DJM01AudioEngine[%p]::matchPropertyTable (%p, %p) = %d", this, table, score, returnValue);
...
void DJM01AudioEngine::registerService(IOOptionBits options)
{
	debugIOLog ("+ DJM01AudioEngine[%p]::registerService ( 0x%lx )", this, options);
...
void DJM01AudioEngine::openStreamInterfaces () {

	FailIf ( NULL == mIOAudioStreamArray, Exit );
	
	for ( UInt32 streamIndex = 0; streamIndex < mIOAudioStreamArray->getCount (); streamIndex++ )
	{
		DJM01AudioStream * audioStream = OSDynamicCast (DJM01AudioStream, mIOAudioStreamArray->getObject (streamIndex) );
...
void DJM01AudioEngine::closeStreamInterfaces () {
...
IOReturn DJM01AudioEngine::clipOutputSamples (const void *mixBuf, void *sampleBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream) {
	IOReturn				result;
	DJM01AudioStream *		usbAudioStream;			

	result = kIOReturnError;
	
	usbAudioStream = OSDynamicCast ( DJM01AudioStream, audioStream );
	...
	if (TRUE == streamFormat->fIsMixable) 
	{
		...
		result = clipAudioToOutputStream (mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat);
			...
			if (!mHaveClipped)
			{
				...
				// debugIOLog ("! DJM01AudioEngine::clipOutputSamples() - Sample frame %lu queued for USB frame %llu (current frame = %llu)", firstSampleFrame, usbAudioStream->getQueuedFrameForSample (firstSampleFrame), mUSBAudioDevice->getUSBFrameNumber());
			}
		...
		if (mUHCISupport)
		{
			...
			if ( start < alternateFrameSize )
			{
				...
				#if DEBUGUHCI
				debugIOLog( "? DJM01AudioEngine::clipOutputSamples () - firstSampleFrame = %d. Just copied %d bytes from %d to %d \n", firstSampleFrame, size, start, start + usbAudioStream->mSampleBufferSize );			
				#endif
			}
		} // UHCI support
	} 
...
IOReturn DJM01AudioEngine::controlledFormatChange (IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
	IOReturn							result;
	...
	debugIOLog ("+ DJM01AudioEngine[%p]::controlledFormatChange (%p, %p, %p)", this, audioStream, newFormat, newSampleRate);
	...
	for (UInt32 streamIndex = 0; streamIndex < mIOAudioStreamArray->getCount (); streamIndex++)
	{
		DJM01AudioStream * stream = OSDynamicCast (DJM01AudioStream, mIOAudioStreamArray->getObject (streamIndex) );
	...
	if (newFormat->fNumChannels != audioStream->format.fNumChannels) 
	{
		...
		debugIOLog ("? DJM01AudioEngine[%p]::controlledFormatChange () - Need to adjust channel controls (cur = %d, new = %d)", this, audioStream->format.fNumChannels, newFormat->fNumChannels);
	...
	if (newSampleRate) 
	{
		debugIOLog ("? DJM01AudioEngine[%p]::controlledFormatChange () - Changing sampling rate to %d", this, newSampleRate->whole);
	...
	debugIOLog("? DJM01AudioEngine[%p]::controlledFormatChange () - New numSamplesInBuffer = %d", this, numSamplesInBuffer );
	...
	debugIOLog ("? DJM01AudioEngine[%p]::controlledFormatChange () - Called setNumSampleFramesPerBuffer with %d", this, usbAudioStream->mSampleBufferSize / (usbAudioStream->mSampleSize ? usbAudioStream->mSampleSize : 1));
	debugIOLog ("? DJM01AudioEngine[%p]::controlledFormatChange () - newFormat->fNumChannels = %d, newFormat->fBitWidth %d", this, newFormat->fNumChannels, newFormat->fBitWidth);

	result = kIOReturnSuccess;

Exit:
	debugIOLog ("- DJM01AudioEngine[%p]::controlledFormatChange () = 0x%x", this, result);
    return result;
...
IOReturn DJM01AudioEngine::convertInputSamples (const void *sampleBuf,
                                                                void *destBuf,
...
#if DEBUGCONVERT
	debugIOLog ("+ DJM01AudioEngine::convertInputSamples (%p, %p, %lu, %lu, %p, %p)", sampleBuf, destBuf, firstSampleFrame, numSampleFrames, streamFormat, audioStream);
#endif
	...
	//	{
	//		debugIOLog ("! DJM01AudioEngine::convertInputSamples () - called before first time stamp!");
	//	}
...
#if DEBUGLOADING
				debugIOLog ("! DJM01AudioEngine::convertInputSamples () - Coalesce from convert %d bytes", lastSampleByte - usbAudioStream->mBufferOffset);
#endif
			} 
			else 
			{
...
#if DEBUGLOADING
				debugIOLog ("! DJM01AudioEngine::convertInputSamples () - Coalesce from convert %d bytes (wrapping)", numBytesToCoalesce);
#endif
			}
		}
...
Exit:	
	...
	{
		debugIOLog ( "! DJM01AudioEngine::convertInputSamples () = 0x%x", result );
	}
	
	return result;
}

IOReturn DJM01AudioEngine::eraseOutputSamples(const void *mixBuf, void *sampleBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream)
{	
	...
	if (mUHCISupport)
	{
		DJM01AudioStream * usbAudioStream = OSDynamicCast ( DJM01AudioStream, audioStream );
		FailIf ( NULL == usbAudioStream, Exit );
...
UInt32 DJM01AudioEngine::getCurrentSampleFrame () {
...
IOReturn DJM01AudioEngine::GetDefaultSampleRate (IOAudioSampleRate * sampleRate) {
	...
	debugIOLog ("+ DJM01AudioEngine[%p]::GetDefaultSampleRate ()", this);
	result = kIOReturnError;
	...
	if (!foundSampleRate)
	{
		...
		debugIOLog ("? DJM01AudioEngine[%p]::GetDefaultSampleRate () - Taking first available alternate setting (%d)", this, newAltSettingID);
	...
	debugIOLog ("? DJM01AudioEngine[%p]::GetDefaultSampleRate () - Default sample rate is %d", this, newSampleRate.whole);
	debugIOLog ("? DJM01AudioEngine[%p]::GetDefaultSampleRate () - Default alternate setting ID is %d", this, newAltSettingID);
	FailIf (0 == newSampleRate.whole, Exit);
	*sampleRate = newSampleRate;
	result = kIOReturnSuccess;
	
Exit:
	debugIOLog ("- DJM01AudioEngine[%p]::GetDefaultSampleRate (%lu) = 0x%x", this, sampleRate->whole, result);
	return result;
}

OSString * DJM01AudioEngine::getGlobalUniqueID () {
    char *						uniqueIDStr;
    OSString *					localID;
    OSString *					uniqueID;
	...
	uniqueIDSize += strlen ("DJM01AudioEngine");
	...
	if ( nameObject )
	{
		...
		{
			debugIOLog ( "? DJM01AudioEngine[%p]::getGlobalUniqueID () - Retrieved vendor name %s from registry", this, nameString->getCStringNoCopy () );
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "? DJM01AudioEngine[%p]::getGlobalUniqueID () - Retrieved product name %s from registry", this, nameString->getCStringNoCopy () );
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "? DJM01AudioEngine[%p]::getGlobalUniqueID () - Retrieved serial number %s from registry", this, nameString->getCStringNoCopy () );
	...
	else 
	{
		...
		debugIOLog ("? DJM01AudioEngine[%p]::getGlobalUniqueID () - Device has a serial number = %s", this, serialNumberString);
	...
	if (NULL != uniqueIDStr) 
	{
		...
		{
			sprintf (uniqueIDStr, "DJM01AudioEngine:%s:%s:%s", manufacturerString, productString, locationIDString);
		} 
		else 
		{
			sprintf (uniqueIDStr, "DJM01AudioEngine:%s:%s:%s", manufacturerString, productString, serialNumberString);
		}
		...
		debugIOLog ("DJM01AudioEngine[%p]::getGlobalUniqueID () - getGlobalUniqueID = %s", this, uniqueIDStr);
...
//--------------------------------------------------------------------------------
bool DJM01AudioEngine::initHardware (IOService *provider) {
...
/*		propertiesReadyObject = NULL;
		...
		{
			propertiesReadyObject = streamInterface->getProperty ( "DJM01AudioStreamPropertiesReady" );
			if ( propertiesReadyObject )
			{
				debugIOLog ( "! DJM01AudioStream[%p]::initHardware () - DJM01AudioStreamPropertiesReady found for stream #%d", this, streamInterfaceNumber->unsigned8BitValue () );
				break;
			}
			debugIOLog ( "! DJM01AudioStream[%p]::initHardware () - DJM01AudioStreamPropertiesReady NOT found for stream #%d, sleeping 10 ms...", this, streamInterfaceNumber->unsigned8BitValue () );
			IOSleep ( 10 );
			FailIf ( mUSBAudioDevice->isInactive (), Exit );
		}
*/				
		// Instantiate one AppleUSBStream per IOUSBInterface.
		DJM01AudioStream  * audioStream = OSTypeAlloc ( DJM01AudioStream );
		FailIf (NULL == audioStream, Exit);
		...
		if ( mStreamInterfaceNumberArray->getCount () > 1 )
		{
			...
			if ( nameObject )
			{
				if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
				{
					debugIOLog ( "! DJM01AudioStream[%p]::initHardware () - Retrieved product name %s", this, nameString->getCStringNoCopy () );
...
//		FailWithAction (!audioStream->initWithAudioEngine (mUSBAudioDevice, this, streamInterface, sampleRate, streamDescription), audioStream->release(), Exit);
	...
	if ( ( NULL != mMainOutputStream ) && ( kAdaptiveSyncType == mMainOutputStream->getSyncType () ) )
	{
		...
		debugIOLog ("? DJM01AudioEngine[%p]::initHardware (%p) - Main output stream (%p) is adaptive and designated as master", this, provider, mMainOutputStream);
...
//	syncInputCompensation = true;
	...
	debugIOLog ("? DJM01AudioEngine[%p]::initHardware (%p) - Compensate for output synchronization: %d", this, provider, syncOutputCompensation);
	...
	debugIOLog ("? DJM01AudioEngine[%p]::initHardware (%p) - Compensate for input synchronization: %d", this, provider, syncInputCompensation);
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "! DJM01AudioEngine[%p]::initHardware () - Retrieved product name %s", this, nameString->getCStringNoCopy () );
	...
	if (NULL != coreAudioPluginPathString)
	{
		debugIOLog ( "! DJM01AudioEngine[%p]::initHardware () - Retrieved CoreAudio plugin path %s", this, coreAudioPluginPathString->getCStringNoCopy () );
...
Exit:
	// <rdar://problem/6576824> DJM01Audio fails to unload properly
	if ( !resultBool )
	{
    ...
    debugIOLog("- DJM01AudioEngine[%p]::initHardware(%p), resultCode = %x, resultBool = %d", this, provider, resultCode, resultBool);
    return resultBool;
}

//--------------------------------------------------------------------------------
// <rdar://7558825> Get the system clock domain
UInt32 DJM01AudioEngine::getSystemClockDomain ( void )
{
...
OSString * DJM01AudioEngine::getChannelNameString ( UInt8 unitID, UInt8 channelNum )
{
	DJMConfigurationDictionary *		configDictionary = NULL;
	...
	debugIOLog ("? DJM01AudioEngine[%p]::getChannelNameString (%d, %d) - Audio cluster descriptor { %d, %d, %d }", this, unitID, channelNum, clusterDescriptor.bNrChannels, clusterDescriptor.bmChannelConfig, clusterDescriptor.iChannelNames);
	...
	if ( ( numPreDefinedChannels != channelNum ) && ( channelNum <= clusterDescriptor.bNrChannels ) )
	{
		...
		if ( 0 != clusterDescriptor.iChannelNames )
		{
			...
			debugIOLog ("? DJM01AudioDevice::getChannelNameString (%d, %d) - stringIndex = %d, stringBuffer = %s, result = 0%x", unitID, channelNum, clusterDescriptor.iChannelNames + channelNum - 1 - numPreDefinedChannels, stringBuffer, result);
		}
	}
	
	if ( kIOReturnSuccess == result )
	{
		debugIOLog ("? DJM01AudioDevice::getChannelNameString () - terminalID = %d, channelNum = %d, stringBuffer = %s", unitID, channelNum, stringBuffer);
...
void DJM01AudioEngine::updateChannelNames ( OSArray* thePath, UInt8 interfaceNum, UInt8 altSettingNum ) {
    ...
    debugIOLog ("+ DJM01AudioEngine[%p]::updateChannelNames (%d, %d)", this, interfaceNum, altSettingNum);
	...
	for ( UInt32 streamIndex = 0; streamIndex < mIOAudioStreamArray->getCount (); streamIndex++ )
	{
		...
		{
			debugIOLog ("? DJM01AudioEngine[%p]::updateChannelNames (%d, %d) - Found audio stream = %p", this, interfaceNum, altSettingNum, audioStream);
			...
			debugIOLog ("? DJM01AudioEngine[%p]::updateChannelNames (%d, %d, %d) - Audio cluster descriptor { %d, %d, %d }", this, unitID, interfaceNum, altSettingNum, clusterDescriptor.bNrChannels, clusterDescriptor.bmChannelConfig, clusterDescriptor.iChannelNames);
			...
			for ( UInt8 channelIndex = 0; channelIndex < clusterDescriptor.bNrChannels; channelIndex++ )
			{
				...
				if ( NULL != nameString )
				{
					...
					debugIOLog ("? DJM01AudioEngine[%p]::updateChannelNames (%d, %d, %d) - Setting %s to %s", this, unitID, interfaceNum, altSettingNum, keyString, nameString->getCStringNoCopy());
...
Exit:
    debugIOLog ("- DJM01AudioEngine[%p]::updateChannelNames (%d, %d, %d)", this, unitID, interfaceNum, altSettingNum);
	return;
}

char * DJM01AudioEngine::ChannelConfigString (UInt8 channel) 
{
...
void DJM01AudioEngine::registerPlugin (DJM01AudioPlugin * thePlugin) {
	mPlugin = thePlugin;
...
void DJM01AudioEngine::pluginLoaded (DJM01AudioEngine * usbAudioEngineObject) {
	IOReturn							result;
...
IOReturn DJM01AudioEngine::pluginDeviceRequest (IOUSBDevRequest * request, IOUSBCompletion * completion) {
	IOReturn						result;
...
void DJM01AudioEngine::pluginSetConfigurationApp (const char * bundleID) {
	if (bundleID) 
	{
...
IOReturn DJM01AudioEngine::performAudioEngineStart () {
    IOReturn				resultCode;
	UInt32					usbFramesToDelay = 0;
	UInt32					lockDelayFrames = 0;
	UInt64					currentUSBFrame = 0;
	UInt32					streamIndex;
	DJM01AudioStream *		audioStream;
	
    debugIOLog ("+ DJM01AudioEngine[%p]::performAudioEngineStart ()", this);
	...
	if (mUSBAudioDevice->mAnchorTime.n == 0)
	{
		...
		debugIOLog ("! DJM01AudioEngine[%p]::performAudioEngineStart () - Getting an anchor for the first timestamp.", this);
	...
	{
		audioStream = OSDynamicCast (DJM01AudioStream, mIOAudioStreamArray->getObject (streamIndex) );
	...
	if (!mUSBStreamRunning) 
	{
		...
		// Added an offset to compensate for the time needed for some of the operation in DJM01AudioStream::startUSBStream();
...
Exit:
	...
	{
		debugIOLog ("! DJM01AudioEngine[%p]::performAudioEngineStart () - NOT started, error = 0x%x", this, resultCode);
				...
				&&	( ! mUSBAudioDevice->recoveryRequested ()))
		{
			...
			if (mUSBAudioDevice)
			{
				debugIOLog ("! DJM01AudioEngine[%p]::performAudioEngineStart () - Device not responding! Requesting a recovery attempt.");
...
IOReturn DJM01AudioEngine::performAudioEngineStop() {
	UInt32					streamIndex;
	DJM01AudioStream *		audioStream;

    debugIOLog("+ DJM01AudioEngine[%p]::performAudioEngineStop ()", this);
    ...
    debugIOLog("? DJM01AudioEngine[%p]::performAudioEngineStop() - stopped", this);
...
IOReturn DJM01AudioEngine::performFormatChange (IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate) {
    IOReturn							result;
	UInt32								controllerResult;
	bool								streamIsRunning = false;

	debugIOLog ("+ DJM01AudioEngine[%p]::performFormatChange (%p, %p, %p)", this, audioStream, newFormat, newSampleRate);
	result = kIOReturnError;
	FailIf (NULL == mUSBAudioDevice, Exit);
	...
	switch (controllerResult)
	{
		...
		case kAUAFormatChangeForced:
			debugIOLog ("? DJM01AudioEngine[%p]::performFormatChange () - This request was forced.");
			result = kIOReturnSuccess;
			break;
		case kAUAFormatChangeForceFailure:
			debugIOLog ("! DJM01AudioEngine[%p]::performFormatChange () - Force of this request was attempted but failed.");
			result = kIOReturnSuccess;
			break;
		case kAUAFormatChangeError:
		default:
			debugIOLog ("! DJM01AudioEngine[%p]::performFormatChange () - Error encountered.");
			result = kIOReturnError;
	}
...
Exit:
	...
	debugIOLog ("- DJM01AudioEngine[%p]::performFormatChange () = 0x%x", this, result);
    return result;
}

void DJM01AudioEngine::resetClipPosition (IOAudioStream *audioStream, UInt32 clipSampleFrame) {
	DJM01AudioStream *		usbAudioStream;			
...
#if defined(__i386__)
void DJM01AudioEngine::takeTimeStamp (bool incrementLoopCount, UInt64 *timestamp)
#else
void DJM01AudioEngine::takeTimeStamp (bool incrementLoopCount, AbsoluteTime *timestamp)
#endif
{
	#if LOGTIMESTAMPS
	UInt64	time_nanos;
	...
	if (true)
	{
		debugIOLog ("? DJM01AudioEngine[%p]::takeTimeStamp (%d, %p) = %llu ns", this, timestamp, time_nanos);
	}
	#endif
	super::takeTimeStamp (incrementLoopCount, (AbsoluteTime *)timestamp);
}

bool DJM01AudioEngine::willTerminate (IOService * provider, IOOptionBits options) {
	debugIOLog ("+ DJM01AudioEngine[%p]::willTerminate (%p)", this, provider);	
	...
	debugIOLog ("- DJM01AudioEngine[%p]::willTerminate ()", this);

	return super::willTerminate (provider, options);
}

// <rdar://problem/6021475> DJM01Audio: Status Interrupt Endpoint support
OSSet * DJM01AudioEngine::copyDefaultAudioControls ( void )
{
...
//	<rdar://5811247>
IOReturn DJM01AudioEngine::doClockSelectorSetup (UInt8 interfaceNum, UInt8 altSettingNum, UInt32 sampleRate) {
	...
	debugIOLog ("+ DJM01AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d )", this, interfaceNum, altSettingNum, sampleRate);
	...
	{
		debugIOLog ("? DJM01AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) - clockSelectorID = %d, clockPathIndex = %d", this, interfaceNum, altSettingNum, sampleRate, clockSelectorID, clockPathIndex);
		...
		for ( UInt32 pathIndex = 0; pathIndex < clockPathGroup->getCount (); pathIndex++ )
		{
			...
			debugIOLog ("? DJM01AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) - %d: clockID = %d, subType = %d", this, interfaceNum, altSettingNum, sampleRate, pathIndex, clockID, subType);
	...
	debugIOLog ("? DJM01AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) - Should poll = %d", this, interfaceNum, altSettingNum, sampleRate, mShouldPollClockStatus);
	#endif // POLLCLOCKSTATUS
	
	result = kIOReturnSuccess;
Exit:

	debugIOLog ("- DJM01AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) = 0x%x", this, interfaceNum, altSettingNum, sampleRate, result);

	return result;
}

//	<rdar://5811247>
IOReturn DJM01AudioEngine::controlChangedHandler (OSObject * target, IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn						result;
	DJM01AudioEngine *				self;

	result = kIOReturnError;

	self = OSDynamicCast (DJM01AudioEngine, target);
...
//	<rdar://5811247>
IOReturn DJM01AudioEngine::protectedControlChangedHandler (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
...
//	<rdar://5811247>
IOReturn DJM01AudioEngine::doClockSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	UInt8						clockSourceID;
	...
	debugIOLog ("+ DJM01AudioEngine[%p]::doClockSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
	...
	if ( oldValue != newValue )
	{
		...
		debugIOLog ("? DJM01AudioEngine[%p]::doClockSelectorChange( %p, 0x%x, 0x%x ) - result = 0x%x, sample rate = %d, clockValidity = %d", this, audioControl, oldValue, newValue, result, newSampleRate.whole, clockValidity);
	...
	debugIOLog ("- DJM01AudioEngine[%p]::doClockSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, result);
...
//	<rdar://5811247>
IOReturn DJM01AudioEngine::republishAvailableFormats () {
	DJMConfigurationDictionary *		configDictionary = NULL;
...
bool DJM01AudioEngine::determineMacSyncMode ( UInt8 clockID )
{
	...
	DJMConfigurationDictionary *		configDictionary = NULL;
	UInt8								controlInterfaceNum;
	DJM01AudioStream *					masterStream = NULL;	// <rdar://7558825>
	bool								macSyncMode = false;

	debugIOLog ( "+ DJM01AudioEngine[%p]::determineMacSyncMode( %d )", this, clockID );
...
Exit:
	debugIOLog ( "- DJM01AudioEngine[%p]::determineMacSyncMode( %d ) = %d", this, clockID, macSyncMode );
	
	return macSyncMode;
}

//	<rdar://6945472>
IOAudioSampleRate DJM01AudioEngine::getCurrentClockPathSampleRate ( void )
{
...
//	<rdar://5811247>
void DJM01AudioEngine::updateClockStatus ( UInt8 clockID )
{
	UInt32				clockRate;
		...
		if ( kIOReturnSuccess == mUSBAudioDevice->getCurClockSourceSamplingFrequency ( clockID, &clockRate, &clockValidity ) )
		{
			if ( NULL != mClockSelectorControl )
			{
								...
								if ( NULL != selectionNumber )
								{
									...
									if ( selection != currentValue )
									{
										...
										if ( kIOReturnSuccess == mUSBAudioDevice->getCurClockSourceSamplingFrequency ( clockID, &clockRate, &clockValidity ) && ( clockValidity ) )
										{
											...
											debugIOLog ( "? DJM01AudioDevice[%p]::updateClockStatus () - Switch over to selection = 0x%x", this, selection );
				...
				else 
				{
					...
					debugIOLog ( "? DJM01AudioDevice[%p]::updateClockStatus () - Current sample rate = %u, new sample rate = %u", this, mCurSampleRate.whole, sampleRate.whole );
					...
					if ( mCurSampleRate.whole != sampleRate.whole )
					{
						...
						debugIOLog ( "? DJM01AudioDevice[%p]::updateClockStatus () - Re-publishing audio formats", this );

						// Re-publish all the streams supported sample rates & formats.
						republishAvailableFormats ();
						
						debugIOLog ( "? DJM01AudioDevice[%p]::updateClockStatus () - Performing format - sample rate changes", this );

						// Do the sample rate change.
						FailMessage ( kIOReturnSuccess != performFormatChange ( NULL, NULL, &sampleRate ) );
						
						debugIOLog ( "? DJM01AudioDevice[%p]::updateClockStatus () - Informing CoreAudio of the sample rate change", this );
			...
			{
				debugIOLog ( "? DJM01AudioDevice[%p]::updateClockStatus () - No clock selector present - update audio formats if necessary", this );
...
//	<rdar://5811247>
void DJM01AudioEngine::runPolledTask () {
